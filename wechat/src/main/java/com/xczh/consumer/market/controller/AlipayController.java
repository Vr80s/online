...
//        AlipayTradeWapPayRequest alipa= payBizRun.(request,response);
        ...
        if(StringUtils.isNotBlank(request.getParameter("formIsWechat"))){
            ...
            if(cCount>1) { //多个课程 跳到订单列表
                ...
                OnlineCourse payCourse = cList.get(0);
                alipay_request.setReturnUrl(returnOpenidUri + "/xcviews/html/personalfor.html?userId=" + onlineOrder.getUserId()+"&id="+payCourse.getId());
            }
        }else{
            cList= ((OnlineOrder)onlineOrderService.getOrderAndCourseInfoByOrderNo(orderNo).getResultObject()).getAllCourse();
            int cCount=cList.size();
            if(cCount>1){ //多个课程 跳到订单列表
                ...
                OnlineCourse payCourse=  cList.get(0);
                    alipay_request.setReturnUrl(returnOpenidUri+"/xcviews/html/personalfor.html?userId="+onlineOrder.getUserId()+"&id="+payCourse.getId());//调到填写报名信息页
                }
        }
...
//        }
    ...
    public ResponseObject getOrderStr(HttpServletRequest req,
                                      HttpServletResponse res) throws SQLException, UnsupportedEncodingException {
...
//SDK已经封装掉了公共参数，这里只需要传入业务参数。以下方法为sdk的model入参方式(model和biz_content同时存在的情况下取biz_content)。
      ...
      //  model.setBody("");
        ...
        model.setSubject("购买课程-【"+onlineOrderService.getCourseNames(onlineOrder.getOrderNo())+"】");
...
//SDK已经封装掉了公共参数，这里只需要传入业务参数。以下方法为sdk的model入参方式(model和biz_content同时存在的情况下取biz_content)。
        ...
        if(verify_result){//验证成功
           ...
           // out.clear();
            ...
            response.getWriter().println("" +
                    "" +
                    "<script>window.open('"+weburl+"/web/html/myStudyCenter.html','_self')</script>");
    ...
    public void alipayNotify(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        //计算得出通知验证结果
        //boolean AlipaySignature.rsaCheckV1(Map<String, String> params, String publicKey, String charset, String sign_type)
        boolean verify_result = AlipaySignature.rsaCheckV1(para, alipayConfig.ALIPAY_PUBLIC_KEY, alipayConfig.CHARSET, "RSA2");
        ...
        if(verify_result){//验证成功
            ...
            alipayPaymentRecordH5.setBuyerPayAmount(para.get("buyer_pay_amount"));
            ...
            if(trade_status.equals("TRADE_SUCCESS")){
                ...
                if(StringUtils.isNotBlank(alipayPaymentRecordH5.getPassbackParams())){
                    ...
                    if(ppbt.equals("1")){ //打赏
                    RewardParamVo rpv=JSONObject.parseObject(alipayPaymentRecordH5.getPassbackParams(),RewardParamVo.class);
                      //  RewardStatement rs=new RewardStatement();
                        ...
                        rs.setPrice(new Double(alipayPaymentRecordH5.getTotalAmount()));
                        rs.setChannel(1);
                       // rewardService.insert(rs);1
                        ...
                        System.out.println("回调数据包："+alipayPaymentRecordH5.getPassbackParams());
                        ...
                        System.out.println("充值回调数据包："+alipayPaymentRecordH5.getPassbackParams());
                        alipayPaymentRecordH5.setUserId(( JSONObject.parseObject(alipayPaymentRecordH5.getPassbackParams()).get("userId").toString()));
                        alipayPaymentRecordH5Service.insert(alipayPaymentRecordH5);
...
//                        userCoinIncrease.setChangeType(0);
     ...
     */
    private boolean httpOnline(String order_no,String transaction_id) throws Exception{
        //TODO
        String s = "out_trade_no="+order_no+"&result_code=SUCCESS"+"&transaction_id="+transaction_id+"&key="+onlinekey;
        ...
        String resXml =
                "<xml>"
                        ...
                        + " </xml> ";
        //请求web端的方法
        String msg = HttpUtil.sendDataRequest(pcUrl+"/web/pay_notify_alipay", "application/xml",
                resXml.toString().getBytes());
