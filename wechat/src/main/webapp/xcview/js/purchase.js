...
requestService("/xczh/order/getByOrderId",{
	...
	var currentPriceXMB = data.resultObject.allCourse[0].currentPrice*10;
...
function getgetRedirectUrl(allCourse,falg){
	...
	if(allCourse.length>1){  //如果此订单中有多个订单，返回到我的订单页面
	    ...
	    if(!c.collection){
	    	 redirectUrl="/xcview/html/live_album.html?courseId="+c.id;
	    }else{
	    	if(c.type == 1 || c.type == 2){ //直播状态1 视频 2 音频 3 直播 4下线班 
	   	 	    redirectUrl="/xcview/html/live_audio.html?courseId="+c.id;
	   	    }else if(c.type == 3){
	   	    	redirectUrl="/xcview/html/live_play.html?course_id="+c.id;
	   	    }else if(c.type == 4){
	   	    	redirectUrl="/xcview/html/live_class.html?course_id="+c.id;
	   	    }
	    }
	    return redirectUrl;
	}
