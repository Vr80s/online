...
function giftShow(gift,f){
	if(gift.messageType==1){
		...
		gif[f]= $("<li class='animation' style='position: absolute;bottom: "+bottom+"rem;'><div class='animation_div'><div class='animation_name'><p class='animation_name_p1'>"+gift.senderInfo.userName+"</p><p class='animation_name_p2'>送 "+gift.giftInfo.name+"</p></div><div class='animation_gift'><img src='"+gift.giftInfo.smallimgPath+"' alt='' /></div><div class='animation_span'>X<span class=addnum"+f+">1</span></div></div></li>");
        try {
...
$(document).ready(function() {
    ...
    $(".balance_send").click(function() {
        ...
        if(connected) {
		   ...
		   requestService("/bxg/gift/sendGift", msgJson, function(data) {
    ...
    function sendMsg(data){
		...
		var msg = $msg({
			...
			from: jid,
			type: 'groupchat'
//		}).c("body", null, data.giver+"yuxin"+data.giftName+"yuxin"+data.count);
