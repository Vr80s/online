 ...
 * FileId: dV0l5ecUkajWOb548TU23HFFSZWlxNCb
 */
...
public class CartServiceImpl extends BaseServiceImpl<Cart, Long> implements CartService {
	...
	public List<CartItem> getCartItemListByIds(Long[] ids){
		List<CartItem> cartItemListByIds = cartItemDao.getCartItemListByIds(ids);
		return cartItemListByIds;
	}

	@Override
	public void deleteByIds(String cartItemIds) {
		List<CartItem> cartItemList = this.getCartItemListByIds(cartItemIds.split(","));
		for (CartItem cartItem:cartItemList){
			this.remove(cartItem.getCart(),cartItem.getSku());
		}
	}

	@Override
	public List<CartItem> getCartItemListByIds(String[] idsStr) {
		Long[] ids = new Long[idsStr.length];
		for (int i = 0; i < idsStr.length; i++) {
			ids[i] = Long.valueOf(idsStr[i]);
		}
		return this.getCartItemListByIds(ids);
	}
