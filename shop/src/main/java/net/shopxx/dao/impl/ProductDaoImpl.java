...
public class ProductDaoImpl extends BaseDaoImpl<Product, Long> implements ProductDao {
	...
	@Override
	public List<Product> findPageXc(GoodsPageParams goodsPageParams, GoodsPageParams.OrderType orderType) {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<Product> findPageKayWordXc(GoodsPageParams goodsPageParams, GoodsPageParams.OrderType orderType) {
		
		FullTextEntityManager fullTextEntityManager = Search.getFullTextEntityManager(entityManager);
		QueryBuilder queryBuilder = fullTextEntityManager.getSearchFactory().buildQueryBuilder()
				.forEntity(Product.class).get();
		
		Query snPhraseQuery = queryBuilder.phrase().onField("sn").sentence(goodsPageParams.getKeyWord()).createQuery();
		BooleanJunction<?> subJunction = queryBuilder.bool().should(snPhraseQuery);
		
		Query namePhraseQuery = queryBuilder.phrase().withSlop(3).onField("name").sentence(goodsPageParams.getKeyWord()).createQuery();
		Query keywordFuzzyQuery = queryBuilder.keyword().fuzzy().onField("keyword").matching(goodsPageParams.getKeyWord()).createQuery();
