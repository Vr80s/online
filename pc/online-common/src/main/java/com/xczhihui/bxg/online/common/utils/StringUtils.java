...
public class StringUtils {
    private static Pattern SRC_PATTERN = Pattern.compile("(?:src=\"?)(.*?)\"?\\s");
     ...
     *
     * @param text      富文本
     * @param filterImg 提取的图片特征
     * @return 图片列表
     */
    public static List<String> matchSrc(String text, String filterImg) {
        Matcher matcher = SRC_PATTERN.matcher(text);
        List<String> list = new ArrayList<>();
        while (matcher.find()) {
            String imgPath = matcher.group(1);
            if (imgPath.contains(filterImg)) {
                list.add(imgPath);
            }
            if (list.size() >= 3) {
                break;
