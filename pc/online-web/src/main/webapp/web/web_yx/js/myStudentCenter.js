...
window.onload = function() {
	...
	var myorder =
		...
		'<div class="or6"><a href="/web/{{$value.order_no}}/findOrderByOrderNo?orderId={{$value.id}}" target="_blank">去支付</a></div>' +
		'{{/if}}' +
	...
	var work1 =
		...
		'{{if $value.status==0}}' +
		'<span class="tool" data-homeId="{{$value.homework_id}}" data-studentId="{{$value.student_id}}"><em>开始练习</em></span>' +
		'{{/if}}' +
		'{{if $value.status==1}}' +
		'<span class="tool" data-homeId="{{$value.homework_id}}" data-studentId="{{$value.student_id}}"><em>继续练习</em></span>' +
		'{{/if}}' +
		'</li>' +
		'{{/each}}';
	//历史作业内容
	var work2 =
		...
		'{{/if}}' +
		'<span class="tool" data-homeId="{{$value.homework_id}}" data-studentId="{{$value.student_id}}"><em>查看练习</em></span>' +
		'</li>' +
		'{{/each}}';
	//	下级用户
	var tongji =
	...
	RequestService("/online/user/isAlive", "GET", null, function(data) { ///online/user/isAlive
		if(data.success === true) {
			...
			if(data.resultObject.shareCode == null || data.resultObject.shareCode == "") {
				...
				var aurl = encodeURIComponent("/web/html/login.html?ways=register");
				$(".myshare .share_link .cont .cont2 .link span").html("http://"+window.location.hostname + "/share?usercode=" + acode + "&url=" + aurl);
			}
			//此处同步判断结束
