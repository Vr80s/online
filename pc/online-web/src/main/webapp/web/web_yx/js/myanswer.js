...
//	"<div class='pointer'></div>" +
	...
	"<div class='buttonbar'>" +
	"<div class='issue btn btn-jiejue' data-type='0'  data-name='issue'>未解决</div>" +
	"<div class='resolve btn' data-type='2'  data-name='issue'>已解决</div>" +
	"</div>" +
	...
	"<div class='buttonbar'>" +
	"<div class='issue btn  btn-jiejue' data-type='false' data-name='answer'>未采纳</div>" +
	"<div class='resolve btn' data-type='true'  data-name='answer'>已采纳</div>" +
	"</div>" +
	...
	"<div class='buttonbar'>" +
	"<div class='issue btn btn-jiejue' data-type='0' data-name='favorite'>未解决</div>" +
	"<div class='resolve btn' data-type='2'  data-name='favorite'>已解决</div>" +
	"</div>" +
...
var issue =
	...
	"<p class='time'>" +
	"{{#tags($issue.tags)}}" +
//	"<span style='margin-right:10px;color:#e4e4e4'>|</span>" +
	"<span style='' class='border'>浏览数({{$issue.browse_sum}})</span>" +
	"<span style='margin:0 10px'>{{#timeTypeChange($issue.create_time)}}</span>" +
	"</p>" +
	"</div>" +
	...
	"<div class='baoliuTip'><img src='../images/personcenter/baoliuTip.png' class='jian'>用户在熊猫中医上发表的全部原创内容（包括但不限于提问，问答和评论），著作权均归用户本人所有。用户可授权第三方以任何方式使用，不需要得到熊猫中医的同意。</div>" +
	"{{/if}}" +
	"</div>" +
	"</div>" +
	"</div>" +
...
var collect =
	...
	"<span class='from' data-mentid='{{$issue.ment_id}}'>来自学科：<span class='fromxueke'>{{$issue.name}}</span></span>" +
	'<div class="back">'+
	"<div class='qx' data-qusId='{{$issue.id}}'><span class='qx1'>x</span><span class='qx2'>取消收藏</span></div>" +
...
//	"<span style='margin-right:10px;color:#e4e4e4'>|</span>" +
	"<span style='' class='border'>浏览数({{$issue.browse_sum}})</span>" +
	"<span style='margin-right:10px'>{{#timeTypeChange($issue.create_time)}}</span>" +
	"</p>" +
	"</div>" +
	/*<!--回复--->*/
	"{{if $issue.answer_sum!=0&&$issue.askAnswerVo!=null}}" +
...
function createPage() {
	...
	$(".view-content-content").html(template.compile(cTemplateIndex));
	...
	$(".btn-item").click(function() {
		$(this).addClass("color2cb").siblings().removeClass("color2cb");
			if($(this).attr("data-type") == "issue") {
				...
				var aleft = $(this).offset().left - $(".view-content-content").offset().left;
		...
		RequestService(url, "get", param, function(data) {
			if(data.resultObject.items == "") {
				...
				if(data.resultObject.totalCount == 0) { //无数据
					$(".container-" + module).css("display", "none");
	...
	function shenglve() {
		...
		$(".all").each(function() {
			...
			if($dot5.height() >40) {
				$dot5.css("height", "40");
				$dot5.append('<span class="qq" style="margin-right:50px"><a class="toggle" href="#" ><span class="open">显示全部</span><span class="rrclose">收起</span></a></span>');
			}
						...
						if(data.success == true) {
							if($(".container-favorite .search-item").length <= 0) {
								...
								var emptyDefaul =
									...
									"<div class='no-title'>暂无数据</div>" +
									"</div>";
								$(".container-favorite").css("display", "none");
								$(".view-content-notbodys").html(emptyDefaul);
								$(".pages").css("display", "none");
							};
						}
