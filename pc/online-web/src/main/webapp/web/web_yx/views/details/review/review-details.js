/**
 *   复习
 *    http://onlineweb.ixincheng.com:58000/web/web_yx/views/index.html#/review/details/1e9ccf69d35d41f3ace0b1603beb768b
 */
define(['template',
		'jquery',
		'layui',
		'layer',
		'laypage',
		'text!tplUrl/details/review/review-details.html',
			...
			function ReviewDetails( redb ){
				...
				this.interaction = function(){
					...
					$(".J-request").unbind().on('click', function(){
						...
						// 获取当前是否可以播放
						onlyshow = $(this).data('only');

						if ( !onlyshow ) {
							if(treeId != undefined ) {
// 切换为视频时，播放从当前的播放点开始播放
								var findReviewStudentPointDB = findReviewStudentPoint( BATHPATH ,loginName, reviewId, treeId);
								if(!findReviewStudentPointDB.success) {
									layer.alert( findReviewStudentPointDB.errorMessage ||"服务器异常!" );
								};
								var Time = findReviewStudentPointDB.resultObject.watchedDuration;
							...
							// 如果当前不能点击就置为空
							$('#video-view,#audio-view,#ppt-view,#plan-view,.pic-bg, #text-view,#download-view').html('<div class="nodeta">当前没有资源</div>');
						}
		...
		//获取复习树节点
		var getStartTree = function(thirdUrl,loginName, reviewId) {
			return common.requestService('../../../../api/callThirdPost','post', {
				thirdUrl: thirdUrl + '/bxg_stu/review/start',
				loginName: loginName,
				reviewId: reviewId
			});
		};
		...
		// 更新进度
		var updateProgress = function(thirdUrl,loginName, reviewId, pointId, watchedDuration) {
			return common.requestService('../../../../api/callThirdPost','post', {
				thirdUrl: thirdUrl + '/bxg_stu/review/updateProgress',
				loginName: loginName,
				reviewId: reviewId,
				pointId: pointId,
		...
		//  获取某个知识点视频的已看时长 -- 复习
		var findReviewStudentPoint = function(thirdUrl,loginName, reviewId, pointId) {
			return common.requestService('../../../../api/callThirdPost','post', {
				thirdUrl: thirdUrl + '/bxg_stu/review/findReviewStudentPoint',
				loginName: loginName,
				reviewId: reviewId,
				pointId: pointId
			});
