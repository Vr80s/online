window.onload = function() {
	...
	var coursetable =
		...
		'{{/each}}';
	//作业模块当前作业
	template.helper('curJindu', function(a, b) {
		var num = 0;
		if(b == 0) {
			num = 0;
		} else {
			num = parseInt((a / b) * 100);
		};
		return num;
	});
	//当前作业
	var curWork =
		'{{each work as $value i}}' +
		'<li class="clearfix">' +
		'<div class="t1"><span class="name" title="{{$value.paper_name}}">{{$value.paper_name}}</span></div>' +
		'<div class="t2"><span class="startTime">{{$value.start_time}}</span></div>' +
		'<div class="t3"><span class="endTime">{{$value.end_time}}</span></div>' +
		'<div class="t4"><span class="jindu"><em style="width:{{#curJindu($value.finishSum,$value.amount)*1.1}}px"></em></span><span class="jindu2">{{#curJindu($value.finishSum,$value.amount)}}%</span></div>' +
		'{{if $value.status==0}}' +
		'<div class="t5"><span class="tool noSta">未开始</span></div>' +
		'{{else}}' +
		'<div class="t5"><span class="tool ready" data-paper="{{$value.id}}">做试卷</span></div>' +
		'{{/if}}' +
		'</li>' +
		'{{/each}}';
	//历史作业
	var hiWork =
		...
		'<div class="t3"><span class="endTime">{{$value.end_time}}</span></div>' +
		'{{if $value.status==3}}' +
		'<div class="t4"><span class="grade">{{$value.score}}</span></div>' +
		'{{else}}' +
		'<div class="t4"><span class="grade">--</span></div>' +
		'{{/if}}' +
		'{{if $value.status==0}}' +
		'<div class="t5"><span class="status">未交卷</span></div>' +
		'<div class="t6"><span class="tool ready zy"  data-id="{{$value.id}}">查看作业</span></div>' +
		'{{/if}}' +
		'{{if $value.status==1}}' +
		'<div class="t5"><span class="status">未批阅</span></div>' +
		'<div class="t6"><span class="tool ready zy" data-id="{{$value.id}}">查看作业</span></div>' +
		'{{/if}}' +
		'{{if $value.status==2}}' +
		'<div class="t5"><span class="status">已批阅</span></div>' +
		'<div class="t6"><span class="tool ready zy" data-id="{{$value.id}}">查看作业</span></div>' +
		'{{/if}}' +
		'{{if $value.status==3}}' +
		'<div class="t5"><span class="status">已批阅</span></div>' +
		'<div class="t6"><span class="tool ready cj" data-id="{{$value.id}}">查看成绩</span></div>' +
		'{{/if}}' +
		'</li>' +
		'{{/each}}';
	//笔记列表
	...
	var emptyDefaul3 =
		"<div class='page-no-result'>" +
		"<img src='../images/personcenter/my_nodata.png' style='margin-top:70px'> " +
		"<div class='no-title'>暂无数据</div>" +
		"</div>";
	//学习计划右侧目录
...
//							success: function(m) {
//								if(m.success === true) {
//									localStorage.userid = m.resultObject.id;
//									/*关闭信息认证*/
//									if(m.resultObject.perfectInformation == false) {
//										$(".personBack").show();
//										createData();
//									}
//								};
...
//						});
						//1为当前作业，2为历史作业
						if($.getUrlParam("work") == 1) {
							$(".course-nav .work").addClass("active").siblings().removeClass("active");
							$(".course-main .main2").show().siblings().hide();
							$(".work-title .check").eq(0).addClass("act").siblings().removeClass("act");
							homeWork(1);
						} else if($.getUrlParam("work") == 2) {
							$(".work-title .check").eq(1).addClass("act").siblings().removeClass("act");
							$(".course-nav .work").addClass("active").siblings().removeClass("active");
							$(".course-main .main2").show().siblings().hide();
							homeWork(2);
						} else {
//							plan();
							...
							$(".course-main .main").show();
							$(".course-main .main1").hide();*/
							$(".course-nav .jihua").hide();
							$(".course-nav .video").addClass("active").siblings().removeClass("active");
							$(".course-main .main").show().siblings().hide();
							nav();
						};
					} else {
						$(".course-nav .work").hide();
						$(".course-nav .jihua").hide();
						$(".course-nav .video").addClass("active");
						$(".course-main .main").show().siblings().hide();
						nav();
			...
			$(".course-nav li").on("click", function() {
				...
				if($(this).hasClass("video")) {
					...
					$(".right-note span").eq(0).addClass("act").siblings().removeClass("act");
					$(".course-main .main").show().siblings().hide();
					Notes();
				} else if($(this).hasClass("wenda")) {
					$(".right-ask span").eq(0).addClass("act").siblings().removeClass("act");
					$(".course-main .main").show().siblings().hide();
					wenda();
				...
				function calder(year, num, rest) {
					...
					$(".calder ul .noban").off().on("click", function() {
						if(!$(this).hasClass("ban")) {
							...
							$("body").animate({
								"scrollTop": $(".course-nav").get(0).offsetTop
							}, 200);
						}
					});
				...
				RequestService("/course/resource/getChapterTree", "get", {
					...
					if(data.success == true) {
						...
						$(".main-left .allinfo .info2").off().on("click", function() {
							...
							$(".main-ppt .right-ppt span").eq(1).attr("data-jiaoan", $(this).attr("data-jiaoan"));
							if($(this).attr("data-ppt") != "" && $(this).attr("data-ppt") != null) {
								$(".page-no-result").hide();
								$(".main-ppt .right-con #ppt").attr("href", $(this).attr("data-ppt")).show();
								$(".main-ppt .right-con #ppt").show();
//								$(".main-ppt .right-con #frame").attr("src", $(this).attr("data-ppt")).show();
							} else {
								$(".main-ppt .right-con #ppt").hide();
//								$(".main-ppt .right-con #frame").hide();
						...
						$(".main-ppt .right-ppt span").off().on("click", function() {
							...
							if($(this).attr("data-ppt") != "" && $(this).attr("data-ppt") != null) {
								$(".page-no-result").hide();
								$(".main-ppt .right-con #ppt").attr("href", $(this).attr("data-ppt")).show();
								$(".main-ppt .right-con #ja").hide();
							} else if($(this).attr("data-jiaoan") != "" && $(this).attr("data-jiaoan") != null) {
								$(".page-no-result").hide();
								$(".main-ppt .right-con #ja").attr("href", $(this).attr("data-jiaoan")).show();
								$(".main-ppt .right-con #ppt").hide();
//								$(".main-ppt .right-con #ja").attr("src", $(this).attr("data-jiaoan")).show();
							} else {
								$(".main-ppt .right-con #ja").hide();
								$(".main-ppt .right-con #ppt").hide();
								$(".page-no-result").show();
			...
			function homeWork(a) {
				...
				//当期那作业
				function cur(status, num, size) {
					$(".work-con .cuWork").show();
					$(".work-con .hiWork").hide();
					$(".allT").hide();
					$(".pages1").css("display", "none");
					RequestService("/paper/findMyCurrentPage", "get", {
						//0:作业  1:试卷
						paperStatus: status ? status : 0,
						pageNumber: num ? num : 1,
						pageSize: size ? size : 10
					}, function(data) {
						if(data.success == true) {
							if(data.resultObject.items == "" || data.resultObject.items == undefined) {
								$(".cuWork ul").html(emptyDefaul3);
							} else {
								if(data.resultObject.totalPageCount == 1 && data.resultObject.totalCount > 0) {
									$(".pages1").css("display", "none");
								} else {
									$(".pages1").css("display", "block");
									$(".pages1 .searchPage .allPage").text(data.resultObject.totalPageCount);
									if(data.resultObject.currentPage == 1) {
										$("#Paginations").pagination1(data.resultObject.totalPageCount, {
											callback: function(page) { //翻页功能
												cur(status, page + 1, size)
											}
										});
									};
								};
								$(".cuWork ul").html(template.compile(curWork)({
									work: data.resultObject.items
								}));
							};

							$(".cuWork ul .ready").off("click").on("click", function() {
								var $this=$(this);
								$.ajax({
									type: "get",
									url: bath + "/online/user/isAlive",
									...
									success: function(data) {
										if(data.success === true) {
											window.open("/web/html/task.html?paperId=" + $this.attr("data-paper"),"task"+$this.attr("data-paper")).focus();
										} else {
											$('#login').modal('show');
											$('#login').css("display", "block");
											$(".loginGroup .logout").css("display", "block");
				...
				//历史作业
				function his(status, time1, time2, num, size) {
					$(".work-con .cuWork").hide();
					$(".work-con .hiWork").show();
					$(".pages1").css("display", "none");
					$(".allT").show();
					RequestService("/paper/findMyHistoryPage", "get", {
						//0:作业  1:试卷
						paperStatus: status ? status : 0,
						pageNumber: num ? num : 1,
						startTime: time1 ? time1 : "",
						endTime: time2 ? time2 : "",
						pageSize: size ? size : 10
					}, function(data) {
						if(data.success == true) {
							if(data.resultObject.items == "" || data.resultObject.items == undefined) {
								$(".hiWork ul").html(emptyDefaul3);
							} else {
								if(data.resultObject.totalPageCount == 1 && data.resultObject.totalCount > 0) {
									$(".pages1").css("display", "none");
								} else {
									...
									if(data.resultObject.currentPage == 1) {
										$("#Paginations").pagination1(data.resultObject.totalPageCount, {
											callback: function(page) { //翻页功能
												his(status, time1, time2, page + 1, size)
											}
										});
									};
								};
								$(".hiWork ul").html(template.compile(hiWork)({
									work: data.resultObject.items
								}));
							};
							//查看成绩和作业
							$(".hiWork ul .cj").off("click").on("click", function() {
								var $this=$(this);
								$.ajax({
									type: "get",
									url: bath + "/online/user/isAlive",
									...
									success: function(data) {
										if(data.success === true) {
											window.open("/web/html/checkHomework.html?paperId=" + $this.attr("data-id"),"checkHomework"+$this.attr("data-id"));
										} else {
											$('#login').modal('show');
											$('#login').css("display", "block");
											$(".loginGroup .logout").css("display", "block");
							...
							$(".hiWork ul .zy").off("click").on("click", function() {
								var $this=$(this);
								$.ajax({
									type: "get",
									url: bath + "/online/user/isAlive",
									...
									success: function(data) {
										if(data.success === true) {
											window.open("/web/html/viewTestPage.html?paperId=" + $this.attr("data-id"),"viewTestPage"+$this.attr("data-id"));
										} else {
											$('#login').modal('show');
											$('#login').css("display", "block");
											$(".loginGroup .logout").css("display", "block");
				...
				//时间筛选
				$(".allT .stTime").off().on("blur", function() {
					var $this = $(this);
					setTimeout(function() {
						if($this.val() == $this.attr("data-saTime")) {
							return false;
						} else {
							$this.attr("data-saTime", $this.val());
							his(0, $(".allT .stTime").val(), $(".allT .enTime").val(), 1, 10);
						};
					}, 500);

				});
				$(".allT .enTime").off().on("blur", function() {
					var $this = $(this);
					setTimeout(function() {
						if($this.val() == $this.attr("data-saTime")) {
							return false;
				...
				//切换当前历史
				$(".work-title .check").off("click").on("click", function() {
					$(this).addClass("act").siblings().removeClass("act");
