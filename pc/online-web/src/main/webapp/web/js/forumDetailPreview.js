...
$(function(){
    ...
    template.helper("tagGroup",function(tagName,tagId){
        ...
        for(var i=0;i<tagName.length;i++){
            if(i==tagName.length-1){
                str+='<a  style="cursor: default" target="_blank">'+tagName[i]+'</a>';
            }else{
                str+='<a  style="cursor: default" target="_blank">'+tagName[i]+'</a>'+"<span style='color:#999;margin:0 3px'>,</span>";
            }
        }
        return str;
    });
    ...
    var articleDetail='<div class="forum-detail">'+
        ...
        '{{if items.isPraise==0}}'+
        '<div class="forum-hitzan" style="cursor: default">'+
    ...
    var commentInfo='{{each items}}'+
        ...
        '{{/if}}'+
        '<div class="reply-comment" style="cursor: default"><i class="iconfont icon-huifu"></i>回复</div>'+
        '{{if $value.mySelf==true}}'+
        '<div class="reply-delete" style="cursor: default" data-commentId="{{$value.id}}"><i class="iconfont icon-shanchu"></i>删除</div>'+
        '{{/if}}'+
    ...
    var relativeArticle = '{{each hotArticle}}' +
        '{{if $index<=2}}' +
        '<li>' +
        '<a  target="_blank" style="cursor: default"><em class="select">{{$index+1}}</em><span title="{{$value.title}}">{{$value.title}}</span></a>' +
        '</li>' +
        '{{else}}' +
        '<li><a  target="_blank" style="cursor: default"><em>{{$index+1}}</em><span title="{{$value.title}}">{{$value.title}}</span></li></a>' +
        '{{/if}}' +
        '{{/each}}';
    var hotTag='{{each hotTag}}'+
        '<li><a  style="cursor: default">{{$value.name}}</a></li>'+
        '{{/each}}';
    var relativeCourse = '{{each item as $value i}}' +
        ...
        '{{/each}}';
    //文章详情
    RequestService("/bxs/article/getArticleById",'GET',{articleId:"",preId:articleId},function(data){
        $(".forum-detailInfo").html(template.compile(articleDetail)({
    ...
    /*$(".community-submit").click(function(){
        var commentCon=$(".forum-community-textarea").val();
        if(commentCon==""){
    ...
    //relative-article
    RequestService("/bxs/article/getCorrelationTitle", "GET", {articleId:articleId }, function (data) {
        if(data.resultObject.length==0){
            $(".hot-article-list").html(template.compile(emptyDefaul));
        }else{
            $(".hot-article-list").html(template.compile(relativeArticle)({
            ...
            forumThs.click(function(){
                RequestService("/online/user/isAlive", "GET", null, function(data) {
                    if (data.success == false) {
                        ...
                        var replayCon=forumThs.parent().parent().find(".reply-input").val();
