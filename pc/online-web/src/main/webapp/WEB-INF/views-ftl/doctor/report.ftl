...
<head lang="en">
    ...
    <link rel="stylesheet" href="/web/css/doctor/report.css"/>
    ...
    <div class="content clearfix" id="main">
        ...
        <div class="main_left">
            <div class="doctor_report">
                <div class="doctor_report_top">
                    <h3>名医报道</h3>
                </div>
                <div class="doctor_report_bottom">
                    <ul id="doctor_report_list">
                    <#list reports.records as report>
                        <li class="clearfix">
                            <div class="doctor_report_left">
                                <a href="${webUrl}/headline/details/${report.id}">
                                    <img src="${report.imgPath}" alt="${report.title}"/>
                                </a>
                            </div>
                            <div class="doctor_report_inf">
                                <h4>${report.title}</h4>
                                <p>${report.content}</p>
                                <span><#if report.createTime ??>${report.createTime?string('yyyy-MM-dd')}</#if></span>
                            </div>
                        </li>
                    </#list>
                    </ul>
                ...
                <div class="doctor_report_more">
                <@cast.page pageNo=reports.current totalPage=reports.pages showPages=5 callUrl="${webUrl}/doctors/report?&page="/>
                </div>
            </div>
        </div>
