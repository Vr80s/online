package com.xczhihui.wechat.dao;
...
public class WechatMaterialDao extends HibernateDao<Course> {
	...
	public Page<WechatMaterial> findWechatMaterialPage(
		 ...
		 * 
		 * if (WechatMaterial.getStatus() != null) { paramMap.put("status",
		 * WechatMaterial.getStatus()); sql.append(" and og.status = :status ");
		 * }
		 */
		sql.append(" order by og.status desc, og.sort desc");

		Page<WechatMaterial> WechatMaterials = this.findPageBySQL(
				sql.toString(), paramMap, WechatMaterial.class, pageNumber,
				pageSize);
