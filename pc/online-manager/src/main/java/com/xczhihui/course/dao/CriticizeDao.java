...
public class CriticizeDao extends SimpleHibernateDao {
	...
	public Page<CriticizeVo> findCriticizePage(CriticizeVo criticizeVo,
		...
		if (criticizeVo.getResponse() != null
				&& "1".equals(criticizeVo.getResponse())) {
			sql.append(" and oc.response is not null and oc.response != '' ");
		}
		if (criticizeVo.getResponse() != null
				&& "0".equals(criticizeVo.getResponse())) {
		...
		} else if (criticizeVo.getSortType() != null
				&& criticizeVo.getSortType() == 2) {
			sql.append(" order by praise_sum desc,create_time desc");
		} else {
			sql.append(" order by create_time desc");
		}
		Page<CriticizeVo> ms = this.findPageBySQL(sql.toString(), paramMap,
				CriticizeVo.class, pageNumber, pageSize);
		return ms;
	}
