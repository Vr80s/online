...
public class ProjectTypeDao extends HibernateDao<Project>{
	...
	public Project findById(String parseInt) {
		StringBuilder sql=new StringBuilder("select * from oe_project where id=:id");
        Map<String,Object> params=new HashMap<String,Object>();
        params.put("id",parseInt);
        List<Project> menus=this.getNamedParameterJdbcTemplate().query(sql.toString(),params,BeanPropertyRowMapper.newInstance(Project.class));
        if(menus.size()>0){
            return menus.get(0);
        }
        return null;
