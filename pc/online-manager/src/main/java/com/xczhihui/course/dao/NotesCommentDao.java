...
public class NotesCommentDao extends SimpleHibernateDao {
	...
	public Page<NotesCommentVo> findNotesCommentPage(
		...
		StringBuilder sql = new StringBuilder("select " + "	onc.id, "
				+ "	onc.content, " + "	onc.create_person " + "from "
		...
		Page<NotesCommentVo> ms = this.findPageBySQL(sql.toString(), paramMap,
				NotesCommentVo.class, pageNumber, pageSize);
		return ms;
	}
}
