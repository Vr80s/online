package com.xczhihui.system.web;
...
public class LinkController {
	 ...
	 * @RequestMapping(value = "initLink")
	 * 
	 * @ResponseBody public TableVo loadLink(TableVo tableVo) { int pageSize =
	 * tableVo.getiDisplayLength(); int index = tableVo.getiDisplayStart(); int
	 * pageNumber = index / pageSize + 1; String params = tableVo.getsSearch();
	 * String urlOrcreatPer = ""; String timeStart = null; String timeEnd =
	 * null; HashMap<String, Object> paramMap = null;
	 * 
	 * try{ Groups groups = Tools.filterGroup(params);
	 * 
	 * for (Group g : groups.getGroupList()) { if
	 * ("urlOrcreatPer".equals(g.getPropertyName())) { urlOrcreatPer =
	 * g.getPropertyValue1().toString(); } if
	 * ("timeStart".equals(g.getPropertyName())) { timeStart =
	 * g.getPropertyValue1().toString();
	 * 
	 * } if ("timeEnd".equals(g.getPropertyName())) { timeEnd =
	 * g.getPropertyValue1().toString();
	 * 
	 * } } paramMap = new HashMap<String,Object>();
	 * paramMap.put("urlOrcreatPer", urlOrcreatPer); paramMap.put("timeEnd",
	 * timeEnd); paramMap.put("timeStart", timeStart);
	 * 
	 * Page<OtherLinkVo> page = service.findOtherLinkPage(paramMap, pageNumber,
	 * pageSize); tableVo.setAaData(page.getItems());
	 * tableVo.setiTotalDisplayRecords(page.getTotalCount());
	 * tableVo.setiTotalRecords(page.getTotalCount()); }catch(Exception e){
	 * e.printStackTrace(); } return tableVo; }
	 */
	...
	@ResponseBody
	public ResponseObject updateLink(HttpServletRequest request, Model model,
			Otherlink olink) {
		ResponseObject responseObj = new ResponseObject();
		try {
