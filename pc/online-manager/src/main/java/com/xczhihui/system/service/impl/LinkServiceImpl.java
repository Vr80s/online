...
public class LinkServiceImpl implements LinkService {
	...
	public Page<OtherLinkVo> findOtherLinkPage(Map<String, Object> paramMap,
		...
		Page<OtherLinkVo> linkPage = this.dao.findOtherLinkPage(paramMap,
				pageNumber, pageSize);
	...
	public boolean findSort(Integer sort, Integer id) {
		...
		if (id != null && !"".equals(id)) {// 修改
			list = dao
					.findByHQL(
							"from Otherlink b where b.isDelete = 0 and b.sort = ? and b.id != ?",
							sort, id);
		} else {
			list = dao.findByHQL(
					"from Otherlink b where b.isDelete = 0 and b.sort = ?",
					sort);
		}
