package com.xczhihui.system.dao;
...
public class LinkDao extends HibernateDao<Otherlink> {
	...
	public PageVo findPagevideo(Groups groups, PageVo page) {
		...
		sql.append(" select a.id as id," + "a.name as orgname,"
				+ "a.url as url," + "a.logo as logo," + "a.status as status,"
	...
	public void deleteBatch(String... ids) {
		for (String id : ids) {
			...
			Otherlink otherlink = this.findByHQLOne(hqlPre,
	...
	public Page<OtherLinkVo> findOtherLinkPage(Map<String, Object> paramMap,
		...
		if (paramMap != null && paramMap.size() > 0) {
			for (Entry<String, Object> entry : paramMap.entrySet()) {
				...
				if (paramValue != null
						&& StringUtils.hasText(paramValue.toString())) {
					if ("urlOrcreatPer".equals(entry.getKey())) {
						sbSql.append(" AND (oe_otherlink.create_person like '%"
								+ entry.getValue()
								+ "%' OR oe_otherlink.url like '%"
								+ entry.getValue() + "%')");
					}
					...
					if ("timeStart".equals(entry.getKey())) {
						sbSql.append(" AND (oe_otherlink.create_time BETWEEN '"
								+ entry.getValue() + "'");
					}
		...
		return this.findPageBySQL(sbSql.toString(), null, OtherLinkVo.class,
				pageNumber, pageSize);
	}
