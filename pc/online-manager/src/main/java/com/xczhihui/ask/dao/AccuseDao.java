...
public class AccuseDao extends SimpleHibernateDao {
	...
	public Page<AccuseVo> findAccusePage(AccuseVo accuseVo, int pageNumber,
			int pageSize) {
		Map<String, Object> paramMap = new HashMap<String, Object>();
		StringBuilder sql = new StringBuilder("select * from ("
				+ "SELECT "
				+ // 问题
				"	oaa.id, " + "	oaa.target_type, " + "	oaa.target_id, "
				+ "	oaa.accuse_type, " + "	oaa.content accuse_type_content, "
				+ "	oaa.create_time, " + "	oaa.status, "
				+ "	oaa.create_person create_person_id, "
				+ "	ou. name createPersonName, " + "	oaq.title content,  "
				+ "	oaq.id questionId,  " + "	oua.name accusePersonName  "
				+ "FROM " + "	oe_ask_accuse oaa "
				+ "JOIN oe_ask_question oaq ON ( " + "	oaa.target_id = oaq.id "
				+ "	AND oaa.target_type = 0 ) " + "JOIN oe_user ou ON ( "
				+ "	oaa.user_id = ou.id ) " + "JOIN oe_user oua ON ( "
				+ "	oaq.user_id = oua.id " + ") ");

		if (accuseVo.getAccuseType() != null
				&& !"".equals(accuseVo.getAccuseType())) {
		...
		sql.append("SELECT "
				...
				+ "	oua.name accusePersonName  " + "FROM "
				+ "	oe_ask_accuse oaa " + "JOIN oe_ask_answer oaan ON ( "
				+ "	oaa.target_id = oaan.id " + "	AND oaa.target_type = 1 "
				+ ") " + "JOIN oe_user ou ON ( " + "	oaa.user_id = ou.id  ) "
				+ "JOIN oe_user oua ON ( " + "	oaan.user_id = oua.id " + ") ");
		if (accuseVo.getAccuseType() != null
				&& !"".equals(accuseVo.getAccuseType())) {
			sql.append(" and oaa.accuse_type = :accuseType ");
			paramMap.put("accuseType", accuseVo.getAccuseType());
