...
public class TagServiceImpl extends OnlineBaseServiceImpl implements TagService {
	...
	public String addTag(TagVo tagVo) {
		...
		List<AskTag> temp = tagDao
				.findEntitiesByJdbc(AskTag.class, sql, params);
		int sort;
		...
		if (tagVo.getName() != null && !"".equals(tagVo.getName())
				&& tagVo.getName().indexOf(",") != -1) {
			...
			for (int i = 0; i < nameArray.length; i++) {
				...
				List<AskTag> TagfindByNames = tagDao
						.findByHQL(
								"from AskTag where menuId=? and name=? and is_delete = 0 ",
								Integer.parseInt(tagVo.getMenuId()),
								nameArray[i].trim());
			...
			String TagfindByNameSql = "SELECT * FROM oe_ask_tag where is_delete = 0 and menu_id = "
					+ tagVo.getMenuId()
					+ " and name = '"
					+ tagVo.getName()
					+ "'";
			AskTag askTag = new AskTag();
			List<AskTag> TagfindByNames = tagDao.findEntitiesByJdbc(
					AskTag.class, TagfindByNameSql, paramMap);
	...
	public void updateTag(TagVo tagVo) {
		// TODO Auto-generated method stub
		Map<String, Object> paramMap = new HashMap<String, Object>();
		AskTag me = tagDao.findOneEntitiyByProperty(AskTag.class, "id",
				tagVo.getId());
		if (me != null) {
			String TagfindByNameSql = "SELECT * FROM oe_ask_tag where menu_id = "
					+ me.getMenuId()
					+ " and is_delete = 0 and name = '"
					+ tagVo.getName() + "' and id!='" + me.getId() + "'";
			List<AskTag> TagfindByNames = tagDao.findEntitiesByJdbc(
					AskTag.class, TagfindByNameSql, paramMap);
			if (tagVo.getName().length() > 20) {
