...
public class GiftStatementDao extends HibernateDao<Course> {
	...
	public Page<GiftStatementVo> findGiftStatementPage(
			GiftStatementVo giftStatementVo, int pageNumber, int pageSize) {
		Map<String, Object> paramMap = new HashMap<>();
		StringBuilder sql = new StringBuilder(
				"SELECT ogs.`id`,ogs.`gift_name` giftName,ogs.`price`,ogs.`count`,ou1.name giver,ca.name receiver,ou2.login_name loginName,"
		...
		return this.findPageBySQL(sql.toString(), paramMap,
				GiftStatementVo.class, pageNumber, pageSize);
	}
}
