...
public class MobileCourseServiceImpl extends OnlineBaseServiceImpl implements
		MobileCourseService {
	@Autowired
	...
	public Page<CourseVo> findCoursePage(CourseVo courseVo, int pageNumber,
			int pageSize) {
		// TODO Auto-generated method stub
		Page<CourseVo> page = mobileCourseDao.findMobileCoursePage(courseVo,
				pageNumber, pageSize);
		return page;
	}
	...
	public List<CourseVo> findCourseById(Integer id) {
		...
		courseVoList.get(0).setLearndCount(
				mobileCourseDao.queryForInt(sql, new Object[] { id }));// 累计默认报名人数
		return courseVoList;
	}
	...
	public Map<String, String> getCourseDetail(String courseId) {
		...
		List<Course> temp = mobileCourseDao.findEntitiesByJdbc(Course.class,
				selectSqlByCourse, paramMap);
	...
	public void updateCourseDetail(String courseId, String smallImgPath,
		...
		Map<String, Object> paramMap = new HashMap<String, Object>();
		paramMap.put("courseId", courseId);
		List<CourseVo> temp = mobileCourseDao.findEntitiesByJdbc(
				CourseVo.class, selectSqlByCourse, paramMap);
		if (temp.size() > 0) {
			mobileCourseDao
					.getNamedParameterJdbcTemplate()
					.getJdbcOperations()
					.update("update  oe_course_mobile set " + "img_url=?, "
							+ "description=? where course_id=?",
							new Object[] { smallImgPath, courseDetail, courseId });
			// 更新
		} else {
			mobileCourseDao
					.getNamedParameterJdbcTemplate()
					.getJdbcOperations()
					.update("INSERT INTO oe_course_mobile ( " + "	id, "
							+ "	course_id, " + "img_url, " + "description )"
							+ " VALUES (REPLACE(UUID(),'-',''), ?, ?, ?)",
							new Object[] { courseId, smallImgPath, courseDetail });
		}

	}

	@Override
	public void addPreview(String courseId, String smallImgPath, Object object,
