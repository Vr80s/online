...
public class MobileCourseDao extends HibernateDao<Course> {
	...
	public Page<CourseVo> findMobileCoursePage(CourseVo courseVo,
			int pageNumber, int pageSize) {
		// TODO Auto-generated method stub
		Map<String, Object> paramMap = new HashMap<String, Object>();
		StringBuilder sql = new StringBuilder(
				" select w.* from ( SELECT oc.id as id ,oc.grade_name as courseName, oc.class_template as classTemplate, om.name as xMenuName,st.name as scoreTypeName,"
						...
						+ "oc.courseType as courseType,count(og.id) as countGradeNum,oc.is_recommend,oc.qqno,oc.course_type as serviceType,oc.user_lecturer_id as userLecturerId, "// TODO
																																													// 杨宣增加userLecturerId
		...
		for (CourseVo entityVo : courseVos.getItems()) {
			 ...
			 * ); List<CourseVo> teacherNames =
			 * this.getNamedParameterJdbcTemplate
			 * ().query(teacherSql.toString(),params
			 * ,BeanPropertyRowMapper.newInstance(CourseVo.class));
			 * entityVo.setLecturerName
			 * (teacherNames==null||teacherNames.size()==
			 * 0?"":teacherNames.get(0).getLecturerName());
			 */
			/**
			 * 杨宣修改 把班级信息去掉
			 */
			...
			StringBuilder teacherSql = new StringBuilder(); // // +
															// "LEFT JOIN oe_user ou on oc.user_lecturer_id = ou.id and oe.is_lecturer = 1 and oe.status = 0");
			teacherSql
					.append("SELECT ou.name as lecturerName from oe_user as ou where ou.id =:user_lecturer_id and ou.is_lecturer = 1 and ou.status = 0 ");
			List<CourseVo> teacherNames = this.getNamedParameterJdbcTemplate()
					.query(teacherSql.toString(), params,
							BeanPropertyRowMapper.newInstance(CourseVo.class));
			entityVo.setLecturerName(teacherNames == null
					|| teacherNames.size() == 0 ? "" : teacherNames.get(0)
					.getLecturerName());
		}
		return courseVos;
		// }
