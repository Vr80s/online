...
public class MobileSearchDao extends HibernateDao<MobileSearchVo> {
	...
	public Page<MobileSearchVo> findMobileSearchPage(MobileSearchVo mobileSearchVo, int pageNumber,
		...
		StringBuilder sql =new StringBuilder("select * from oe_mobile_search where is_delete=0 ");
		if(mobileSearchVo.getSearchType() != null && !"".equals(mobileSearchVo.getSearchType())){
			sql.append(" and search_type = :type ");
			paramMap.put("type", mobileSearchVo.getSearchType());
		}
		sql.append(" order by  status desc,seq asc ");
		return this.findPageBySQL(sql.toString(), paramMap, MobileSearchVo.class, pageNumber, pageSize);
	}
	...
	public MobileSearchVo findById(String parseInt) {
		StringBuilder sql=new StringBuilder("select * from oe_mobile_search where id=:id");
        Map<String,Object> params=new HashMap<String,Object>();
        params.put("id",parseInt);
        List<MobileSearchVo> menus=this.getNamedParameterJdbcTemplate().query(sql.toString(),params,BeanPropertyRowMapper.newInstance(MobileSearchVo.class));
        if(menus.size()>0){
            return menus.get(0);
        }
        return null;
