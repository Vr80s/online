...
public class BarrierServiceImpl extends OnlineBaseServiceImpl implements
	...
	public Page<BarrierVo> findBarrierPage(BarrierVo barrierVo,
		...
		Page<BarrierVo> page = barrierDao.findBarrierPage(barrierVo,
				pageNumber, pageSize);
		return page;
	}
	...
	public Page<CourseVo> findCoursePage(CourseVo courseVo, int pageNumber,
			int pageSize) {
		Page<CourseVo> page = barrierDao.findCloudClassCoursePage(courseVo,
				pageNumber, pageSize);
		return page;
	...
	public void addBarrier(BarrierVo barrierVo) {
		...
		// if
		// (barrierDao.getNamedParameterJdbcTemplate().getJdbcOperations().queryForObject
		// ("select count(id) from oe_barrier where course_id=? and `status`=1",
		// Integer.class,barrierVo.getCourseId()) > 0) {
		// throw new RuntimeException("关卡已被启用，不能再删除、修改上级！");
		// }

		int status = 0;
		String parentId = "".equals(barrierVo.getParentId()) ? null : barrierVo
				.getParentId();
		String barrierId = null;

		Barrier old = dao.findOneEntitiyByProperty(Barrier.class, "courseId",
				barrierVo.getCourseId());
		if (old != null) {
		...
		barrier.setParentId("".equals(barrierVo.getParentId()) ? null
				: barrierVo.getParentId());
		...
		for (int i = 0; i < barrierVo.getStrategyCnt().length; i++) {
			...
			barrierStrategy.setQuestionSum(Integer.parseInt(barrierVo
					.getStrategyCnt()[i]));
			barrierStrategy.setTotalScore(Integer.parseInt(barrierVo
					.getStrategyTotalScore()[i]));
			barrierStrategy.setQustionScore(Integer.parseInt(barrierVo
					.getStrategyScore()[i]));
		...
		if (barrierVo.getKpointIds() != null) {
			...
			for (int i = 0; i < kpoints.length; i++) {
				Chapter chapter = dao.findOneEntitiyByProperty(Chapter.class,
						"id", kpoints[i]);
		...
		if (status == 1) {
			...
			String statusSql = parentId != null ? " ifnull((select barrier_status from oe_barrier_user where user_id=t3.id and barrier_id='"
					+ parentId + "' limit 0,1),0) "
					: "1";
			String sql = "insert into oe_barrier_user (id,user_id,barrier_id,course_id,lock_status) "
					+ " select replace(uuid(),'-',''),t3.id,'"
					+ barrierId
					+ "','"
					+ barrier.getCourseId()
					+ "', "
	...
	public void updateBarrier(BarrierVo barrierVo) {
		...
		Barrier barrier = dao.findOneEntitiyByProperty(Barrier.class, "id",
				barrierVo.getId());
		String voParentId = barrierVo.getParentId() == null ? "" : barrierVo
				.getParentId();
		String parentId = barrier.getParentId() == null ? "" : barrier
				.getParentId();
		...
		if (!voParentId.equals(parentId)) {
			if (barrierDao
					...
					.queryForObject(
							"select count(id) from oe_barrier where course_id=? and `status`=1",
							Integer.class, barrier.getCourseId()) > 0) {
				throw new RuntimeException("关卡已被启用，不能再删除、修改上级！");
			}
		}
		...
		for (int i = 0; i < barrierVo.getStrategyCnt().length; i++) {
			...
			BarrierStrategy barrierStrategy = dao.findByHQLOne(hql,
					new Object[] { barrierVo.getId(), i + "" });
	...
	public BarrierVo getBarrierDetail(BarrierVo barrierVo) {
		...
		List<BarrierStrategy> list = dao.findByHQL(hql,
				new Object[] { barrierVo.getId() });
		...
		barrierVo.setKpointIds(dao
				.getNamedParameterJdbcTemplate()
				.getJdbcOperations()
				.queryForObject(sql, new Object[] { barrierVo.getId() },
						String.class));
		// 获取到所有本关卡下的知识点名字
		sql = "select group_concat(oc.name) names from oe_chapter oc where oc.barrier_id = ?  and oc.is_delete = 0 ";
		barrierVo.setKpointNames(dao
				.getNamedParameterJdbcTemplate()
				.getJdbcOperations()
				.queryForObject(sql, new Object[] { barrierVo.getId() },
						String.class));
	...
	public List getQuestions(String kpointIds) {
		StringBuffer sql = new StringBuffer("SELECT "
				+ "	oq.question_type questionType, " + "	count(1) cnt "
				+ "FROM " + "	oe_question oq,oe_question_kpoint oqk "
				+ "WHERE oq.is_delete = 0 " + "AND oq.`status` = 1 "
	...
	public void update_UseBarrier(String courseId) {
		...
		dao.getNamedParameterJdbcTemplate()
				.update("update oe_barrier set `status`=1 where course_id = :course_id",
						paramMap);
