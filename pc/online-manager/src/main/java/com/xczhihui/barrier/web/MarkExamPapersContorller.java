...
public class MarkExamPapersContorller {
	...
	public TableVo findGradeList(TableVo tableVo) {
		...
		Page<MarkGradeVo> page = service.findGradeList(searchVo, currentPage,
				pageSize);
	...
	public ModelAndView barriers(HttpServletRequest request, Integer grade_id,
			String grade_name) {
	...
	public TableVo findbarriers(TableVo tableVo) {
		...
		String params = tableVo.getsSearch();
		Groups groups = Tools.filterGroup(params);
		Integer grade_id = Integer.parseInt(groups.findByName("grade_id")
				.getPropertyValue1().toString());

		Page<MarkBarrierVo> page = service.findbarriers(grade_id, currentPage,
				pageSize);
	...
	public ModelAndView barrierRecord(HttpServletRequest request,
			Integer grade_id, String barrier_id, String grade_name,
			String barrier_name) {
	...
	public TableVo findBarrierRecord(TableVo tableVo) {
		...
		Integer grade_id = Integer.parseInt(groups.findByName("grade_id")
				.getPropertyValue1().toString());
		String barrier_id = groups.findByName("barrier_id").getPropertyValue1()
				.toString();
		Integer barrier_status = Integer.parseInt(groups
				.findByName("barrier_status").getPropertyValue1().toString());
		String user_name = groups.findByName("user_name").getPropertyValue1()
				.toString();

		Page<MarkRecordVo> page = service.findBarrierRecord(barrier_status,
				user_name, grade_id, barrier_id, currentPage, pageSize);
		int total = page.getTotalCount();
		tableVo.setAaData(page.getItems());
		tableVo.setiTotalDisplayRecords(total);
		tableVo.setiTotalRecords(total);
	...
	public ModelAndView examPaper(HttpServletRequest request, Integer grade_id,
			String barrier_id, String grade_name, String barrier_name,
