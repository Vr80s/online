...
public class TeachMethodDao extends HibernateDao<TeachMethod> {
	...
	public Page<TeachMethodVo> findTeachMethodPage(TeachMethodVo teachMethodVo,
		...
		StringBuilder sql = new StringBuilder(
				"select * from teach_method where is_delete = 0 ");
		sql.append(" order by sort desc ");
		Page<TeachMethodVo> pages = this.findPageBySQL(sql.toString(),
				paramMap, TeachMethodVo.class, pageNumber, pageSize);
		...
		for (TeachMethodVo temp : pages.getItems()) {
			...
			List<TeachMethodVo> res = this.findEntitiesByJdbc(
					TeachMethodVo.class, sqlCourses, paramMap);
	...
	public TeachMethod findById(String id) {
		StringBuilder sql = new StringBuilder(
				"select * from teach_method where id=:id");
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("id", id);
		List<TeachMethod> menus = this.getNamedParameterJdbcTemplate().query(
				sql.toString(), params,
	...
	public TeachMethod findByNotEqId(TeachMethod searchEntity) {
		StringBuilder sql = new StringBuilder(
				"select * from teach_method where id <> :id and name=:name and  is_delete =0");
		Map<String, Object> params = new HashMap<String, Object>();
	...
	public void deletes(String[] _ids) {
		if (_ids.length > 0) {
			...
			StringBuilder sql = new StringBuilder(
					" update teach_method set is_delete=1 where  id IN (");
