...
import com.xczhihui.cloudClass.service.EssenceRecommendService;
...
public class EssenceRecommendController {
	...
	public TableVo courses(TableVo tableVo) {
		...
		if (course_type != null
				&& StringUtils.isNotBlank(course_type.getPropertyValue1()
						.toString())) {
			searchVo.setType(Integer.valueOf(course_type.getPropertyValue1()
					.toString()));
		}
		...
		if (course_liveStatus != null
				&& StringUtils.isNotBlank(course_liveStatus.getPropertyValue1()
						.toString())) {
			searchVo.setLiveStatus(Integer.valueOf(course_liveStatus
					.getPropertyValue1().toString()));
		}

		// 媒体类型
		Group course_multimediaType = groups
				.findByName("course_multimediaType");
		if (course_multimediaType != null
				&& StringUtils.isNotBlank(course_multimediaType
						.getPropertyValue1().toString())) {
			searchVo.setMultimediaType(Integer.valueOf(course_multimediaType
					.getPropertyValue1().toString()));
		}
		...
		if (is_essence != null) {
			searchVo.setIsEssence(Integer.valueOf(is_essence
					.getPropertyValue1().toString()));
		}
		...
		if (is_type_recommend != null) {
			searchVo.setIsTypeRecommend(Integer.valueOf(is_type_recommend
					.getPropertyValue1().toString()));
		}
