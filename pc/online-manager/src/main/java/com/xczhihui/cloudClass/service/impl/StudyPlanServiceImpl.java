 ...
 */
@Service
public class StudyPlanServiceImpl extends OnlineBaseServiceImpl implements
		StudyPlanService {
	...
	public List<Map<String, Object>> planInfo(Integer courseId, Integer week) {
		...
		List<Map<String, Object>> planList = dao
				.getNamedParameterJdbcTemplate().getJdbcOperations()
				.queryForList(sql.toString(), courseId, startDay, weekSize);
		StringBuffer kpointSql = new StringBuffer();
		kpointSql.append(" select op.id,op.`name`,");
		kpointSql
				.append(" (select sum(time_to_sec(CONCAT('00:',video_time))) div 60 from oe_video where chapter_id=op.id)as min,");
		kpointSql
				.append(" (select sum(time_to_sec(CONCAT('00:',video_time))) mod 60 from oe_video where chapter_id=op.id)as sec");
		kpointSql
				.append(" from oe_chapter op where op.is_delete=0 and op.plan_template_id=? order by op.sort");
		for (Map<String, Object> planInfo : planList) {
			String templateId = (String) planInfo.get("id");
			List<Map<String, Object>> kPoints = dao
					.getNamedParameterJdbcTemplate().getJdbcOperations()
					.queryForList(kpointSql.toString(), templateId);
			for (Map<String, Object> kPoint : kPoints) {
				int min = Integer.parseInt(kPoint.get("min") == null ? "0"
						: kPoint.get("min").toString());
				int sec = Integer.parseInt(kPoint.get("sec") == null ? "0"
						: kPoint.get("sec").toString());
	...
	public List<TreeNode> getChapterTree(Integer courseId, String templateId) {
		...
		List<TreeNode> treeNodes = videoResDao.findEntitiesByJdbc(
				TreeNode.class, sql, null);
		StringBuffer kpointSql = new StringBuffer();
		kpointSql.append(" select op.id,op.`name`");
		kpointSql
				.append(" from oe_chapter op where op.is_delete=0 and op.plan_template_id=?");
		List<Map<String, Object>> kPoints = dao.getNamedParameterJdbcTemplate()
				.getJdbcOperations()
				.queryForList(kpointSql.toString(), templateId);
	...
	@Override
	public ResponseObject updateStudyDay(Integer totalDay, Integer oldDay,
			Integer courseId) {
		...
		if (totalDay > oldDay) {
			...
			dao.getNamedParameterJdbcTemplate().getJdbcOperations()
					.batchUpdate(updateSql.toString(), batchArgs);
		}
		// 新的授课天数小于原授课天数删除多余天数和配置的知识点，保留剩余天数数据,
		if (totalDay < oldDay) {
			updateSql = "update oe_chapter set plan_template_id=null where plan_template_id in (select id from oe_plan_template where course_id=? and day>"
					+ totalDay + ")";
			dao.getNamedParameterJdbcTemplate().getJdbcOperations()
					.update(updateSql, courseId);
			updateSql = "delete from oe_plan_template where course_id=? and day>"
					+ totalDay;
			dao.getNamedParameterJdbcTemplate().getJdbcOperations()
					.update(updateSql, courseId);
		}
	...
	public ResponseObject ifExistPlan(Integer courseId) {
		...
		int useTemp = dao.getNamedParameterJdbcTemplate().getJdbcOperations()
				.queryForObject(sql.toString(), Integer.class, courseId);
		if (useTemp > 0) {
	...
	public ResponseObject ifExistTemplate(Integer courseId) {
		...
		int temp = dao.getNamedParameterJdbcTemplate().getJdbcOperations()
				.queryForObject(sql.toString(), Integer.class, courseId);
		if (temp > 0) {
	...
	public ResponseObject savePlanTemplate(Integer totalDay, Integer courseId) {
		...
		for (int i = 1; i < totalDay + 1; i++) {
			Object[] args = new Object[] {
					UUID.randomUUID().toString().replace("-", ""), courseId, i };
			batchArgs.add(args);
		}
		dao.getNamedParameterJdbcTemplate().getJdbcOperations()
	...
	public ResponseObject ifHasResource(Integer courseId) {
		...
		int res = dao.getNamedParameterJdbcTemplate().getJdbcOperations()
				.queryForObject(sql.toString(), Integer.class, courseId);
		if (res > 0) {
			return ResponseObject.newSuccessResponseObject(true);
		} else {
