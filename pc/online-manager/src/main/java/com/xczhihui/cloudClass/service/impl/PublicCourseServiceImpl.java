...
import com.xczhihui.cloudClass.util.Task;
import com.xczhihui.cloudClass.vo.ChangeCallbackVo;
...
public class PublicCourseServiceImpl extends OnlineBaseServiceImpl implements
		PublicCourseService {
	@Autowired
	...
	public Page<CourseVo> findCoursePage(CourseVo courseVo, int pageNumber,
			int pageSize) {
		Page<CourseVo> page = publicCourseDao.findCloudClassCoursePage(
	...
	public List<Lecturer> getTeacher(Integer menuId) {
		String sql = "select * from oe_lecturer where is_delete=0 and role_type =1 and menu_id ="
				+ menuId;
		List<Lecturer> Lecturers = dao.findEntitiesByJdbc(Lecturer.class, sql,
	...
	public void updateCourse(CourseVo courseVo) {
		...
		Course entity = dao.findOneEntitiyByProperty(Course.class, "id",
				courseVo.getId());
		entity.setSmallImgPath(courseVo.getSmallingPath());// 课程展示图
		entity.setBigImgPath(courseVo.getSmallingPath());
		entity.setGradeName(courseVo.getCourseName()); // 课程名称
	...
	public void saveOpenCourseToSend() {
		...
		for (Course course : courseList) {
			if (!"dev".equals(envFlag)) {
				Subscribe.setting(course.getId(), courseService,
						courseSubscribeDao);
			}
		}
		System.out.println("======================初始化" + courseList.size()
				+ "个直播课程预约======================");
	}
