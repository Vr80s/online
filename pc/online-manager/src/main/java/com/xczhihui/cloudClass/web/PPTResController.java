...
public class PPTResController {
	...
	public TableVo findPPTList(TableVo tableVo) {
		...
		Page<Files> page = pptResService.findPage(searchVo, currentPage,
				pageSize);
	...
	public TableVo findCaseList(TableVo tableVo, HttpServletRequest request) {
		...
		Page<Files> page = pptResService.findCasePage(searchVo, currentPage,
				pageSize, domain);
		int total = page.getTotalCount();
		tableVo.setAaData(page.getItems());
		tableVo.setiTotalDisplayRecords(total);
	...
	public ResponseObject add(Files resourcePpt) {
		...
		resourcePpt.setCreatePerson(ManagerUserUtil.getUsername());
		resourcePpt.setStatus(0);
		if (".ppt".equals(resourcePpt.getSuffix())
				|| ".pptx".equals(resourcePpt.getSuffix())) {
