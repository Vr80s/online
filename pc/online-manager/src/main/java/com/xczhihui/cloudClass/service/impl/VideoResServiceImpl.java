...
public class VideoResServiceImpl implements VideoResService {
	...
	public Page<VideoResVo> findPage(VideoResVo searchVo, int currentPage,
		...
		return videoResDao.findCloudVideoResPage(searchVo, currentPage,
				pageSize);
	}
	...
	public List<TreeNode> getChapterTree(String courseId, String courseName) {
		...
		String sql = "select id,name ,parent_id as pId,level ,level as type ,level as contenttype from oe_chapter where is_delete=0 and course_id= "
				+ courseId + " order by sort";
		List<TreeNode> treeNodes = videoResDao.findEntitiesByJdbc(
				TreeNode.class, sql, paramMap);

		List<Map<String, Object>> allvideos = videoResDao
				.getNamedParameterJdbcTemplate()
				.queryForList(
						"select z.id zid,j.id jid,zsd.id zsdid,sp.id spid,sp.is_try_learn from oe_chapter z,oe_chapter j,oe_chapter zsd,oe_video sp "
								+ " where z.id = j.parent_id and j.id = zsd.parent_id and zsd.id = sp.chapter_id and sp.is_delete=0 and z.parent_id = '"
								+ courseId + "' ",
		...
		/*
		 * if(treeNodes!=null&&treeNodes.size()>0){ for(TreeNode
		 * chapter:treeNodes){
		 * 
		 * if(chapter.getLevel()==2){//章 chapter.setActName(chapter.getName());
		 * List<TreeNode>
		 * sections=getSectionByChapter(chapter.getId());//根据章获取获取节 boolean flag
		 * =false; for(TreeNode section : sections){ List<TreeNode> topics =
		 * getTopicBySection(section.getId());//根据节获取知识点 for(TreeNode
		 * topic:topics){ // Map<String, Object> params =null; // String
		 * topicSql =
		 * "select * from oe_video where is_delete=0 and is_try_learn=1 and chapter_id = '"
		 * +topic.getId()+"'"; // List<Video>
		 * videos=videoResDao.findEntitiesByJdbc(Video.class, topicSql, params);
		 * // if(videos!=null&&videos.size()>0){ // flag = true; // }
		 * 
		 * for (Map<String, Object> v : allvideos) { if
		 * (v.get("zsdid").toString().equals(topic.getId()) &&
		 * Boolean.valueOf(v.get("is_try_learn").toString())) { flag = true; } }
		 * } } if(flag){ chapter.setName(chapter.getName()+"---(试学)");
		 * courseVideoflag = true; }
		 * chapter.setName(chapter.getName()+"  "+getVideoCountByChapter
		 * (chapter.getId()));
		 * courseVideoSum=courseVideoSum+getVideoCountByChapter
		 * (chapter.getId()); }
		 * 
		 * 
		 * 
		 * if(chapter.getLevel()==3){//节 chapter.setActName(chapter.getName());
		 * List<TreeNode> topics = getTopicBySection(chapter.getId());//根据节获取知识点
		 * boolean flag =false; for(TreeNode topic:topics){ Map<String, Object>
		 * params =null; String topicSql =
		 * "select * from oe_video where is_delete=0 and is_try_learn=1 and chapter_id = '"
		 * +topic.getId()+"'"; List<Video>
		 * videos=videoResDao.findEntitiesByJdbc(Video.class, topicSql, params);
		 * if(videos!=null&&videos.size()>0){ flag = true; } } if(flag){
		 * chapter.setName(chapter.getName()+"---(试学)"); };
		 * chapter.setName(chapter
		 * .getName()+"  "+getVideoCountBySection(chapter.getId())); }
		 * 
		 * 
		 * if(chapter.getLevel()==4){//知识点
		 * chapter.setActName(chapter.getName()); Map<String, Object> params
		 * =null; String topicSql =
		 * "select * from oe_video where is_delete=0 and is_try_learn=1 and chapter_id = '"
		 * +chapter.getId()+"'"; List<Video>
		 * videos=videoResDao.findEntitiesByJdbc(Video.class, topicSql, params);
		 * if(videos!=null&&videos.size()>0){
		 * chapter.setName(chapter.getName()+"---(试学)"); }
		 * chapter.setName(chapter
		 * .getName()+"   "+getVideoCountByTopic(chapter.getId())); } } }
		 */
	...
	public int getVideoCountByTopic(String topicId) {
		Map<String, Object> paramMap = null;
		String sql = "select * from oe_video where is_delete=0 and chapter_id = '"
				+ topicId + "'";
		List<Video> videos = videoResDao.findEntitiesByJdbc(Video.class, sql,
				paramMap);
	...
	public List<TreeNode> getTopicBySection(String sectionId) {
		Map<String, Object> paramMap = null;
		String sql = "select * from oe_chapter where is_delete=0 and parent_id= '"
				+ sectionId + "'";
		List<TreeNode> TreeNodes = videoResDao.findEntitiesByJdbc(
				TreeNode.class, sql, paramMap);
		return TreeNodes;
	...
	public int getVideoCountBySection(String sectionId) {
		...
		StringBuilder sql = new StringBuilder(
				"select * from oe_video where is_delete=0 ");
		...
		sql.append(" ) ");
		Map<String, Object> paramMap = null;
		List<Video> videos = videoResDao.findEntitiesByJdbc(Video.class,
	...
	public void updateChapter(TreeNode treeNode) {
		Chapter chapter = videoResDao.findOneEntitiyByProperty(Chapter.class,
				"id", treeNode.getId());
	...
	public void deleteChapter(String ids) {
		...
		for (String id : idArr) {
			...
			List<Video> videos = videoResDao.findEntitiesByProperty(
					Video.class, "chapterId", id);
	...
	public void addVideo(Video video) {
		...
		List<Video> temp = videoResDao.findEntitiesByJdbc(Video.class, sql,
				params);
		int sort;
		if (temp.size() > 0) {
		...
		List<UserRVideo> UserRVideos = videoResDao.findByHQL(hql,
				video.getCourseId());
	...
	public void updateVideo(Video video) {
		Video entity = videoResDao.findOneEntitiyByProperty(Video.class, "id",
				video.getId());
	...
	public void deleteVideoById(String id) {
		...
		videoResDao
				.getNamedParameterJdbcTemplate()
				.update("update user_r_video set is_delete = :isDelete where video_id = :videoId",
						paramMap);
	...
	public void deletes(String[] ids) {
		for (String id : ids) {
			Video entity = videoResDao.findOneEntitiyByProperty(Video.class,
					"id", id);
			entity.setDelete(true);
			videoResDao.update(entity);
	...
	public String updateNodeSort(LibraryVo libraryVo) {
		...
		if (chapter.getSort() > sort) {// 移动节点大于目标节点 说明是向下移动
			/*
			 * List<Chapter> list =
			 * videoResDao.findChapterByOrderId(target.getId(),
			 * target.getSort(), chapter.getSort()); for (Chapter
			 * changeObj:list) { changeObj.setSort(changeObj.getSort()+1);
			 * updateChapter(changeObj); } if
			 * (libraryVo.getMoveType().equals("prev")) {//向前移动
			 * chapter.setSort(sort); //更新被拖动的节点 }else{ chapter.setSort(sort+1);
			 * target.setSort(sort); updateChapter(target); }
			 */
			chapter.setSort(sort + 1);
			updateChapter(chapter);
			String sql = "update oe_chapter t set t.sort = t.sort + 1 where sort>"
					+ sort + 1 + " and t.parent_id =:pId and t.id<>:id";
			Map<String, Object> paramMap = new HashMap<String, Object>();
			...
			updateChapter(chapter);
			String sql = "update oe_chapter t set t.sort = t.sort + 1 where sort>"
					+ sort + " and t.parent_id =:pId and t.id<>:id";
			Map<String, Object> paramMap = new HashMap<String, Object>();
			paramMap.put("pId", chapter.getParentId());
			paramMap.put("id", chapter.getId());
			videoResDao.getNamedParameterJdbcTemplate().update(sql, paramMap);
	...
	public void updateSortUp(String id) {
		...
		Video videoNext = videoResDao.findByHQLOne(hqlNext, new Object[] { id,
				videoPre.getChapterId() });
		...
		videoResDao
				.getNamedParameterJdbcTemplate()
				.update("update user_r_video set sort = :sort where video_id = :videoId ",
						paramMapPre);
	...
	public List<Chapter> findVideoUse(String NodeIds, Integer courseId) {
		Map<String, Object> paramMap = null;
		StringBuilder sql = new StringBuilder(
				"SELECT c.id FROM oe_chapter c,oe_video v,user_r_video uv where c.is_delete=0 and v.is_delete=0 and uv.is_delete=0 and v.chapter_id = c.id and v.id =uv.video_id and c.id in( ");
		...
		List<Chapter> chapters = videoResDao.findEntitiesByJdbc(Chapter.class,
				sql.toString(), paramMap);
		return chapters;
	}
	...
	public void updateStatus(String id) {
		...
		videoResDao
				.getNamedParameterJdbcTemplate()
				.update("update user_r_video set status = :status where video_id = :videoId",
						paramMap);
	}
	...
	public List<Video> saveCopyTree(String level, String courentTreeId,
		...
		String sql = " SELECT id,create_person,create_time,is_delete,status,sort,name,parent_id,course_id,level from oe_chapter WHERE id in(SELECT parent_id from oe_chapter WHERE id in (SELECT parent_id from oe_chapter c where is_delete = 0 and id in ('"
				+ str
				+ "') group by c.parent_id))"
				+ " UNION"
				+ " SELECT id,create_person,create_time,is_delete,status,sort,name,parent_id,course_id,level from oe_chapter WHERE id in (SELECT parent_id from oe_chapter c where is_delete = 0 and id in ('"
				+ str
				+ "') group by c.parent_id)"
				+ " UNION"
				+ " SELECT id,create_person,create_time,is_delete,status,sort,name,parent_id,course_id,level from oe_chapter c where is_delete = 0 and id in ('"
				+ str + "') ";
		List<Chapter> chapters = videoResDao.findEntitiesByJdbc(Chapter.class,
				sql, paramMap);// 章、节、知识点

		String sqlVideo = "select * from oe_video where is_delete = 0 and chapter_id in ('"
				+ str + "') order by sort";
		List<Video> videos = videoResDao.findEntitiesByJdbc(Video.class,
				sqlVideo, paramMap);// 选中知识点下的所有视频
			...
			for (int i = 0; i < chapters.size(); i++) {// 循环章、节、知识点
				...
				if (2 == chapters.get(i).getLevel()) {// 章的情况
					...
					for (int j = 0; j < tempsOld.size(); j++) {
						if (tempsOld.get(j).equals(
								chapters.get(i).getParentId())) {// 节父ID是旧章ID的情况下，进行对应替换
							String oldId = chapters.get(i).getId();// 旧节的ID
							chapters.get(i).setParentId(
									oldNew.get(tempsOld.get(j)));
					...
					for (int j = 0; j < tempsOld.size(); j++) {
						if (tempsOld.get(j).equals(
								chapters.get(i).getParentId())) {// 知识点PID是旧节ID的情况下，进行对应替换
	...
	public void addVideoToUerVideo(Integer courseId, Video video) {
		Video temp = videoResDao.findOneEntitiyByProperty(Video.class, "id",
				video.getId());
		// 新增视频同步到用户视频
		String hql = "from UserRVideo where courseId =? group by userId ";
	...
	public List<TreeNode> getChapterTreeCriticize(String courseId,
			String courseName) {
		Map<String, Object> paramMap = null;
		String sql = " select * from (select id,name ,parent_id as pId,level -1 level ,level - 1 as type ,level -1 as contenttype,sort,0 cntNum from oe_chapter where is_delete=0 and course_id= "
				+ courseId
				+ " union all "
				+ " select ov.id, "
				...
				+ " from oe_video ov where ov.is_delete = 0 and ov.course_id = "
				+ courseId
		...
		if (treeNodes != null && treeNodes.size() > 0) {
			for (TreeNode chapter : treeNodes) {
				...
				if (chapter.getLevel() == 1) {// 章
					chapter.setActName(chapter.getName());
					List<TreeNode> sections = getSectionByChapter(chapter
							.getId());// 根据章获取获取节
					boolean flag = false;
					for (TreeNode section : sections) {
						List<TreeNode> topics = getTopicBySection(section
								.getId());// 根据节获取
						for (TreeNode topic : topics) {
							Map<String, Object> params = null;
							String topicSql = "select * from oe_video where is_delete=0 and is_try_learn=1 and chapter_id = '"
									+ topic.getId() + "'";
							List<Video> videos = videoResDao
									.findEntitiesByJdbc(Video.class, topicSql,
											params);
							if (videos != null && videos.size() > 0) {
								flag = true;
							}
					...
					chapter.setName(chapter.getName() + "  "
							+ getCntNumByParent(chapter, treeNodes));// 章 ID
					courseVideoSum = courseVideoSum
							+ getVideoCountByChapter(chapter.getId());
				}
				...
				if (chapter.getLevel() == 2) {// 节
					...
					for (TreeNode topic : topics) {
						...
						String topicSql = "select * from oe_video where is_delete=0 and is_try_learn=1 and chapter_id = '"
								+ topic.getId() + "'";
						List<Video> videos = videoResDao.findEntitiesByJdbc(
								Video.class, topicSql, params);
						if (videos != null && videos.size() > 0) {
							flag = true;
						}
					}
					...
					;
					chapter.setName(chapter.getName() + "  "
							+ getCntNumByParent(chapter, treeNodes));// 节 ID
				}
				...
				if (chapter.getLevel() == 3) {// 知识点
					...
					chapter.setName(chapter.getName() + "   "
							+ getCntNumByParent(chapter, treeNodes));// 知识点 ID
				}
	...
	public List<TreeNode> getChapterTreeBarrier(String courseId,
		...
		Map<String, Object> paramMap = null;
		// String
		// sql="select id,name ,parent_id as pId,level ,level as type ,level as contenttype from oe_chapter where is_delete=0 and course_id= "+courseId
		// +" order by sort";
		String sql = " select * from ( "
				+ " select id,name ,parent_id as pId,level ,level as type ,level as contenttype,sort,true open from oe_chapter where is_delete=0 and course_id= "
				+ courseId
				+ " UNION ALL "
				...
				+ " where ob.is_delete = 0 and och.is_delete = 0 and och.`level` = 4 and ob.course_id = "
				+ courseId + " ) t "
	...
	public List<TreeNode> getChapterTreeBarrierAdd(String courseId,
			String courseName) {
		Map<String, Object> paramMap = null;
		String sql = "select id,name ,parent_id as pId,level ,level as type ,level as contenttype,(case when barrier_Id is not null and barrier_Id != '' and level = 4 then true else false end) chkDisabled from oe_chapter where is_delete=0 and course_id= "
				+ courseId + " order by sort";
		List<TreeNode> treeNodes = videoResDao.findEntitiesByJdbc(
				TreeNode.class, sql, paramMap);
