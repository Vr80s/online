...
import com.xczhihui.cloudClass.vo.CourseLecturVo;
...
public class CourseServiceImpl extends OnlineBaseServiceImpl implements
		CourseService {
	...
	public Page<CourseVo> findCoursePage(CourseVo courseVo, int pageNumber,
			int pageSize) {
		Page<CourseVo> page = courseDao.findCloudClassCoursePage(courseVo,
				pageNumber, pageSize);
		return page;
	...
	@Override
	public Page<CourseVo> findCourseRecPage(CourseVo courseVo, int pageNumber,
			int pageSize) {
		Page<CourseVo> page = courseDao.findCloudClassCourseRecPage(courseVo,
				pageNumber, pageSize);
		return page;
	 ...
	 */
	@Override
	public List<CourseLecturVo> getCourseAndLecturerlist(Integer number,
			Integer courseType, Integer pageNumber, Integer pageSize) {

		List<CourseLecturVo> courseLecturVos = null;
		// 先根据菜单编号获取相关课程信息
		List<Course> courses = this.findCourseListByNumber("", number,
				courseType, pageNumber, pageSize);
		if (!CollectionUtils.isEmpty(courses)) {
			courseLecturVos = new ArrayList<>();
			for (Course course : courses) {
				// 根据教师ID号查找老师信息
				// Lecturer lecturer =
				// lecturerService.findLecturerById(course.getLecturerId());
				...
				courseLecturVo.setCourseType(Integer.valueOf(course
						.getCourseType()));
	 ...
	 * @return Example 分页列表
	 */
	public List<Course> findCourseListByNumber(String name, Integer menuNumber,
			Integer courseType, Integer pageNumber, Integer pageSize) {
		...
		String sql = "";
		if (courseType != 0) {
			sql = "from Course where 1=1 and isDelete=0 and courseType="
					+ courseType + and + " order by  liveTime ";
		} else {
			sql = "from Course where 1=1 and isDelete=0 and menuNumber like '"
					+ menuNumber + "%'" + and + " order by  liveTime ";
		}
		Page<Course> page = dao.findPageByHQL(sql, paramMap, pageNumber,
				pageSize);
	...
	public List<MenuVo> getsecoundMenus(String firstMenuNumber) {
		...
		// String
		// sql="SELECT number,name FROM oe_menu where name!='全部'and number<>:firstMenuNumber and SUBSTR(number FROM 1 FOR 4)=:firstMenuNumber";
	...
	public List<CourseVo> list(String courseType) {
		...
		List<CourseVo> voList = dao.findEntitiesByJdbc(CourseVo.class, sql,
				params);
		return voList;
	}
	...
	public List<CourseVo> getCourselist(String search) {
		...
		String sql = "select *,grade_name as courseName from oe_course where is_delete=0 and grade_name like '%"
				+ search + "%'";
		Map<String, Object> params = new HashMap<String, Object>();
		List<CourseVo> voList = dao.findEntitiesByJdbc(CourseVo.class, sql,
				params);
	...
	public void addCourse(CourseVo courseVo) {
		...
		List<Course> essenceSorttemp = dao.findEntitiesByJdbc(Course.class,
				essenceSortsql, params);
		int essenceSort;
		...
		if (courseVo.getCoursePwd() != null
				&& !"".equals(courseVo.getCoursePwd().trim())) {
	...
	public List<CourseVo> findCourseById(Integer id) {
		...
		// String sql =
		// "SELECT oc.id as id,oc.grade_name as courseName,oc.class_template as classTemplate,oc.subtitle,oc.lecturer,om.name as xMenuName,st.name as scoreTypeName,oc.multimedia_type multimediaType,oc.start_time startTime,oc.end_time endTime,oc.address,"
		// +
		...
		List<CourseVo> courseVoList = dao.findEntitiesByJdbc(CourseVo.class,
				sql, paramMap);
		sql = "select sum(IFNULL(t.default_student_count,0)) from oe_grade t where t.course_id = ?";
		courseVoList.get(0).setLearndCount(
				courseDao.queryForInt(sql, new Object[] { id }));// 累计默认报名人数
		return courseVoList;
	}
	...
	public void updateCourse(CourseVo courseVo) {
		...
		Course course = dao.findOneEntitiyByProperty(Course.class, "id",
				courseVo.getId());
		// 当课程存在密码时，设置的当前价格失效，改为0.0
		if (courseVo.getCoursePwd() != null
				&& !"".equals(courseVo.getCoursePwd().trim())) {
	...
	/**
	 * Description：校验是否重名 creed: Talk is cheap,show me the code
	 * 
		...
		for (Course entity : entitys) {
			...
			if (!entity.isDelete() && entity.getId().intValue() != id) {
				...
				if (entity.getApplyId() == null
						|| !ids.contains(entity.getApplyId())) {
	...
	private void getApplyIdsByChild(Integer applyId, List<Integer> ids) {
		CourseApplyInfo cai = dao.findOneEntitiyByProperty(
				CourseApplyInfo.class, "id", applyId);
	...
	public void updateStatus(Integer id) {
		...
		if (course.getStatus() != null && "1".equals(course.getStatus())) {
			...
			/*
			 * if(course.getType()!=null&&1==course.getType()){ String hqlo
			 * ="from Course where  isDelete=0 and status =1 and type=1";
			 * List<Course> temps=dao.findByHQL(hqlo, new Object[] {});
			 * if(temps.size()>=4){ throw new RuntimeException ("只能启用4条数据！"); }
			 * }
			 */
			course.setStatus("1");
		}
	...
	@Override
	public void updateCourseDetail(String courseId, String smallImgPath,
			String detailImgPath, String courseDetail, String courseOutline,
			String commonProblem, String lecturerDescription) {
		Course c = courseDao.findOneEntitiyByProperty(Course.class, "id",
				Integer.valueOf(courseId));

		// zhuwenbao 2018-01-09 在课程详情页面已经移除调smallImgPath选项
		// 不加判断的话会将之前的smallImgPath设置为null
	...
	@Override
	public void addPreview(String courseId, String smallImgPath,
			String detailImgPath, String courseDetail, String courseOutline,
			String commonProblem) {
		if (courseDetail == null) {
			courseDetail = "";
		}
		CoursePreview c = courseDao.get(Integer.valueOf(courseId),
				CoursePreview.class);
		if (c != null) {
			courseDao
					...
					.getJdbcOperations()
					.execute(
							"delete from oe_course_preview where id = "
									+ courseId);
		}
		courseDao
				.getNamedParameterJdbcTemplate()
				.getJdbcOperations()
				.execute(
						"INSERT INTO oe_course_preview ( " + "	id, "
								+ "	create_person, " + "	create_time, "
								+ "	is_delete, " + "	bigimg_path, "
								+ "	cloud_classroom, " + "	description, "
								+ "	detailimg_path, " + "	grade_name, "
								+ "	graduate_time, " + "	live_time, "
								+ "	smallimg_path, " + "	sort, "
								+ "	courseType, " + "	STATUS, "
								+ "	learnd_count, " + "	original_cost, "
								+ "	current_price, " + "	course_length, "
								+ "	menu_id, " + "	course_type_id, "
								+ "	is_free, " + "	class_template, "
								+ "	course_detail, " + "	course_outline, "
								+ "	common_problem, " + "	lecturer_id, "
								+ "	is_recommend, " + "	recommend_sort, "
								+ "	qqno, " + "	description_show "
								+ ") SELECT " + "	id, " + "	create_person, "
								+ "	create_time, " + "	is_delete, "
								+ "	bigimg_path, " + "	cloud_classroom, "
								+ "	description, " + "	detailimg_path, "
								+ "	grade_name, " + "	graduate_time, "
								+ "	live_time, " + "	smallimg_path, "
								+ "	sort, " + "	courseType, " + "	1, "
								+ "	learnd_count, " + "	original_cost, "
								+ "	current_price, " + "	course_length, "
								+ "	menu_id, " + "	course_type_id, "
		...
		courseDao
				...
				.update("update oe_course_preview set smallimg_path=?,bigimg_path=?,detailimg_path=?,course_detail=?,course_outline=?,common_problem=? where id=?",
						smallImgPath, smallImgPath, detailImgPath,
	...
	public boolean updateCityRec(String[] ids, int isRecommend) {
		...
		for (String id : ids) {
			...
			OffLineCity course = dao.findByHQLOne(hqlPre,
					new Object[] { Integer.valueOf(id) });
			if (course != null) {
				course.setIsRecommend(isRecommend);
				dao.update(course);
	...
	public List<Course> findByName(String name) {
		List<Course> courses = dao.findEntitiesByProperty(Course.class,
				"gradeName", name);
		/*
		 * if(course!=null&&!course.isDelete()){ return course; }else{ return
		 * null; }
		 */
		return courses;
	...
	public Object lectereListByCourseIdAndRoleType(int roleType, String courseId) {
		...
		sql.append("SELECT " + "	ol.id, " + "	ol. name, " + "	CASE "
				+ "WHEN grl.lecturer_id > 0 THEN " + "	1 " + "ELSE " + "	0 "
				+ "END status " + "FROM " + "	oe_lecturer ol "
				+ "LEFT JOIN course_r_lecturer grl ON ( "
				+ "	ol.id = grl.lecturer_id "
				+ "	AND grl.course_id =:courseId " + ") "
				+ "JOIN oe_course oc ON ( " + "	ol.menu_id = oc.menu_id "
				+ "	AND oc.id =:courseId " + ") " + "WHERE "
				+ "	ol.role_type =:roleType " + " AND ol.is_delete = 0 ");
	...
	public void saveTeachers(String gradeId, String courseId, String userName,
		...
		if (roleTypes != null && roleTypes.size() > 0) {
			for (String roleType1 : roleTypes) {
				...
				dao.getNamedParameterJdbcTemplate().update(insertRoleType1,
						roleType1Param);
			}
		}
	}
	...
	public Course findOpenCourseById(Integer id, String version) {
		...
		if (course != null && version != null
				&& version.equals(course.getVersion()) && !course.isDelete()
				&& "1".equals(course.getStatus())) {
			return course;
		}
		return null;
	}
	...
	public String updateCourseVideoInfo(String id) {
		...
		List<Map<String, Object>> vsmp = dao.getNamedParameterJdbcTemplate()
		...
		for (Map<String, Object> map : vsmp) {
			vs.put(String.valueOf(map.get("id")),
		...
		for (String categoryid : categories) {
			for (int i = 1; i < 999999; i++) {
				...
				String requestURL = APIServiceFunction.createHashedQueryString(
						paramsMap, time, OnlineConfig.CC_API_KEY);
				String responsestr = APIServiceFunction
						.HttpRetrieve(Config.api_category_videos + "?"
								+ requestURL);
				...
				Map<String, Object> root = (Map<String, Object>) mp
						.get("videos");
				ArrayList<Object> videos = (ArrayList<Object>) root
						.get("video");
				...
				for (Object object : videos) {
					...
					if (cs.containsKey(title)) {
						double oldduration = Double.valueOf(cs.get(title)
								.split("_#_")[2]);
						if (d > oldduration) {
		...
		for (Map.Entry<String, String> video : vs.entrySet()) {
			...
			if (vinfo != null) {
				...
				sql = "update oe_video set video_id='" + vid + "',video_time='"
						+ ms + "' where id='" + video.getKey() + "' ";
	...
	public void createCourseCategories(String courseId, List<CategoryBean> cs)
			throws Exception {
		String name = null;
		List<Map<String, Object>> lst = dao
				.getNamedParameterJdbcTemplate()
				.getJdbcOperations()
				.queryForList(
						"select o.grade_name from oe_course o where o.is_delete=0 and (o.type=2 or o.type=0) and id="
								+ courseId);
		...
		if (!b) {
			...
			String requestURL = APIServiceFunction.createHashedQueryString(
					paramsMap, time, OnlineConfig.CC_API_KEY);
			String responsestr = APIServiceFunction
					.HttpRetrieve("http://spark.bokecc.com/api/category/create?"
							+ requestURL);
			if (responsestr.contains("error")) {
			...
			Gson g = new GsonBuilder().create();
			Map<String, Object> mp = g.fromJson(responsestr, Map.class);
			Map<String, Object> category = (Map<String, Object>) mp
					.get("category");
		}
	}

	public void createChapterCategories(String courseId, List<CategoryBean> cs)
			throws Exception {
		...
		List<Map<String, Object>> lst = dao
				...
				.getJdbcOperations()
				.queryForList(
						"select o.grade_name,z.`name` from oe_course o,oe_chapter z "
								+ "where z.parent_id=o.id and z.`level`=2 and o.is_delete=0 and z.is_delete=0 and (o.type=2 or o.type=0) and o.id="
								+ courseId);
			...
			for (CategoryBean bean : cs) {
				if (grade_name.equals(bean.getName())) {
					...
					if (!b) {
						...
						paramsMap.put("format", "json");
						long time = System.currentTimeMillis();
						String requestURL = APIServiceFunction
								.createHashedQueryString(paramsMap, time,
										OnlineConfig.CC_API_KEY);
						String responsestr = APIServiceFunction
								.HttpRetrieve("http://spark.bokecc.com/api/category/create?"
										+ requestURL);
						if (responsestr.contains("error")) {
						...
						Gson g = new GsonBuilder().create();
						Map<String, Object> mp = g.fromJson(responsestr,
								Map.class);
						Map<String, Object> category = (Map<String, Object>) mp
								.get("category");
						Thread.sleep(500);
	...
	public String updateCourseVideo(String id) {
		...
		paramMap.put("id", id);
		List<String> courseNames = dao.getNamedParameterJdbcTemplate()
		...
		for (String categoryid : categories) {
			for (int i = 1; i < 999999; i++) {
				...
				for (Object object : videos) {
					...
					String hm = String.valueOf(Double.valueOf(duration)
							.intValue() / 60);
					...
					// if (cs.containsKey(title)) {
					// double oldduration =
					// Double.valueOf(cs.get(title).split("_#_")[2]);
					// if (d > oldduration) {
		...
		if (vinfo != null) {
			String vid = vinfo.split("_#_")[0];
			String ms = vinfo.split("_#_")[1];
			sql = "update oe_course set direct_id='" + vid
					+ "',course_length='" + ms + "' where id=" + id + "";
			dao.getNamedParameterJdbcTemplate().update(sql, paramMap);
		} else {
			msg += (courseName + "<br>");
		}
	...
	public void addCourseCity(String city) {
		...
		if (!findCourseCityByName(city)) {
			...
			List<OffLineCity> temp = dao.findEntitiesByJdbc(OffLineCity.class,
					sql, params1);
			int sort;
			if (temp.size() > 0) {
				sort = temp.get(0).getSort().intValue() + 1;
	...
	public Boolean findCourseCityByName(String city) {
		Map<String, Object> params = new HashMap<String, Object>();
		List<Integer> list = dao.getNamedParameterJdbcTemplate().queryForList(
				"select count(*) as c from oe_offline_city o where  o.city_name = '"
						+ city + "'", params, Integer.class);
	...
	public void deleteCourseCityByName(String city) {
		String savesql = " delete from oe_offline_city where city_name = '"
				+ city + "'";
		Map<String, Object> params = new HashMap<String, Object>();
		dao.getNamedParameterJdbcTemplate().update(savesql, params);
	}
	...
	@Override
	public Page<OffLineCity> getCourseCityList(OffLineCity searchVo,
			Integer pageNumber, Integer pageSize) {
		String sql = "select  *  from  oe_offline_city where  is_delete = 0 and status = 1 ";
		// List<OffLineCity> list= dao.findByHQL(hqlPre);
		if (searchVo.getCityName() != null
				&& !"".equals(searchVo.getCityName())) {
			sql += " and city_name = '" + searchVo.getCityName() + "'";
		}
		sql += " order by is_recommend desc,sort desc ";
		Map<String, Object> params = new HashMap<String, Object>();
		Page<OffLineCity> courseVos = dao.findPageBySQL(sql, params,
				OffLineCity.class, pageNumber, pageSize);
		return courseVos;
	}
	...
	public void updateCourseCityStatus(Integer courseId) {
		 ...
		 */
		List<Integer> list = dao.getNamedParameterJdbcTemplate().queryForList(
				"select count(*) as c from oe_course o where  o.status=1 and "
						+ "o.city = '" + course.getCity() + "'", params,
				Integer.class);
		...
		String savesql = " update  oe_offline_city  set status = '" + status
				+ "' where city_name = '" + course.getCity() + "' ";
		dao.getNamedParameterJdbcTemplate().update(savesql, params);
	}
	...
	public void deleteCourseCityStatus(Integer courseId) {
		...
		// 查出所有为禁用的
		List<Integer> list = dao.getNamedParameterJdbcTemplate().queryForList(
				"select count(*) as c from oe_course o where o.is_delete=0  and "
						+ "o.city = '" + course.getCity() + "'", params,
				Integer.class);
		...
		String savesql = " update  oe_offline_city  set  is_delete= '" + status
				+ "' where city_name = '" + course.getCity() + "' ";
		dao.getNamedParameterJdbcTemplate().update(savesql, params);
	}
	...
	public void updateCourseCity(OffLineCity offLineCity) {
		// TODO Auto-generated method stub
		String savesql = " update  oe_offline_city  set icon = '"
				+ offLineCity.getIcon() + "' where id = '"
				+ offLineCity.getId() + "' ";
		Map<String, Object> params = new HashMap<String, Object>();
		dao.getNamedParameterJdbcTemplate().update(savesql, params);
	}
	...
	public Course findCourseInfoById(Integer id) {
		...
		if ((course.getCollection() == null || !course.getCollection())
				&& course.getDirectId() != null
				&& course.getType() == CourseForm.VOD.getCode()) {
			String audioStr = "";
			if (course.getMultimediaType() == Multimedia.AUDIO.getCode()) {
				audioStr = "_2";
			}
			String src = "https://p.bokecc.com/flash/single/"
					+ OnlineConfig.CC_USER_ID + "_" + course.getDirectId()
		...
		if (course.getCollection() != null && course.getCollection()) {
			List<Course> courses = courseDao.getCourseByCollectionId(course
					.getId());
	...
	public void updateRecommendSort(Integer id, Integer recommendSort,
		...
		if (course != null) {
			if (recommendTime != null && !recommendTime.equals("")) {
				course.setSortUpdateTime(DateUtil.parseDate(recommendTime,
						"yyyy-MM-dd HH:mm:ss"));
			}
	...
	public String updateWebinar(Course entity) {
		// 更新封面
		VhallUtil.setActiveImage(entity.getDirectId(),
				VhallUtil.downUrlImage(entity.getSmallImgPath(), "image"));
		Webinar webinar = new Webinar();
