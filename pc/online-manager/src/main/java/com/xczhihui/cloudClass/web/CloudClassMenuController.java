 ...
 */
@RestController()
@RequestMapping(value = "/cloudClass/menu")
public class CloudClassMenuController {

	@Autowired
	private CloudClassMenuService service;
	@Autowired
	private UserService userService;

	@RequestMapping(value = "/index")
	public ModelAndView index() {
		ModelAndView mav = new ModelAndView("cloudClass/menu");
		return mav;
	}
	...
	public ResponseObject addMenuCourseType(String id, Integer menuId) {
		return ResponseObject.newSuccessResponseObject(service
				.addMenuCourseType(id, menuId));
	}
	...
	public ResponseObject updateMenuCourseType(MenuVo menu) {
		...
		try {
			...
			if (menu.getChildMenuNames() != null
					&& !"".equals(menu.getChildMenuNames())) {
				...
				for (int i = 0; i < ChildMenuAry.length; i++) {
					service.addMenuCourseType(ChildMenuAry[i],
							menu.getParentId());
				}
			}
	...
	public TableVo list(TableVo tableVo) {
		...
		Group typeGroup = groups.findByName("search_status");
		 ...
		 * searchVo
		 * .setTime_start(DateUtil.parseDate(time_startGroup.getPropertyValue1
		 * ().toString(), "yyyy-MM-dd")); } if(time_startGroup!=null){
		 * searchVo.setTime_end
		 * (DateUtil.parseDate(time_endGroup.getPropertyValue1
		 * ().toString(),"yyyy-MM-dd")); }
		 */
		if (nameGroup != null) {
		...
		Page<MenuVo> page = service.findMenuPage(searchVo, currentPage,
				pageSize);
	 ...
	 */
	/*
	 * @RequestMapping(value = "/getAllMenu") public ResponseObject
	 * getAllMenu(){ return
	 * ResponseObject.newSuccessResponseObject(service.getAllMenu
	 * ("",null,null)); }
	 */
	...
	@ResponseBody
	public ResponseObject addChildren(ScoreTypeVo scoreTypeVo)
	...
	@ResponseBody
	public ResponseObject deleteChildren(MenuVo menu)
			throws InvocationTargetException, IllegalAccessException {
	...
	@ResponseBody
	public ResponseObject add(MenuVo menu) throws InvocationTargetException,
		...
		entity.setBoSort(service.getMaxYunSort() + 1);
		entity.setLevel("1");
		entity.setStatus(1);
		entity.setAskLimit(1);
		try {
			service.save(entity);
			responseObj.setSuccess(true);
	...
	@ResponseBody
	public ResponseObject update(MenuVo menu) throws InvocationTargetException,
			IllegalAccessException {
