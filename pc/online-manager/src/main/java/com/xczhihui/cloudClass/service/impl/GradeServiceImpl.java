 ...
 */
@Service
public class GradeServiceImpl extends OnlineBaseServiceImpl implements
		GradeService {
	...
	public Page<GradeVo> findGradeList(GradeDetailVo gradeVo,
		...
		if (StringUtils.isNotEmpty(gradeVo.getMenuId())
				&& !"-1".equals(gradeVo.getMenuId())) {
			sql.append(" and ds.menuId=:menuId");
			paramMap.put("menuId", gradeVo.getMenuId());
		}
		if (StringUtils.isNotEmpty(gradeVo.getScoreTypeId())
				&& !"-1".equals(gradeVo.getScoreTypeId())) {
			sql.append(" and ds.course_type_id=:courseTypeId");
			paramMap.put("courseTypeId", gradeVo.getScoreTypeId());
		}
		if (StringUtils.isNotEmpty(gradeVo.getTeachMethodId())
				&& !"-1".equals(gradeVo.getTeachMethodId())) {
		...
		Page<GradeVo> page = dao.findPageBySQL(sql.toString(), paramMap,
				GradeVo.class, pageNumber, pageSize);
		for (int i = 0; i < page.getItems().size(); i++) {
			...
			page.getItems()
					.get(i)
					.setHasPlan(
							dao.queryForInt(sqlTemp, new Object[] { page
									.getItems().get(i).getId() }));

			sqlTemp = "select max(day) from oe_plan_template t where t.course_id = ? ";
			// page.getItems().get(i).setDefaultStudentCount(dao.queryForInt(sqlTemp,
			// new Object[]{page.getItems().get(i).getCourseId()}));
			// //2017.08.09 yuruixin
		}
		return page;
	}
	...
	public void deleteGrades(String[] ids) {
		...
		if (ids.length > 0) {
			StringBuffer sb = new StringBuffer(
					"update oe_grade set is_delete=1 where is_delete=0 and  id in('");
			StringBuffer sqlDeGRL = new StringBuffer(
					"update grade_r_lecturer set is_delete=1 where  grade_id in('");
			...
			dao.getNamedParameterJdbcTemplate().update(sb.toString(),
					new HashMap<String, Object>());
			dao.getNamedParameterJdbcTemplate().update(sqlDeGRL.toString(),
					new HashMap<String, Object>());
	...
	public List<Map<String, Object>> lectereListByGradeIdAndRoleType(
		...
		sql.append("SELECT " + "	ol.id, " + "	ol. name, " + "	CASE "
				...
				+ "	ol.id = grl.lecturer_id " + "	AND grl.grade_id = :gradeId "
				+ "	AND grl.is_delete = 0 " + ") " + "JOIN oe_course oc ON ( "
				+ "	ol.menu_id = oc.menu_id " + "	AND oc.id = :courseId "
				+ ") " + "WHERE " + "	ol.role_type = :roleType "
				+ " AND ol.is_delete = 0 ");
	...
	public List<CourseVo> findCourseList(Integer pageNumber, Integer pageSize) {
		...
		Page<CourseVo> page = dao.findPageBySQL(sql,
				new HashMap<String, Object>(), CourseVo.class, pageNumber,
				pageSize);
	...
	public Map<String, Object> findMenuByGradeId(String gradeId) {
		...
		return dao.getNamedParameterJdbcTemplate().queryForMap(sql.toString(),
				params);
	}
	...
	public int updateGradeStatus(Integer gradeId, Integer isGradeStatus) {
		String sql = String.format(
				"update oe_grade set grade_status=%d where id =:id",
				new Object[] { isGradeStatus });
	...
	public int updatePreSortEntity(Integer id) {
		...
		List<Grade> grades = dao.getNamedParameterJdbcTemplate().query(sql,
				paramSource, BeanPropertyRowMapper.newInstance(Grade.class));
