...
import com.xczhihui.cloudClass.dao.PlanDao;
...
public class PlanServiceImpl extends OnlineBaseServiceImpl implements
	...
	public void addPlan(PlanVo planVo) {
		...
		GradeVo gradeVo = planDao.getNamedParameterJdbcTemplate()
				.queryForObject(sql, paramMap,
						new BeanPropertyRowMapper<GradeVo>(GradeVo.class));
		...
		sql = "SELECT "
				...
				+ "			when 7 THEN '星期六' " + "	end) week, " + "	opte.day, "
				+ "	0 chuanjiangHas, " + "	null chuanjiangName, "
				+ "	null chuanjiangStartTime,  " + "	null chuanjiangEndTime, "
				+ "	0 chuanjiangDuration, " + "	0 chuanjiangMode, "
				+ "	null chuanjiangRoomId, " + "	null chuanjiangRoomLink, "
				+ "	0 restHas, " + "	'' gradeId " + "FROM "
		...
		List<PlanVo> list = planDao.getNamedParameterJdbcTemplate().query(sql,
				paramMap, new BeanPropertyRowMapper<PlanVo>(PlanVo.class));
		...
		Integer workDaySum = gradeVo.getWorkDaySum() == null ? 0 : gradeVo
				.getWorkDaySum();
		Integer restDaySum = gradeVo.getRestDaySum() == null ? 0 : gradeVo
				.getRestDaySum();
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if ((workDaySum == 1 && i > 0)
					|| (workDaySum > 0 && (i + 1) > workDaySum && (i + 1)
							% workDaySum == 1)) {
				for (int j = 0; j < restDaySum; j++) {
					PlanVo planVoTemp2 = new PlanVo();
					planVoTemp2.setId(UUID.randomUUID().toString()
							.replace("-", ""));
		...
		SqlParameterSource[] sqlParameterSources = SqlParameterSourceUtils
				.createBatch(listParam.toArray());
		planDao.getNamedParameterJdbcTemplate().batchUpdate(sql,
				sqlParameterSources);
	}
	...
	public void addAppendPlan(PlanVo planVo) {
		...
		Map<String, Object> paramMap = new HashMap<String, Object>();
		paramMap.put("id", planVo.getGradeId());
		PlanVo planVoParam = planDao.getNamedParameterJdbcTemplate()
				.queryForObject(sql, paramMap,
						new BeanPropertyRowMapper<PlanVo>(PlanVo.class));

		sql = "select max(t.`day`) from oe_plan_template t where t.course_id = ?";
		Integer maxDay = planDao.queryForInt(sql,
				new Object[] { planVo.getCourseId() });
		if (maxDay > planVoParam.getDay()) {// 只有模板的最大天数 大于现在计划的最大天数 才追加
			...
			Integer restNum = planDao.queryForInt(sql,
					new Object[] { planVo.getGradeId() });
			...
			// 先批量查出应该添加的值
			sql = "SELECT " + "	REPLACE (UUID(), '-', '') id, "
					+ "	opte.id templateId, "
					+ "	date_add(:curriculumTime,INTERVAL (opte.day + "
					+ restNum
					+ ") day) planDate, "
					+ "	(case DAYOFWEEK(date_add(:curriculumTime,INTERVAL (opte.day + "
					+ restNum
					+ ") day))  "
					+ "			when 1 THEN '星期日'  "
					+ "			when 2 THEN '星期一'  "
			...
			List<PlanVo> list = planDao.getNamedParameterJdbcTemplate().query(
					sql, paramMap,
					new BeanPropertyRowMapper<PlanVo>(PlanVo.class));
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if ((workDaySum == 1 && i > 0)
					...
					for (int j = 0; j < restDaySum; j++) {
						...
						planVoTemp2.setRestHas(1);
						planVoTemp2.setGradeId(planVo.getGradeId());
						planVoTemp2.setCreatePerson(planVoTemp
								.getCreatePerson());
						gc.add(5, 1);
						listParam.add(planVoTemp2);
					}
				...
				planVoTemp.setGradeId(planVo.getGradeId());
				listParam.add(planVoTemp);
				gc.add(5, 1);// 第一个参数 取1加1年,取2加半年,取3加一季度,取4加一周 取5加一天 第二个参数为值
								// 此处为加一天
			}
	...
	public void addOneRestPlan(PlanVo planVo) {
		...
		planDao.getNamedParameterJdbcTemplate().getJdbcOperations()
				.update(sql, new Object[] { planVo.getId() });
		...
		planDao.getNamedParameterJdbcTemplate().update(sql,
				new BeanPropertySqlParameterSource(planVoTemp2));
	}
	...
	public List getGradePlanChapter(PlanVo planVo) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (totalSec == 0) {
				...
				int min = totalSec / 60;
				int sec = totalSec % 60;
				planVoTemp.setVideoTime((min < 10 ? "0" + min : min) + ":"
						+ (sec < 10 ? "0" + sec : sec));
			}
	...
	public void updatePlan(PlanVo planVo) {
		String sql = "select chuanjiang_has from oe_plan where id = ?";// 先取出之前是否有串讲
																		// 判断
																		// 是添加串讲
																		// 还是修改串讲
																		// 如果添加串讲
																		// 就发送消息提醒
		Boolean isSendMessage = planDao.queryForInt(sql,
				new Object[] { planVo.getId() }) == 0;
		...
		if (isSendMessage) {// 如果是 新增 需要判断 是否是今天之前
			...
			// 当前时间 大于 开课时间就不能添加
			if (new Date().getTime() > planVo.getchuanjiangStartTime()
					.getTime())// 如果开课时间 是今天 并且 时间大于现在 也不能添加
			{
		...
		sql = "update oe_plan t set " + "	t.chuanjiang_has = ?, "
				+ "	t.chuanjiang_name = ?, " + "	t.chuanjiang_duration = ?, "
				+ "	t.chuanjiang_start_time = ?, "
				+ "	t.chuanjiang_end_time = ?, " + "	t.chuanjiang_mode = ?, "
				+ "	t.chuanjiang_room_id = ?, "
				+ "	t.chuanjiang_lecturer_id = ?, "
				+ "	t.chuanjiang_room_link = ?, " + "	t.micro_course_ids = ? "
				+ " where t.id = ? ";
		planDao.getNamedParameterJdbcTemplate()
				.getJdbcOperations()
				.update(sql,
						new Object[] { planVo.getChuanjiangHas(),
								...
								planVo.getMicroCourseIds(), planVo.getId() });

		Long sc = (planVo.getchuanjiangStartTime().getTime() - new Date()
				.getTime()) / (1000 * 60);
		...
		if (isSendMessage && (sc <= 120)) {// 发送消息提醒 新增 且 在两小时内提醒
			...
			Map map = planDao.getNamedParameterJdbcTemplate().queryForMap(sql,
					paramMap);
			...
			String msg_link = "";
			String msg_link2 = weburl
					+ "/web/html/CourseDetailZhiBo.html?courseId=" + courseId;

			if (planVo.getChuanjiangMode() == 0) {// 本站
				msg_link = weburl + "/web/livepage/" + courseId + "/"
						+ planVo.getChuanjiangRoomId() + "/" + planId;// 直播间链接地址
			} else {// 外部
				msg_link = planVo.getChuanjiangRoomLink();
			}

			// 拼出消息
			String time = new SimpleDateFormat("MM月dd日 HH:mm").format(planVo
					.getchuanjiangStartTime());
			String message = "您的班级<a href=\"javascript:void(0)\" onclick=\"on_click_msg('msg_id','"
					+ msg_link2
					+ "');\">"
					+ gradeName
					+ "</a>新增一门串讲课，名称为《"
					...
					+ "》,"
					+ time
					+ "开课,<a href=\"javascript:void(0)\" onclick=\"on_click_msg('msg_id','"
					+ msg_link + "');\">点击进入直播教室>></a>";
			...
			if (planVo.getMicroCourseIds() != null
					&& !"".equals(planVo.getMicroCourseIds())) {
				message = "职业课&nbsp;"
						+ gradeName
						+ "&nbsp;新增一门串讲课，名称为《"
						+ planVo.getChuanjiangName()
				...
				// 再发送给包含的微课用户
				sql = "insert into oe_message (id,context,user_id,type,create_person,create_time,status,is_online) "
						+ " select max(a.id),replace(:context,'msg_id',max(a.id)),a.user_id,1,:createPeson,now(),1,0 from(select REPLACE(UUID(),'-','') id,oa.user_id from oe_apply oa LEFT JOIN apply_r_grade_course arg on oa.id= arg.apply_id where arg.grade_id<>:gradeId and arg.course_id in("
			...
			// 设置为已发送
			sql = "update oe_plan t set " + "	t.chuanjiang_is_send = 1 "
					+ " where t.id = ? ";
			planDao.getNamedParameterJdbcTemplate().getJdbcOperations()
					.update(sql, new Object[] { planVo.getId() });
		}
	...
	public List exportExcelPlan(PlanVo planVo) {
		String sql = " select id,planDate,name,week,day,CONVERT(videoTime,CHAR(100)) videoTime,sort,chuanjiangHas from (SELECT "
				...
				+ " where (t.chuanjiangHas = 0 or (t.chuanjiangHas = 1 and t.`name` is not null and t.`name` <> ''))"
				+ // #剔除掉有串讲 但是没有知识点的记录
				" order by t.planDate  ,t.sort asc ";
		Map<String, Object> paramMap = new HashMap<String, Object>();
		paramMap.put("gradeId", planVo.getGradeId());
		List<Map<String, Object>> list = planDao
				.getNamedParameterJdbcTemplate().queryForList(sql, paramMap);
		for (int i = 0; i < list.size(); i++) {
			Map<String, Object> map = list.get(i);
			if (map.get("sort") == null
					|| !"999999999999".equals(map.get("sort").toString())) {// 不处理休息日
				int totalSec = Integer
						.parseInt(map.get("videoTime").toString());
				if (totalSec == 0) {
					map.put("videoTime", "00:00");
				} else {
					int min = totalSec / 60;
					int sec = totalSec % 60;
					map.put("videoTime", (min < 10 ? "0" + min : min) + ":"
							+ (sec < 10 ? "0" + sec : sec));
				}
			}
			list.set(i, map);
	...
	public List getLecturers(Integer courseId) {
		String sql = " SELECT " + "	t2.`name`, " + "	t2.id " + " FROM "
				+ "	course_r_lecturer t, " + "	oe_lecturer t2 " + " WHERE "
				+ "	t.lecturer_id = t2.id " + " AND t2.is_delete = 0 "
				+ " AND t.is_delete = 0 " + " AND t.course_id = :courseId ";
		Map<String, Object> paramMap = new HashMap<String, Object>();
		paramMap.put("courseId", courseId);
		return planDao.getNamedParameterJdbcTemplate().queryForList(sql,
				paramMap);
	}
	...
	public List getMicroCourseList(Integer courseId) {
		String sql = " SELECT "
				...
				+ "			oe_course t2 " + "		WHERE "
				+ "			t2.menu_id = t.menu_id " + "		AND t2.id = :courseId ) ";
