...
import com.xczhihui.cloudClass.service.CriticizeService;
...
public class CriticizeController {
	...
	public TableVo findCriticizeList(TableVo tableVo) {
		...
		if (gradeIdGroup != null) {
			searchVo.setGradeId(Integer.parseInt(gradeIdGroup
					.getPropertyValue1().toString()));
		}
		...
		if (chapterLevelGroup != null) {
			if (!"4".equals(chapterLevelGroup.getPropertyValue1().toString())) {
				searchVo.setChapterId(chapterIdGroup.getPropertyValue1()
						.toString());
			} else {
				searchVo.setVideoId(chapterIdGroup.getPropertyValue1()
						.toString());
			}
		}
		...
		if (sortType != null) {
			searchVo.setSortType(Integer.valueOf(sortType.getPropertyValue1()
					.toString()));
		}
		if (response != null) {
		...
		Page<CriticizeVo> page = criticizeService.findCriticizePage(searchVo,
	...
	public TableVo findCourseList(TableVo tableVo) {
		...
		Page<CourseVo> page = criticizeService.findCoursePage(searchVo,
				currentPage, pageSize);
		int total = page.getTotalCount();
		tableVo.setAaData(page.getItems());
		tableVo.setiTotalDisplayRecords(total);
	...
	public ModelAndView criticizeDetail(CriticizeVo criticizeVo,
			HttpServletRequest request) {
		ModelAndView mav = new ModelAndView("/cloudClass/criticizeDetail");
		request.setAttribute("gradeList",
				gradeService.getGradeByCourseId(criticizeVo.getCourseId()));
		return mav;
	}
