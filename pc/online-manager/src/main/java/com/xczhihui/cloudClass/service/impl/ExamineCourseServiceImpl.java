...
import com.xczhihui.cloudClass.dao.ExamineCourseDao;
import com.xczhihui.cloudClass.dao.PublicCourseDao;
...
import com.xczhihui.cloudClass.vo.*;
...
public class ExamineCourseServiceImpl extends OnlineBaseServiceImpl implements
		ExamineCourseService {
	@Autowired
	...
	public Page<LiveExamineInfoVo> findCoursePage(
		...
		Page<LiveExamineInfoVo> page = examineCourseDao
				.findCloudClassCoursePage(liveExamineInfoVo, pageNumber,
						pageSize);
		return page;
	}
	...
	public void updateCourse(LiveExamineInfoVo le) {
		...
		LiveExamineInfo entity = dao.findOneEntitiyByProperty(
				LiveExamineInfo.class, "id", le.getId());
	...
	public void synchronizingCourse(LiveExamineInfo le) {
		 ...
		 */
		Course course = publicCourseService.findCourseVoByLiveExanmineId(le
				.getId());
		if (course == null && "1".equals(le.getExamineStatus())) { // 保存课程，并且生成课程id
			...
			/*
			 * double f = Double.parseDouble(l+""); BigDecimal b = new
			 * BigDecimal(f); double f1 = b.setScale(2,
			 * BigDecimal.ROUND_HALF_UP).doubleValue();
			 */
			// BigDecimal bd=new BigDecimal(l+"");//建议使用String参数
			// //BigDecimal bd_half_even =
		...
		} else if (course != null && "2".equals(le.getExamineStatus())) { // 更新
																			// 直播课程状态变为0
			// 更新课程仅仅是把状态变为：0
			course.setStatus("0");
			dao.update(course);
	...
	public String updateApply(String id) {
		LiveExamineInfo lei = dao.findOneEntitiyByProperty(
				LiveExamineInfo.class, "id", id);

		/**
		 * 判断这个用户是否拥有讲师权限啦
		 */
		OnlineUser ou = onlineUserService
				.getOnlineUserByUserId(lei.getUserId());
	...
	public Page<LiveExamineInfoVo> findAppealListPage(
			LiveExamineInfoVo liveExamineInfoVo, int pageNumber, int pageSize) {
		Page<LiveExamineInfoVo> page = examineCourseDao
				.findCloudClassCoursePage1(liveExamineInfoVo, pageNumber,
						pageSize);
		return page;
	}
	...
	public void deletes(String[] _ids) {
		...
		for (String id : _ids) {
			...
			if (lei != null) {
				 ...
				 * 
				 * 如果删除了审核通过的那么就让此课程不显示在前台。对应中的直播课程也变为无效。 如果删除了未审核的
				 * 如果删除了审核未通过的那么就让此课程变为未审核的状态。对应的申诉中的信息也变成了无效的。
				 */
				if (lei.getExamineStatus() != null
						&& "1".equals(lei.getExamineStatus())) {
	...
	public void updateCxBoHui(String id) {
		...
		List<LiveAppealInfo> list = examineCourseDao
				.findeAppealInfosByExamineId(id);
	...
	public List<LiveAppealInfoVo> getApplysByExamId(String id) {
		// TODO Auto-generated method stub
		List<LiveAppealInfoVo> list = examineCourseDao
				.findeAppealInfoVosByExamineId(id);
		return list;
	}
