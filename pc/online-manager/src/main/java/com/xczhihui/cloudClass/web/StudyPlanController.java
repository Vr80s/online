...
public class StudyPlanController extends AbstractController {
	...
	public String template(HttpServletRequest request, Integer courseId,
			String courseName, Integer totalDay) {
	...
	public ResponseObject planInfo(Integer courseId, Integer week) {
		return ResponseObject.newSuccessResponseObject(service.planInfo(
				courseId, week));
	}
	...
	@ResponseBody
	public ResponseObject editStudyDay(Integer totalDay, Integer oldDay,
			Integer courseId) {
	...
	@ResponseBody
	public ResponseObject savePlanKnowledge(String templateId,
			String knowledgeIds) {
		...
		if (knowledgeIds != null) {
			...
			for (String id : ids) {
				if (id.split("_").length == 2
						&& "kpoint".equalsIgnoreCase(id.split("_")[0])) {
