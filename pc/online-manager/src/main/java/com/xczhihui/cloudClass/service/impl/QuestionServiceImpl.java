...
public class QuestionServiceImpl extends OnlineBaseServiceImpl implements
	...
	public List<TreeNode> findDataForTreeRight(String currentNodeIds) {
		...
		if (StringUtils.isNotEmpty(currentNodeIds)) {
			...
			String sql = "SELECT DISTINCT(p.id),p.name ,p.parent_id as pId,p.level ,p.level as type ,p.level as contenttype  from oe_chapter z ,oe_chapter j ,oe_chapter p where z.id=j.parent_id and j.id=p.parent_id and p.id in( '"
					+ str
					+ "' ) UNION All "
					+ "SELECT DISTINCT(j.id), j.name ,j.parent_id as pId,j.level ,j.level as type ,j.level as contenttype  from oe_chapter z ,oe_chapter j ,oe_chapter p where z.id=j.parent_id and j.id=p.parent_id and p.id in( '"
					+ str
					+ "' )  UNION All "
					+ "SELECT DISTINCT(z.id),z.name ,z.parent_id as pId,z.level ,z.level as type ,z.level as contenttype  from oe_chapter z ,oe_chapter j ,oe_chapter p where z.id=j.parent_id and j.id=p.parent_id and p.id in( '"
					+ str + "' ) ";
			List<TreeNode> treeNodes = dao.findEntitiesByJdbc(TreeNode.class,
	...
	public List<TreeNode> findDataForTreeLeft(String courseId) {
		...
		List<TreeNode> treeNodes = dao.findEntitiesByJdbc(TreeNode.class, sql,
				paramMap);
	...
	public Page<QuesStore> findPage(QuesStore searchVo, int currentPage,
			int pageSize) {
		// TODO Auto-generated method stub
		Page<QuesStore> res = quesStoreDao.findQuesPage(searchVo, currentPage,
				pageSize);
	...
	public List<QuestionOptionPicture> getQopList(String questionId,
		...
		if (ques != null) {
			...
			if (ques.getOptionsPicture() != null) {
				...
				ArrayList<String> array = g.fromJson(ques.getOptionsPicture(),
						ArrayList.class);
		...
		if (!"".endsWith(question.getChapterId())) {
			...
			if (!CollectionUtils.isEmpty(list)) {
				for (String kpId : list) {
					...
					dao.getNamedParameterJdbcTemplate().update(sqlInsert,
							params);
	...
	public void deleteById(String id, String chapterIds) {
		...
		if (chapterIds == null || "".equals(chapterIds)) {
			...
			for (String chapterId : array) {
				...
				dao.getNamedParameterJdbcTemplate().getJdbcOperations()
						.update(sql, entity.getId(), chapterId);
			}
		}
	...
	@Override
	public StringBuffer checkXls(Sheet sheet, List<QuesStore> excelList,
		...
		for (int i = 1; i <= totalrow; i++) {
			...
			List<Course> s = dao
					.findEntitiesByJdbc(
							Course.class,
							"select * from oe_course c where c.is_delete=0 and c.type=2 and c.grade_name = '"
									+ name + "'", new HashMap<String, Object>());
			if (!courseName.equals(name)) {
				content.append(" {\"第" + rowIndex + "行 课程名称 " + name + version
						+ " 不符合当前课程！\"}");
				...
				if (knowledgePoints != null && !"".equals(knowledgePoints)) {
					...
					if (array != null && array.length > 0) {
						for (String string : array) {
							if (!isKnowledgeTemplateNotExist(rowIndex, "知识点",
									ksysId, string, content)) {
								ischecked = false;
							} else {
								Map<String, Object> paramMap = new HashMap<String, Object>();
								String sql = "select * from oe_chapter where level=4 and name = '"
										+ string
										+ "' and course_id="
										+ Integer.parseInt(ksysId);
								List<Chapter> k = dao.findEntitiesByJdbc(
										Chapter.class, sql, paramMap);
								if (k != null) {
									knowledgePointIds.append(k.get(0).getId())
											.append(",");
								}
							}
						}
			...
			if (!("填空题".equals(type) || "简答题".equals(type)
					|| "代码题".equals(type) || "实操题".equals(type))) {
			...
			if (!("填空题".equals(type) || "代码题".equals(type) || "实操题"
					.equals(type))) {
				if (!isEmpty(rowIndex, "答案", answer, content)) {
					ischecked = false;
				} else if ("单选题".equals(type)
						&& !isAnswerError(rowIndex, "答案", option, answer,
								content)) {
					ischecked = false;
				}
			}
			...
			if (questionType == 3) {// 填空
				...
				questionContent = ParseUtil.replaceAnswers(questionContent,
						ParseUtil._REGEX, "");
				...
				for (int j = 0; j < questionContentArray.length; j++) {
					if (j > 0) {
						questionContentStringBuffer
								.append("<p>")
								.append(questionContentArray[j].replaceAll(" ",
										"&nbsp;")).append("</p>");
					} else {
						questionContentStringBuffer
								.append(questionContentArray[j].replaceAll(" ",
			...
			if (questionType == 0) {// 单选
				...
				for (String string : array) {
					if (!StringUtil.checkNull(string)) {
						String optionval = string.substring(
				...
				for (String string : array) {
					if (!StringUtil.checkNull(string)) {
						os.add(string.substring(string.indexOf(".") + 1,
								string.length()));
					}
				}
				question.setOptions(gson.toJson(os));
				...
				String v = answer.substring(answer.lastIndexOf(".") + 1,
						answer.length()).toUpperCase();
				if ("TXT".equals(v) || "DOC".equals(v) || "DOCX".equals(v)
					...
					if (zipSet.contains(answer)) {
						...
						content.append("第" + rowIndex + "行附件（ " + answer
								+ "）在压缩包不存在！<br/>");
					}
				} else {
					content.append("第" + rowIndex + "行附件（ " + answer
							+ "）格式错误！请上传txt,word,excel,zip,rar或者图片的附件格式.<br/>");
				}
				...
				if ("TXT".equals(v) || "DOC".equals(v) || "DOCX".equals(v)
					...
					content.append("第" + rowIndex + "行附件（ " + answer
							+ "）格式错误！请上传txt,word,excel,zip,rar的附件格式.<br/>");
				}
			}
			if (!"".equals(difficult) && !"简单".equals(difficult)
					&& !"一般".equals(difficult) && !"困难".equals(difficult)
					&& !"非常困难".equals(difficult)) {
			...
			case "简单":
				...
				break;
			case "一般":
				question.setDifficulty("B");
				break;
			case "困难":
				question.setDifficulty("C");
				break;
			case "非常困难":
				question.setDifficulty("D");
				break;
			default:
				break;
			}
			...
			if (!StringUtil.checkNull(solution)) {
				...
				for (int j = 0; j < solutionArray.length; j++) {
					if (j > 0) {
						solutionStringBuffer
								.append("<p>")
								.append(solutionArray[j].replaceAll(" ",
										"&nbsp;")).append("</p>");
					} else {
						solutionStringBuffer.append(solutionArray[j]
								.replaceAll(" ", "&nbsp;"));
					}
				}
			}
	...
	private boolean isEmpty(int rowIndex, String colName, String value,
	 ...
	 * @return
	 */
	private boolean isCourseNotExist(int rowIndex, String colName,
			String value, StringBuffer content) {
		Course ks = dao.findOneEntitiyByProperty(Course.class, "gradeName",
				value);
		// KSystem ks = system.isExsitName(value, null);
		if (null == ks) {
			content.append(" {\"第" + rowIndex + "行 " + colName + value
					+ "不存在！\"}");
			return false;
		}
		return true;
	...
	private boolean isEmptyKsystem(int rowIndex, String colName, String value,
			StringBuffer content) {
		if (StringUtils.isEmpty(value)) {
			content.append("第" + rowIndex + "行 " + colName + "不能为空！<br/>");
			return false;
			...
			Pattern pattern = Pattern
					.compile("^(?!_)(?!.*?_$)[\\u4E00-\\u9FA5\\uF900-\\uFA2D\\w\\[\\]\\+]{0,}$");
			if (!pattern.matcher(value.trim()).matches()) {
				content.append("第" + (rowIndex + 1) + "行 " + colName
						+ "格式错误！<br/>");
				return false;
			}
		}
	 ...
	 * @return
	 */
	private boolean isKnowledgeTemplateNotExist(int rowIndex, String colName,
			String ksysid, String value, StringBuffer content) {
		Map<String, Object> paramMap = new HashMap<String, Object>();
		String sql = "select * from oe_chapter where level =4 and name = '"
				+ value + "' and course_id=" + Integer.parseInt(ksysid);
		List<Chapter> kPoint = dao.findEntitiesByJdbc(Chapter.class, sql,
				paramMap);
	...
	private boolean isOptionError(int rowIndex, String colName, String option,
		...
		for (String string : array) {
			...
			if (k < 0) {
				...
				String optionval = string.substring(string.indexOf(".") + 1,
						string.length());
				if ("".equals(optionval) || optionval == null) {
					content.append(" {\"第" + rowIndex + "行 " + colName
							+ "答案有空的！\"}");
				} else {
					os.add(optionval);
				}
	...
	private boolean isAnswerError(int rowIndex, String colName, String option,
			String answer, StringBuffer content) {
		List<String> os = new ArrayList<String>();
		String[] array = option.split(";");
		for (String string : array) {
	...
	@Override
	public String saveBatch(List<QuesStore> excelList,
			Map<String, byte[]> zipMap, String ticket) {
		try {
			// 保存数据
			for (int i = 0; i < excelList.size(); i++) {
				...
				/*
				 * // 处理附件题 if (q.getQuestionType() == 5 || q.getQuestionType()
				 * == 6) { String name = q.getAnswer(); if
				 * (StringUtils.isNotBlank(name)) { Attachment attachment =
				 * attachmentCenterService
				 * .addAttachment(ManagerUserUtil.getId()
				 * ,AttachmentType.QUESTION, name,zipMap.get(name),
				 * "zip",ticket);
				 * 
