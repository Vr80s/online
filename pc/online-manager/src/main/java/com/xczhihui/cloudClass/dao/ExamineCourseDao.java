...
import com.xczhihui.cloudClass.vo.LiveAppealInfoVo;
...
public class ExamineCourseDao extends HibernateDao<Course> {
	...
	public Page<CourseVo> findCloudClassCoursePage(CourseVo courseVo,
		...
		// StringBuilder("select c.*,c.grade_name as courseName ,l.name as lecturerName ,m.name as menuName from oe_course c,oe_menu m,oe_lecturer l where c.menu_id=m.id and c.lecturer_id = l.id and c.is_delete=0 and c.type=1");
		/**
		 * 2017-08-13 杨宣修改 讲师角色切换 当前 --用户角色
		 * 
		 * oe_course_mobile 从这个里面判断是否存在课程详情啊
		 */
		StringBuilder sql = new StringBuilder(
				"select c.*,c.grade_name as courseName ,ou.name as lecturerName ,m.name as menuName,c.`course_pwd` coursePwd "
	...
	public Page<LiveExamineInfoVo> findCloudClassCoursePage(
			LiveExamineInfoVo liveExamineInfoVo, int pageNumber, int pageSize) {
		// TODO Auto-generated method stub
		Map<String, Object> paramMap = new HashMap<String, Object>();

		StringBuilder sql = new StringBuilder(
				"select le.*,ou.name as lecturerName ,m.name as menuName,"
		...
		Page<LiveExamineInfoVo> pageList = this.findPageBySQL(sql.toString(),
				paramMap, LiveExamineInfoVo.class, pageNumber, pageSize);

		/*
		 * for (LiveExamineInfoVo lv : pageList.getItems()) {
		 * if(lv.getPrice()!=null){ BigDecimal bd = lv.getPrice(); BigDecimal
		 * bd_half_up = bd.setScale(2,RoundingMode.HALF_UP);
		 * lv.setPrice(bd_half_up); BigDecimal bd = lv.getPrice();
	...
	public List<LiveAppealInfo> findeAppealInfosByExamineId(String examineId) {
		...
		return this.getNamedParameterJdbcTemplate().query(sql.toString(),
				paramMap,
				BeanPropertyRowMapper.newInstance(LiveAppealInfo.class));
	}
	...
	public List<LiveAppealInfoVo> findeAppealInfoVosByExamineId(String examineId) {
		...
		return this.getNamedParameterJdbcTemplate().query(sql.toString(),
				paramMap,
				BeanPropertyRowMapper.newInstance(LiveAppealInfoVo.class));
	}
	...
	public Page<LiveExamineInfoVo> findCloudClassCoursePage1(
		...
		StringBuilder sql = new StringBuilder(); // live_appeal
													// live_examine_info
