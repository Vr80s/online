...
public class VideoResController {
	...
	public TableVo findVideoList(TableVo tableVo) {
		...
		searchVo.setCourseId(Integer.valueOf(courseIdSearch.getPropertyValue1()
				.toString()));
		if (chapterIdSearch != null) {
			searchVo.setChapterId(chapterIdSearch.getPropertyValue1()
					.toString());
		}
		Page<VideoResVo> page = videoResService.findPage(searchVo, currentPage,
				pageSize);
	...
	@ResponseBody
	public ResponseObject enbaleStatus(String ids)
	...
	@ResponseBody
	public ResponseObject disableStatus(String ids)
			throws InvocationTargetException, IllegalAccessException {
		ResponseObject responseObject = new ResponseObject();
		if (ids != null) {
			String[] _ids = ids.split(",");
	...
	@ResponseBody
	public List<TreeNode> showTree(HttpServletRequest request, String courseId,
			String courseName, String currentNodePid) {

		List<TreeNode> list = videoResService.getChapterTree(courseId,
				courseName);
		String str = "";
	...
	@ResponseBody
	public List<TreeNode> showTreeCriticize(HttpServletRequest request,
			String courseId, String courseName) {

		List<TreeNode> list = videoResService.getChapterTreeCriticize(courseId,
				courseName);
		for (TreeNode treeNode : list) {
			treeNode.setNocheck(true);
		}
	...
	@ResponseBody
	public List<TreeNode> showTreeNotes(HttpServletRequest request,
			String courseId, String courseName) {

		List<TreeNode> list = videoResService.getChapterTreeNotes(courseId,
				courseName);
		for (TreeNode treeNode : list) {
			treeNode.setNocheck(true);
		}
	...
	@ResponseBody
	public List<TreeNode> showTreeBarrier(HttpServletRequest request,
			String courseId, String courseName) {

		List<TreeNode> list = videoResService.getChapterTreeBarrier(courseId,
				courseName);
		for (TreeNode treeNode : list) {
			treeNode.setNocheck(true);
	...
	@ResponseBody
	public List<TreeNode> showTreeBarrierAdd(HttpServletRequest request,
			String courseId, String courseName) {
	...
	@ResponseBody
	public List<TreeNode> showTreeExamPaperAdd(HttpServletRequest request,
			String courseId, String courseName) {
	...
	@ResponseBody
	public List<TreeNode> getTreeByCourse(HttpServletRequest request,
			String courseId, String courseName, String level) {
		List<TreeNode> list = videoResService.getChapterTree(courseId,
				courseName);
		for (TreeNode treeNode : list) {
	...
	@ResponseBody
	public ResponseObject saveCopyTree(String level, String courentTreeId,
			Integer courseId, String[] kpos) {
		ResponseObject responseObj = new ResponseObject();
		List<Video> videos = videoResService.saveCopyTree(level, courentTreeId,
				courseId, kpos);
	...
	public TreeNode addNode(HttpServletRequest request, TreeNode treeNode) {
		...
		// try {
		// this.videoResService.createCCCategory(chapter.getId(),
		// chapter.getCourseId(),treeNode.getName());
		// } catch (Exception e) {
		// e.printStackTrace();
		// }
		// }
	...
	@ResponseBody
	public String deleteNode(HttpServletRequest request, String id,
			String contenttype) {
	 ...
	 * @throws IOException
	 */
	// @RequestMapping(value = "uploadSuccessCallback", method =
	// RequestMethod.GET)
	// public void uploadSuccessCallback(HttpServletResponse res, String
	// duration, String image, String status,
	// String videoid, String time, String hash) throws IOException {
	// videoResService.uploadSuccessCallback(duration, image, status, videoid,
	// time, hash);
