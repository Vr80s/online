 ...
 */
@Service
public class ArticleTypeServiceImpl extends OnlineBaseServiceImpl implements
		ArticleTypeService {
	@Override
	public Page<ArticleTypeVo> findArticleTypePage(ArticleTypeVo typeVo,
		...
		StringBuilder sql = new StringBuilder(
				"SELECT at.id,at.`name`,at.create_time,at.`status`,");
		...
		Page<ArticleTypeVo> types = dao.findPageBySQL(sql.toString(), paramMap,
				ArticleTypeVo.class, pageNumber, pageSize);
		types.getItems()
				.stream()
				.filter(temp -> typeVo.getSortType() != null
						&& typeVo.getSortType() == 1)
				.forEach(temp -> temp.setSortType(1));
		return types;
	}
	...
	public void saveType(ArticleTypeVo typeVo) {
		...
		int count = dao.getNamedParameterJdbcTemplate().getJdbcOperations()
				.queryForObject(sql, Integer.class, typeVo.getName());
		if (count > 0) {
	...
	public void updateTypeById(ArticleTypeVo typeVo) {
		...
		if (count > 0) {
			...
			dao.getNamedParameterJdbcTemplate().getJdbcOperations()
					.update(sql, typeVo.getName(), typeVo.getId());
		}
	}
	...
	public void updateStatus(String id) {
		...
		List<Map<String, Object>> types = dao.getNamedParameterJdbcTemplate()
				.getJdbcOperations().queryForList(sql, id);
		...
		if (type != null) {
			...
			if (1 == status) {// 禁用
				...
				int count = dao.getNamedParameterJdbcTemplate()
	...
	public void deletes(Set<String> ids) {
		...
		int count = dao.getNamedParameterJdbcTemplate().queryForObject(sql,
	...
	public void updateSortUp(String id) {
		// 查询排序相邻的两个文章分类
		String sql = "select id,name,create_time,status,sort from article_type where id=?";
		List<Map<String, Object>> typeList = dao
		...
		String nextSql = "select id,name,create_time,status,sort from article_type where sort >"
				+ sort + " and status=1 order by sort asc limit 1";
		List<Map<String, Object>> nextTypeList = dao
				.getNamedParameterJdbcTemplate().getJdbcOperations()
				.queryForList(nextSql);
		Map<String, Object> nextType = nextTypeList.size() > 0 ? nextTypeList
				.get(0) : null;
		...
		if (type != null && nextType != null) {
			...
			dao.getNamedParameterJdbcTemplate().getJdbcOperations()
					.update(edit_sql, nextSort, preId);
			dao.getNamedParameterJdbcTemplate().getJdbcOperations()
					.update(edit_sql, sort, nextId);
		}
	}
	...
	public void updateSortDown(String id) {
		...
		int sort = Integer.parseInt(type.get("sort").toString());
		String nextSql = "select id,name,create_time,status,sort from article_type where sort <"
				+ sort + " order by sort desc limit 1";
		List<Map<String, Object>> nextTypeList = dao
				.getNamedParameterJdbcTemplate().getJdbcOperations()
				.queryForList(nextSql);
		Map<String, Object> nextType = nextTypeList.size() > 0 ? nextTypeList
