...
@Service
public class BannerServiceImpl extends OnlineBaseServiceImpl implements
		BannerService {
	@Autowired
	...
	public Page<ArticleVo> findBannerPage(ArticleVo articleVo, int currentPage,
		...
		Page<ArticleVo> page = bannerDao.findBannerPage(articleVo, currentPage,
				pageSize);
		return page;
	}
	...
	public void updateSortUp(Integer id) {
		...
		List<Map<String, Object>> articleList = dao
				.getNamedParameterJdbcTemplate().getJdbcOperations()
				.queryForList(sql, id);
		Map<String, Object> vo = articleList.size() > 0 ? articleList.get(0)
				: null;
		String preId = vo.get("id").toString();
		int sort = Integer.parseInt(vo.get("sort").toString());
		String nextSql = "select id,title,create_time,status,sort from oe_bxs_article  where is_recommend =1 and sort >"
				+ sort + " order by sort asc limit 1";
		List<Map<String, Object>> nextArticleList = dao
				.getNamedParameterJdbcTemplate().getJdbcOperations()
				.queryForList(nextSql);
		Map<String, Object> nextVo = nextArticleList.size() > 0 ? nextArticleList
				.get(0) : null;
