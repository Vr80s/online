...
public class AppraiseDao extends SimpleHibernateDao {
	...
	public Page<AppraiseVo> findAppraisePage(AppraiseVo appraiseVo,
		...
		StringBuilder sql = new StringBuilder(
				"SELECT a.id , a.content ,a.article_id ,a.user_id name ,a.create_time from oe_bxs_appraise a ,oe_bxs_article art where a.is_delete = 0 and a.article_id = art.id ");

		if (appraiseVo.getArticleId() != null
				&& !"".equals(appraiseVo.getArticleId())) {
			sql.append(" and a.article_id = :articleId ");
			paramMap.put("articleId", appraiseVo.getArticleId());
		}

		if (appraiseVo.getContent() != null
				&& !"".equals(appraiseVo.getContent())) {
		...
		Page<AppraiseVo> page = this.findPageBySQL(sql.toString(), paramMap,
				AppraiseVo.class, currentPage, pageSize);
		return page;
	}

	public void deleteById(String id) {
		StringBuilder sql = new StringBuilder(
				"update oe_bxs_article t set t.praise_sum = t.praise_sum - 1 "
