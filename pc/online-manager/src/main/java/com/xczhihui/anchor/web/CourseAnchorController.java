...
public class CourseAnchorController extends AbstractController {
	...
	public TableVo courses(TableVo tableVo) {
		...
		if (searchLecturer != null) {
			searchVo.setType(Integer.valueOf(searchLecturer.getPropertyValue1()
					.toString()));
		}

		Page<CourseAnchor> page = anchorService.findCourseAnchorPage(searchVo,
	...
	public TableVo recList(TableVo tableVo) {
		...
		Page<CourseAnchor> page = anchorService.findCourseAnchorRecPage(
	...
	/**
	 * Description：更新推荐状态 creed: Talk is cheap,show me the code
	 * 
	...
	public String anchorCourse(HttpServletRequest request, String userId,
			String anchorNname, String loginName) {
	...
	public TableVo courseDetail(TableVo tableVo) {
		...
		if (search_applyStatus != null
				&& StringUtils.isNotBlank(search_applyStatus
						.getPropertyValue1().toString())) {
			courseApplyInfo.setApplyStatus(Integer.valueOf(search_applyStatus
					.getPropertyValue1().toString()));
		}
		// 课程类型 1.直播 2.点播 3.线下课
		Group search_courseForm = groups.findByName("search_courseForm");
		if (search_courseForm != null
				&& StringUtils.isNotBlank(search_courseForm.getPropertyValue1()
						.toString())) {
			courseApplyInfo.setCourseForm(Integer.valueOf(search_courseForm
					.getPropertyValue1().toString()));
		}
		// 是否上架（所有、已上架、未上架）；禁用0，启用，1
		Group search_status = groups.findByName("search_status");
		if (search_status != null
				&& StringUtils.isNotBlank(search_status.getPropertyValue1()
						.toString())) {
			courseApplyInfo.setStatus(Integer.valueOf(search_status
					.getPropertyValue1().toString()));
		}
		...
		Page<CourseApplyInfo> page = courseApplyService.findCoursePageByUserId(
				courseApplyInfo, currentPage, pageSize);
		int total = page.getTotalCount();
		tableVo.setAaData(page.getItems());
		tableVo.setiTotalDisplayRecords(total);
		tableVo.setiTotalRecords(total);
	...
	/**
	 * Description：设置主播推荐值 creed: Talk is cheap,show me the code
	 * 
	 * @author name：wangyishuai <br>
	 *         email: wangyishuai@ixincheng.com
