package com.xczhihui.common.dao;
...
import com.xczhihui.utils.Order;
import com.xczhihui.utils.PageVo;
import com.xczhihui.utils.ReflectionUtils;
...
public class HibernateDao<T> extends SimpleHibernateDao{
	...
	private String composeString(final Groups groups, PageVo page, String[] selectStr, String[] groupStr, List<Object> values) {
		...
		if ((groups.getOrderbys() == null || groups.getOrderbys().length <= 0) && 
			...
			if (groups.isOrder()) {
				order.setSortType(Order.SortType.ASC);
			} else {
				order.setSortType(Order.SortType.DESC);
			}
			orders.add(order);
		}
		if (groups.getOrderbys() != null && groups.getOrderbys().length > 0) {
			...
			for (String string : strings) {
				...
				try {
					...
					// 如果是程序
						...
						if (order.getSortType() == Order.SortType.ASC) {
