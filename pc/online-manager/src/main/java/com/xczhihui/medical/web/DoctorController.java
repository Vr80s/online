...
public class DoctorController extends AbstractController {
    ...
    public String index(HttpServletRequest request,
        ...
        //医师的详细地址，从已关联的医馆中查询 非常诡异的设计
        List<MedicalHospital> medicalHospitals = doctorService.getMedicalHospital(doctorId);
        if (medicalHospitals != null && !medicalHospitals.isEmpty()) {
            MedicalHospital hospital = medicalHospitals.get(0);
            doctor.setDetailedAddress(hospital.getDetailedAddress());
            if (StringUtils.isBlank(doctor.getCity())) {
                doctor.setCity(hospital.getCity());
            }
            if (StringUtils.isBlank(doctor.getProvince())) {
