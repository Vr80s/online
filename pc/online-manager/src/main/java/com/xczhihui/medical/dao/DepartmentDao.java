...
public class DepartmentDao extends HibernateDao<MedicalDepartment> {
	...
	public Page<MedicalDepartment> findMedicalDepartmentPage(
			MedicalDepartment menuVo, int pageNumber, int pageSize) {
		Map<String, Object> paramMap = new HashMap<String, Object>();
		StringBuilder sql = new StringBuilder(
				"SELECT * FROM `medical_department` ds where deleted=0 ");
		...
		Page<MedicalDepartment> pages = this.findPageBySQL(sql.toString(),
				paramMap, MedicalDepartment.class, pageNumber, pageSize);
	...
	public MedicalDepartment findById(String parseInt) {
		StringBuilder sql = new StringBuilder(
				"select * from medical_department where id=:id");
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("id", parseInt);
		List<MedicalDepartment> menus = this.getNamedParameterJdbcTemplate()
				.query(sql.toString(),
						params,
						BeanPropertyRowMapper
								.newInstance(MedicalDepartment.class));
		if (menus.size() > 0) {
			return menus.get(0);
		}
		return null;
	...
	public MedicalDepartment findByNotEqId(MedicalDepartment searchEntity) {
		StringBuilder sql = new StringBuilder(
				"select * from medical_department where name=:name and  deleted =0");
		Map<String, Object> params = new HashMap<String, Object>();
