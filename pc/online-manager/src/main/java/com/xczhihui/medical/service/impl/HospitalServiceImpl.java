 ...
 */
@Service
public class HospitalServiceImpl extends OnlineBaseServiceImpl implements
		HospitalService {
	...
	public Page<MedicalHospital> findMedicalHospitalPage(
		...
		Page<MedicalHospital> page = hospitalDao.findMedicalHospitalPage(
				medicalHospital, pageNumber, pageSize);
		return page;
	...
	public Page<MedicalHospital> findRecMedicalHospitalPage(
			MedicalHospital medicalHospital, int pageNumber, int pageSize) {
		Page<MedicalHospital> page = hospitalDao.findRecMedicalHospitalPage(
				medicalHospital, pageNumber, pageSize);
		return page;
	...
	public void updateStatus(String id) {
		...
		MedicalHospital medicalHospital = dao.findByHQLOne(hql,
				new Object[] { id });
	...
	public void deletes(String[] ids) {
		for (String id : ids) {
			...
			MedicalHospital medicalHospital = dao.findByHQLOne(hqlPre,
					new Object[] { id });
	 ...
	 */
	@Override
	public void updateMedicalHospitalDetail(String medicalHospitalId,
		...
		List<MedicalHospital> hospitals = dao.findEntitiesByProperty(
				MedicalHospital.class, "id", medicalHospitalId);
		if (CollectionUtils.isEmpty(hospitals)) {
			...
			if (StringUtils.isNotBlank(hospitals.get(0).getSourceId())
					|| StringUtils.isNotBlank(hospitals.get(0)
							.getCreatePerson())) {
				throw new MedicalException(MedicalExceptionEnum.MUST_NOT_HANDLE);
			}
		}

		List<MedicalHospitalPicture> mhps = dao.findEntitiesByProperty(
				MedicalHospitalPicture.class, "hospitalId", medicalHospitalId);
	...
	public void savePicture(String medicalHospitalId, String picture,
			String version) {
	...
	public Map<String, Object> getMedicalHospitalDetail(String medicalHospitalId) {
		MedicalHospital mh = hospitalDao.findOneEntitiyByProperty(
				MedicalHospital.class, "id", medicalHospitalId);

		String sql = "select * from medical_hospital_picture where hospital_id = '"
				+ medicalHospitalId + "' and deleted = 0 order by version";

		List<MedicalHospitalPicture> voList = dao.findEntitiesByJdbc(
				MedicalHospitalPicture.class, sql, null);

		// List<MedicalHospitalPicture> c =
		// dao.findEntitiesByProperty(MedicalHospitalPicture.class,
		// "hospitalId", medicalHospitalId);
	...
	public List<MedicalHospital> findByName(String name) {
		List<MedicalHospital> MedicalHospitals = dao.findEntitiesByProperty(
				MedicalHospital.class, "name", name);
	...
	public boolean updateRec(String[] ids, int isRecommend) {
		...
		if (isRecommend == 1) {// 如果是要推荐 那么就验证 推荐数量是否大于4
			...
			// 已经存在的数量 + 即将添加的数量
			/*
			 * if((list.size()+ids2.size()) > 10){ return false; }
			 */
		} else {// 如果是取消推荐
			for (int i = 0; i < ids.length; i++) {
				ids2.add(ids[i]);
	...
	public void updateSortUpRec(String id) {
		String hqlPre = "from MedicalHospital where  deleted=0 and id = ?";
		MedicalHospital coursePre = dao.findByHQLOne(hqlPre,
				new Object[] { id });
		...
		MedicalHospital courseNext = dao.findByHQLOne(hqlNext,
				new Object[] { id });
