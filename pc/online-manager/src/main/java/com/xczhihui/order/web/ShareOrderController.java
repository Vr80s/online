...
public class ShareOrderController {
	...
	public TableVo findShareOrderList(TableVo tableVo) {
		...
		Group sharePersonNameGroup = groups
				.findByName("search_sharePersonName");
		...
		if (createPersonNameGroup != null) {
			searchVo.setCreatePersonName(createPersonNameGroup
					.getPropertyValue1().toString());
		}
		if (sharePersonNameGroup != null) {
			searchVo.setShareUserName(sharePersonNameGroup.getPropertyValue1()
					.toString());
		}
		...
		Page<ShareOrderVo> page = shareOrderService.findShareOrderPage(
	...
	public TableVo findShareOrderListDeatil(TableVo tableVo) {
		...
		if (targetUserIdGroup != null) {
			searchVo.setTargetUserId(targetUserIdGroup.getPropertyValue1()
					.toString());
		}
		if (shareOrderNoGroup != null) {
			searchVo.setShareOrderNo(shareOrderNoGroup.getPropertyValue1()
					.toString());
		}
		if (courseNameGroup != null) {
			searchVo.setCourseName(courseNameGroup.getPropertyValue1()
					.toString());
		}
		if (createPersonNameGroup != null) {
			searchVo.setCreatePersonName(createPersonNameGroup
		...
		if (levelGroup != null) {
			searchVo.setLevel(Integer.parseInt(levelGroup.getPropertyValue1()
					.toString()));
		}
		Page<ShareOrderVo> page = shareOrderService.findShareOrderDetailPage(
				searchVo, currentPage, pageSize);
	...
	public ResponseObject getOrderDetailList(ShareOrderVo shareOrderVo) {
		...
		responseObj.setResultObject(shareOrderService
				.getOrderDetailList(shareOrderVo));
		return responseObj;
	}
	...
	public ResponseObject getShareOrderDetail(ShareOrderVo shareOrderVo) {
		...
		responseObj.setSuccess(true);
		responseObj.setResultObject(shareOrderService
				.getShareOrderDetail(shareOrderVo));
		return responseObj;
	}
}
