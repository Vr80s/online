...
public class ShareOrderDao extends SimpleHibernateDao {
	public Page<ShareOrderVo> findShareOrderPage(ShareOrderVo shareOrderVo, int pageNumber, int pageSize){
		...
		//"	(select count(1) from oe_user ou where ou.parent_id = oe.id) shareCount," +//#总共发展人数
		//"	(select IFNULL(sum(t2.subsidies),0) from oe_share_order t2 where t2.target_user_id = oe.id ) totalShareMoney, " +//#累计佣金
		//"	0 getShareMoney " +
		List<ShareOrderVo> list = ms.getItems();
		
		for(int i=0;i<list.size();i++){
			String sqlShareCount = "select count(1) from oe_user ou where ou.parent_id = ? ";
			list.get(i).setShareCount(this.queryForInt(sqlShareCount, new Object[]{list.get(i).getId()}));
			String sqlTotalShareMoney = "select IFNULL(sum(t2.subsidies),0) from oe_share_order t2 where t2.target_user_id = ?";
			list.get(i).setTotalShareMoney(this.getNamedParameterJdbcTemplate().getJdbcOperations().queryForObject(sqlTotalShareMoney, new Object[]{list.get(i).getId()}, Double.class));
...
//	}
	...
	public Page<ShareOrderVo> findShareOrderDetailPage(ShareOrderVo shareOrderVo, int pageNumber, int pageSize){
		Map<String,Object> paramMap=new HashMap<String,Object>();
		StringBuilder sql=new StringBuilder("SELECT " +
											"	oso.order_no, " +
											"	oso.share_order_no, " +
											"	oso.course_name, " +
											"	oso.buy_user_id, " +
											"	ou.login_name createPersonName, " +
											"	oso.actual_pay, " +
											"	oso.pay_time, " +
											"	oso.`level`, " +
											"	oso.subsidies " +
