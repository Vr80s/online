...
public class BBSLabelDao extends SimpleHibernateDao {
    ...
    public int deleteByIds(List<Integer> ids) {
        ...
        return this.getNamedParameterJdbcTemplate().update("DELETE FROM quark_label WHERE id IN (:ids) AND posts_count <= 0", mapSqlParameterSource);
    }

    public int updateStatusByIds(List<Integer> ids) {
        MapSqlParameterSource mapSqlParameterSource = new MapSqlParameterSource();
        mapSqlParameterSource.addValue("ids", ids);
        return this.getNamedParameterJdbcTemplate().update("UPDATE quark_label SET is_disable = !is_disable WHERE id IN (:ids)", mapSqlParameterSource);
    }
    ...
    public int update(BBSLabel bbsLabel) {
        ...
        return this.getNamedParameterJdbcTemplate().update("UPDATE quark_label SET details = :details, name = :name, sort = :sort," +
                " label_img_url = :labelImgUrl WHERE id = :id", mapSqlParameterSource);
    }

    public Page<BBSLabelVo> list(int page, int size) {
        return super.findPageBySQL(
                "select id as id, details as details, name as name, posts_count as postsCount," +
                        ...
                        " from quark_label order by sort desc", null, BBSLabelVo.class, page, size);
    }

    public void updatePostCount(Integer labelId, Integer postCount) {
        MapSqlParameterSource mapSqlParameterSource = new MapSqlParameterSource();
        mapSqlParameterSource.addValue("id", labelId).addValue("postCount", postCount);
        this.getNamedParameterJdbcTemplate().update("UPDATE quark_label SET posts_count = :postCount WHERE id = :id", mapSqlParameterSource);
    }
}
