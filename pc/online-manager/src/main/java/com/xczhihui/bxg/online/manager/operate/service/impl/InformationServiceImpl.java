...
import com.xczhihui.bxg.online.manager.operate.dao.InformationDao;
import com.xczhihui.bxg.online.manager.operate.service.InformationService;
import com.xczhihui.bxg.online.manager.operate.vo.InformationVo;
import com.xczhihui.bxg.online.manager.operate.vo.TypeVo;

@Service
public class InformationServiceImpl extends OnlineBaseServiceImpl implements InformationService {
	...
	public Page<InformationVo> findInformationPage(InformationVo informationVo, Integer pageNumber, Integer pageSize) {
		Page<InformationVo> page = informationDao.findInformationPage(informationVo, pageNumber, pageSize);
		return page;
	}
	...
	public void deletes(String[] ids) {
		...
		for(String id:ids){
			...
			Information info= dao.findByHQLOne(hqlPre,new Object[] {Integer.valueOf(id)});
            if(info !=null){
            	info.setDelete(true);
                 dao.update(info);
            }
	...
	public String updateStatus(Integer id) {
		// TODO Auto-generated method stub
		 ...
		 List<Information> liveInfo = dao.findEntitiesByJdbc(Information.class, sql, new HashMap());
		 //List<Information> liveInfo=dao.findEntitiesByProperty(Information.class, "status", 1);
	...
	public void updateInfo(InformationVo info) {
		// TODO Auto-generated method stub
		 String hql="from Information where  isDelete=0 and id = ?";
		 Information temp= dao.findByHQLOne(hql,new Object[] {Integer.parseInt(info.getId())});
	...
	public void updateTypes(TypeVo vo) {
		...
		//全部删除
		SystemVariate p = informationDao.findByHQLOne("from SystemVariate where value='informationType' and (parentId is null or parentId='') ");
		Map<String, Object> paramMap = new HashMap<String, Object>();
		paramMap.put("parent_id", p.getId());
		informationDao.getNamedParameterJdbcTemplate().update("delete from system_variate where parent_id=:parent_id ", paramMap);
		...
		for (TypeVo t : vos) {
			if (t != null && StringUtils.hasText(t.getName())) {
				...
				informationDao.getNamedParameterJdbcTemplate().update("insert into system_variate (id,is_delete,display_order,name,parent_id,value)"
						+ " values (:id,false,:display_order,:name,:parent_id,:value) ",paramMap);
				display_order ++;
			}
		}
