...
public class TagDao extends SimpleHibernateDao {
	...
	public List<TagVo> findTag(TagVo tagVo) {
		...
		StringBuilder sql = new StringBuilder("select oat.name,oat.menu_id from oe_ask_tag oat where is_delete = 0 and status = 1");
		...
		List<TagVo> list = this.getNamedParameterJdbcTemplate().query(sql.toString(),paramMap,BeanPropertyRowMapper.newInstance(TagVo.class));
	...
	public Page<TagVo> findTagPage(TagVo searchVo, int currentPage, int pageSize) {
        ...
        Page<TagVo> TagVos=this.findPageBySQL(sql.toString(), paramMap, TagVo.class, currentPage, pageSize);
        
        
        for(TagVo temp:TagVos.getItems()){
        	String quesCountSql=" SELECT count(1) as quesCount from (SELECT find_in_set('"+temp.getName()+"',tags) as a FROM oe_ask_question where ment_id = :menuId) q WHERE a >0";
        	List<TagVo> templist=this.findEntitiesByJdbc(TagVo.class, quesCountSql, params);
        	temp.setQuesCount(templist.get(0).getQuesCount());
        	
        	if(searchVo.getMonthSort()!=null&&!"".equals(searchVo.getMonthSort())){
        		if("30".equals(searchVo.getMonthSort())){
        			String quesCountSqlOneMonth=" SELECT count(1) as citesCount from (SELECT find_in_set('"+temp.getName()+"',tags) as a FROM oe_ask_question where ment_id = :menuId and DATE_SUB(CURDATE(), INTERVAL 30 DAY) <= create_time) q WHERE a >0";
                	List<TagVo> templistOneMonth=this.findEntitiesByJdbc(TagVo.class, quesCountSqlOneMonth, params);
                	temp.setCitesCount(templistOneMonth.get(0).getCitesCount());
        		}else if("60".equals(searchVo.getMonthSort())){
        			String quesCountSqlOneMonth=" SELECT count(1) as citesCount from (SELECT find_in_set('"+temp.getName()+"',tags) as a FROM oe_ask_question where ment_id = :menuId and DATE_SUB(CURDATE(), INTERVAL 60 DAY) <= create_time) q WHERE a >0";
                	List<TagVo> templistOneMonth=this.findEntitiesByJdbc(TagVo.class, quesCountSqlOneMonth, params);
                	temp.setCitesCount(templistOneMonth.get(0).getCitesCount());
    		 ...
    		 if (searchVo.getQuesSort()!=null&&!"".equals(searchVo.getQuesSort())) {
    			 temp.setSortType("question");
			} else if (searchVo.getCitesSort()!=null&&!"".equals(searchVo.getCitesSort())){
	...
	public AskTag updateDirectionUp(String id) {
		 String sql=" select * from oe_ask_tag where seq > (select seq from oe_ask_tag where id=:id ) and  is_delete=0 and status=1 and menu_id=(select menu_id from oe_ask_tag where id=:id ) order by seq asc";
	        Map<String,Object> params=new HashMap<String,Object>();
	        params.put("id", id);
	        List<AskTag> askTags=this.getNamedParameterJdbcTemplate().query(sql.toString(), params, BeanPropertyRowMapper.newInstance(AskTag.class));
	        if(askTags.size()>0){
