...
import com.xczhihui.bxg.online.manager.bbs.service.BBSPostService;
...
public class BBSPostServiceImpl implements BBSPostService {
    ...
    public TableVo list(TableVo tableVo) {
        ...
        if (nickname != null && nickname.getPropertyValue1() != null) {
            params.put("nickname", "%" + nickname.getPropertyValue1().toString() + "%");
        } else {
        ...
        Page<BBSPostVo> bbsReplyVoPage = bbsPostDao.list(params, tableVo.getCurrentPage(), tableVo.getiDisplayLength());
    ...
    protected void updateLabelPostCount(List<Integer> ids) {
        for (int id : ids) {
            BBSPost bbsPost = bbsPostDao.findOneEntitiyByProperty(BBSPost.class, "id", id);
            Integer labelId = bbsPost.getLabelId();
            bbsLabelDao.updatePostCount(labelId, bbsPostDao.countByLabelId(labelId));
        }
    }
