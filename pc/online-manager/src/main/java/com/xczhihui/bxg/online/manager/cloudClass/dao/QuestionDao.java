...
import com.xczhihui.bxg.online.manager.cloudClass.vo.VideoResVo;
...
public class QuestionDao extends HibernateDao<QuesStore>{
	...
	public Page<QuesStore> findQuesPage(QuesStore searchVo, int currentPage,
		...
		Map<String,Object> paramMap=new HashMap<String,Object>();
	 	StringBuilder sql =new StringBuilder("select DISTINCT(q.id) as uniqueId ,q.* from oe_question_kpoint qk,oe_question q where q.course_id = "+searchVo.getCourseId()+ " and qk.course_id="+searchVo.getCourseId()+" and q.is_delete = 0 and qk.question_id=q.id" );
	 	if(!"0".equals(searchVo.getChapterId())&&!"".equals(searchVo.getChapterId())&&searchVo.getChapterId()!=null){
	 	...
	 	if(!"".equals(searchVo.getQuestionType())&&searchVo.getQuestionType()!=null){
	 		sql.append(" and q.question_type =:quesType ");
	 		paramMap.put("quesType", searchVo.getQuestionType());
	 	}
	 	if(!"".equals(searchVo.getQuestionHeadText())&&searchVo.getQuestionHeadText()!=null){
	 	...
	 	Page<QuesStore> ms = this.findPageBySQL(sql.toString(), paramMap, QuesStore.class, currentPage, pageSize);
	 	return ms;
	}
	...
	public List<String> getKnowledgePointsName(String id) {
		...
		list = this.getNamedParameterJdbcTemplate().queryForList(sql, map, String.class);
		return list;
	}
	/**
	 * 获取题目的所有知识点id
	...
	public List<String> getKnowledgePointsId(String id) {
		...
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("id", id);
		return this.getNamedParameterJdbcTemplate().queryForList(sql, map, String.class);
	}
	/**
	 * 获取题目的所有知识点id
	 * @param id
