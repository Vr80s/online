...
public class ScoreTypeDao extends HibernateDao<ScoreType> {

    public Page<ScoreTypeVo> findScoreTypePage(ScoreTypeVo menuVo, int pageNumber, int pageSize){
        Map<String,Object> paramMap=new HashMap<String,Object>();
        StringBuilder sql=new StringBuilder("select ds.*,(select count(1) from score_type where  is_delete=0 and status=1 ) as courseCount,(select group_concat(name) from score_type where  is_delete=0 and status=1 and name <> '全部') as childMenuNames from (select * from score_type where  is_delete=0 and name<>'全部')ds where 1=1 ");
        ...
        Page<ScoreTypeVo> pages = this.findPageBySQL(sql.toString(), paramMap, ScoreTypeVo.class, pageNumber, pageSize);
        ...
        for(ScoreTypeVo temp :pages.getItems() ){
        	...
        	List<ScoreTypeVo> res = this.findEntitiesByJdbc(ScoreTypeVo.class, sqlIsUsed, paramMap);
	...
	public ScoreType findById(String parseInt) {
        ...
        List<ScoreType> menus=this.getNamedParameterJdbcTemplate().query(sql.toString(),params,BeanPropertyRowMapper.newInstance(ScoreType.class));
        if(menus.size()>0){
            return menus.get(0);
        }
        return null;
	...
	public String deleteById(String id) {
		String s = "";
//		String sql  = "select * from menu_courseType where course_type_id = :id and is_delete = 0 ";
		String sql  =   " SELECT " +
						"	* " +
						" FROM " +
						"	oe_menu t, " +
