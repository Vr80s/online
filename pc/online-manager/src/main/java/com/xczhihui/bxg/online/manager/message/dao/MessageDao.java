package com.xczhihui.bxg.online.manager.message.dao;
...
public class MessageDao extends HibernateDao<Menu> {
	...
	/**
	 * 返回指定页面的消息推送
	 * @param vNameOrCreater 指定条件
	 * @param orderByName 排序字段
	 * @param pageNumber 页码
	 * @param pageSize 页大小
	 * @return 页对象
	 */
	public com.xczhihui.bxg.common.util.bean.Page<Message> findPageMessages(String vNameOrCreater, String orderByName, int pageNumber,
																	 int pageSize) {
	 ...
	 * @return 页对象
	 */
	public com.xczhihui.bxg.common.util.bean.Page<Message> findPageMessages(MessageVo vo, String orderByName, int pageNumber,
																		 int pageSize) {
		DetachedCriteria dc = DetachedCriteria.forClass(Message.class);
		dc.add(Restrictions.eq("isDelete", false));
		...
		if(!StringUtils.isEmpty(vo.getTime_start())){
			dc.add(Restrictions.ge("createTime", TimeUtil.parseDate(vo.getTime_start() + " 00:00:00")));
		}
		if(!StringUtils.isEmpty(vo.getTime_end())){
			dc.add(Restrictions.le("createTime", TimeUtil.parseDate(vo.getTime_end() + " 23:59:59")));
		}
		Order order = Order.desc(orderByName);
		dc.addOrder(order);
		return this.findPageByCriteria(dc, pageNumber, pageSize);
	 ...
	 * @return 分页对象
	 */
	public com.xczhihui.bxg.common.util.bean.Page<Message> findPageMessagesByUser(UserVo userVo,String vNameOrCreater, String orderByName, int pageNumber,
																		 int pageSize) {
		DetachedCriteria dc = DetachedCriteria.forClass(Message.class);
		dc.add(Restrictions.eq("isDelete", false));
		dc.add(Restrictions.eq("type",1));
