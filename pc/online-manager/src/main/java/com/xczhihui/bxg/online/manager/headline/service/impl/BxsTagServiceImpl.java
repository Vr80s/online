...
import com.xczhihui.bxg.online.manager.headline.dao.BxsTagDao;
import com.xczhihui.bxg.online.manager.headline.service.BxsTagService;
 ...
 */
@Service
public class BxsTagServiceImpl  extends OnlineBaseServiceImpl implements BxsTagService {
	...
	public String addTag(TagVo tagVo) {
		...
		for(int i=0; i<nameArray.length; i++){
			...
			int hasTagName = tagDao.queryForInt("select count(1) from oe_bxs_tag obt where obt.name = ?", new Object[]{nameArray[i].trim()});
			...
			tagDao.getNamedParameterJdbcTemplate().update(sql, new BeanPropertySqlParameterSource(bxsTag));
	   }
	...
	public String deletes(String[] ids) {
        ...
        for(String id:ids){
        	tagVo.setId(id);
        	tagDao.getNamedParameterJdbcTemplate().update("delete from oe_bxs_tag where id = :id", new BeanPropertySqlParameterSource(tagVo));
        }
        return  msg;
	}
	...
	public void updateStatus(String id) {
		...
		tagDao.getNamedParameterJdbcTemplate().update(sql, new BeanPropertySqlParameterSource(tagVo));
	}
	...
	public void updateTag(TagVo tagVo) {
		...
		if(tagDao.queryForInt(sql, new Object[]{tagVo.getId(),tagVo.getName()})>0){
