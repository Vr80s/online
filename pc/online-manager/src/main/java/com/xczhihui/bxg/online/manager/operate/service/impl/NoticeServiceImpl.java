...
import com.xczhihui.bxg.online.manager.operate.dao.NoticeDao;
...
@Service
public class NoticeServiceImpl extends OnlineBaseServiceImpl implements NoticeService {

	@Autowired
    private NoticeDao noticeDao;
	
	@Override
	public Page<NoticeVo> findNoticePage(NoticeVo noticeVo, Integer pageNumber, Integer pageSize) {
		Page<NoticeVo> page = noticeDao.findNoticePage(noticeVo, pageNumber, pageSize);
		return page;
	}
	...
	public void addNotice(NoticeVo noticeVo) {
		...
		noticeDao.getNamedParameterJdbcTemplate().update(sql, new BeanPropertySqlParameterSource(noticeVo));
	}
	...
	public boolean updateStatus(NoticeVo noticeVo) {
		...
		String sql =" UPDATE oe_notice " +
			   ...
			   sql +=" WHERE " +
					"	`id` = :id";
		return noticeDao.getNamedParameterJdbcTemplate().update(sql, new BeanPropertySqlParameterSource(noticeVo)) > 0;
	}
	...
	public void deletes(String[] ids) {
        ...
        for(String id:ids){
        	noticeVo.setId(id);
        	noticeDao.getNamedParameterJdbcTemplate().update(" delete from oe_notice where id = :id", new BeanPropertySqlParameterSource(noticeVo));
        }
	}
}
