...
import com.xczhihui.bxg.online.manager.order.dao.OrderDao;
import com.xczhihui.bxg.online.manager.order.service.OrderService;
...
public class OrderServiceImpl extends OnlineBaseServiceImpl implements OrderService {
	...
	public Page<OrderVo> findOrderPage(OrderVo orderVo, Integer pageNumber, Integer pageSize) {
		Page<OrderVo> page = orderDao.findOrderPage(orderVo, pageNumber, pageSize);
		return page;
	}
	...
	public List getOrderPreferenty(String orderNo){
		...
		//String sqlOld="select * from oe_order_preferenty_record where order_no=:orderNo";
		paramMap.put("orderNo",orderNo);
		List<Map<String, Object>> list = orderDao.getNamedParameterJdbcTemplate().queryForList(sql, paramMap);
		Double sum=0.0;
		for (Map<String, Object> r : list) {
			if(r.get("preferenty")!=null&&StringUtil.isNumber(r.get("preferenty").toString())){
				...
				r.put("actualPay",Double.parseDouble(r.get("actualPay").toString())-1000 );
				
			}
			/*if(r.get("preferenty")!=null&&!StringUtil.isNumber(r.get("preferenty").toString())&&!"老学员优惠".equals(r.get("preferenty").toString())){
				sum=sum+Double.parseDouble(r.get("actualPay").toString());
				r.put("actualPay",sum-Double.parseDouble(r.get("minusMoney").toString()) );
			}*/
		}
		return list;
	}
