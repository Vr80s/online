package com.xczhihui.bxg.online.manager.barrier.dao;

import com.xczhihui.bxg.common.support.dao.SimpleHibernateDao;
import com.xczhihui.bxg.online.manager.barrier.vo.ExamPaperVo;
import com.xczhihui.bxg.online.manager.barrier.vo.PaperInfoVo;
...
public class MarkExamPapersDao extends SimpleHibernateDao {
    public PaperInfoVo getQuestionInfoByType(String record_id, Integer type){
        ...
        List<PaperInfoVo> list = this.findEntitiesByJdbc(PaperInfoVo.class,sql.toString(),paramMap);
    ...
    public List<ExamPaperVo> getQuestionByType(String record_id,Integer type){
        ...
        paramMap.put("record_id",record_id);
        paramMap.put("type",type);
        return this.findEntitiesByJdbc(ExamPaperVo.class,sql.toString(),paramMap);
    }
    public Integer getExamScore(String record_id){
        StringBuilder sql=new StringBuilder() ;
        sql.append("SELECT score from oe_barrier_record where id=?");
        return this.getNamedParameterJdbcTemplate().getJdbcOperations().queryForObject(sql.toString(),Integer.class,record_id);
    }
}
