...
import com.xczhihui.bxg.online.manager.gift.vo.RewardVo;
...
public class RewardDao extends HibernateDao<Course>{
	 public Page<RewardVo> findRewardPage(RewardVo giftVo, int pageNumber, int pageSize){
		 ...
		 StringBuilder sql = new StringBuilder("SELECT oer.`id`,oer.`price`,oer.`is_freedom` isFreeDom,oer.`sort`,oer.`status`,oer.`create_time` createTime,"
...
//			 sql.append(" and oer.status = :status ");
		 ...
		 Page<RewardVo> giftVos = this.findPageBySQL(sql.toString(), paramMap, RewardVo.class, pageNumber, pageSize);
		 return giftVos;
	 }
