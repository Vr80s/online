...
public class EnchashmentServiceImpl extends OnlineBaseServiceImpl implements EnchashmentService {
	...
	public Page<EnchashmentApplyInfo> findEnchashmentPage(EnchashmentApplyInfo orderVo, Integer pageNumber, Integer pageSize) {
		Page<EnchashmentApplyInfo> page = enchashmentDao.findEnchashmentPage(orderVo, pageNumber, pageSize);
		for (EnchashmentApplyInfo enchashmentApplyInfo:page.getItems()){
			if(enchashmentApplyInfo.getStatus()==ApplyStatus.NOT_PASS.getCode()){
				enchashmentApplyInfo.setDismissalText(EnchashmentDismissal.getDismissal(enchashmentApplyInfo.getDismissal()));
			}
		}
		return page;
	}
	...
	/**
	 * Description：提现被驳回，更新用户账户（rmb加回）
	...
	private void updateUserCoinForEnchashment(EnchashmentApplyInfo eai) {
		...
		sql.append("where user_id=\"" + uci.getUserId() + "\"  and uc.version =\"" + uc.getVersion() + "\"");
