package com.xczhihui.bxg.online.manager.headline.dao;
...
public class BxsTagDao extends SimpleHibernateDao {
	...
	public Page<TagVo> findTagPage(TagVo searchVo, int currentPage, int pageSize) {
		...
		}else if(searchVo.getSortType() != null && searchVo.getSortType() == 2){
			sql += " order by "+
					" (SELECT " +
        			"	count( " +
        			"		CASE " +
        			"		WHEN FORMAT(t.create_time, 'yyyy-mm-dd') >= FORMAT(DATE_SUB(now(), INTERVAL - " + searchVo.getLatelyNum() + " MONTH),'yyyy-mm-dd') THEN " +
        			"			1 " +
        			"		ELSE " +
        			"			NULL " +
        			"		END " +
        ...
        for(int i = 0;i<tagVos.getItems().size();i++){
        	...
        	tagVos.getItems().get(i).setArticleCnt(this.queryForInt(sql, new Object[]{tagVos.getItems().get(i).getId()}));
        	
        	sql =  " SELECT " +
        			"	count( " +
        			"		CASE " +
        			"		WHEN FORMAT(t.create_time, '%Y-%m-%d') >= FORMAT(DATE_ADD(now(), INTERVAL - " + searchVo.getLatelyNum() + " MONTH),'%Y-%m-%d') THEN " +
        			"			1 " +
        			"		ELSE " +
        			"			NULL " +
        			"		END " +
        			...
        			"	t.tag_id = ? " +
        			"	 and exists(select 1 from oe_bxs_article t2 where t.article_id = t2.id and t2.is_delete = 0 ) ";
        	tagVos.getItems().get(i).setArticleCntLately(this.queryForInt(sql, new Object[]{tagVos.getItems().get(i).getId()}));
        }
        return tagVos;
	}
}
