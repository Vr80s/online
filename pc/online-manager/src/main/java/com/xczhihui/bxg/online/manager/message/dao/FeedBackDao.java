...
public class FeedBackDao extends HibernateDao<Menu> {
	...
	public PageVo findPageFeedBack(Groups groups, PageVo page){
		...
		sql.append(" select m.id as id,"
					...
					+ "m.create_person as anwerName,"
					+ "m.context as context,"
					+ "m.replytext as replytext,"
	...
	public com.xczhihui.bxg.common.util.bean.Page<Message> findPageMessages(MessageVo vo, String orderByName, int pageNumber,
		...
		if(vo.getAnswerStatus()!=null&&vo.getAnswerStatus()!=-1) {
			if (vo.getAnswerStatus() != 0) {
				...
				dc.add(Restrictions.or(Restrictions.eq("answerStatus", vo.getAnswerStatus()), Restrictions.isNull("answerStatus")));
			}
		}
