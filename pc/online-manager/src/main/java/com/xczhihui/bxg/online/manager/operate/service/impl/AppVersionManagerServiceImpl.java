...
import com.xczhihui.bxg.online.manager.operate.dao.AppVersionInfoDao;
import com.xczhihui.bxg.online.manager.operate.service.AppVersionManagerService;

@Service
public class AppVersionManagerServiceImpl extends OnlineBaseServiceImpl implements AppVersionManagerService {

	@Autowired
    private AppVersionInfoDao appVersionInfoDao;
	
	@Override
	public Page<AppVersionInfo> findAppVersionInfoPage(AppVersionInfo appVersionInfo, Integer pageNumber, Integer pageSize) {
		Page<AppVersionInfo> page = appVersionInfoDao.findAppVersionInfoPage(appVersionInfo, pageNumber, pageSize);
		return page;
	}
	...
	public boolean updateStatus(AppVersionInfo appVersionInfo) {
	    ...
	    sql =" UPDATE app_version_info " +
			" SET status = abs(status - 1) " +
			" WHERE " +
			"	id = ? ";
		dao.getNamedParameterJdbcTemplate().getJdbcOperations().update(sql, new Object[]{appVersionInfo.getId()});
		return true;
	}
	...
	public void deletes(String[] ids) {
		for(String id:ids){
			...
			dao.getNamedParameterJdbcTemplate().getJdbcOperations().update(sql, new Object[]{id});
        }
	}
	...
	public void updateSortUp(String id) {
		 ...
		 Integer mobileBannerPreSort = dao.queryForInt(sqlPre, new Object[]{id});
         
         String sqlNext="select sort,id from app_version_info where status = 1 and sort > ? order by sort desc limit 1 ";//然后取出他相邻的顺序
         Map map = dao.getNamedParameterJdbcTemplate().getJdbcOperations().queryForMap(sqlNext,new Object[] {mobileBannerPreSort});
         Integer mobileBannerNextSort = Integer.valueOf(map.get("sort").toString());
         String sql = "update app_version_info set sort = ? where id = ? ";
         dao.getNamedParameterJdbcTemplate().getJdbcOperations().update(sql, new Object[]{mobileBannerNextSort,id});
         dao.getNamedParameterJdbcTemplate().getJdbcOperations().update(sql, new Object[]{mobileBannerPreSort,map.get("id").toString()});
	}
