...
import com.xczhihui.bxg.online.manager.cloudClass.vo.CourseSubscribeVo;
...
public class RemindTask extends TimerTask {
	...
	public RemindTask(int courseId, String version, CourseService courseService, CourseSubscribeDao courseSubscribeDao) {
...
//		String startTime = sdf.format(course.getStartTime());   
        ...
        if(!course.isSent()){
        	List<CourseSubscribeVo> courseSubscribeList = courseSubscribeDao.getCourseSubscribeByCourseId(courseId);
        	
        	for (CourseSubscribeVo courseSubscribeVo : courseSubscribeList) {
        		try {
					SmsUtil.sendSmsSubscribe(courseSubscribeVo.getPhone(), course.getGradeName(), null, m+"", false);
