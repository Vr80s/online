...
@Repository("askMenuDao")
public class AskMenuDao extends HibernateDao<Menu> {

    public Page<MenuVo> findCloudClassMenuPage(MenuVo menuVo, int pageNumber, int pageSize){
        Map<String,Object> paramMap=new HashMap<String,Object>();
        StringBuilder sql=new StringBuilder("select om.id,om.name,om.remark,om.create_person,om.create_time,om.bo_status,om.bo_sort,om.ask_limit"
        		+ " from oe_menu om  where om.is_delete = 0 and om.id !=0 and om.status=1 ");
        sql.append(" and om.id!=1 and  om.id!=1 and  om.id!=2 and  om.id!=3 ");
        ...
        sql.append(" order by om.bo_status desc, om.bo_sort desc");
        ...
        for (MenuVo m : ms.getItems()) {
			...
			//查询学科下标签数量
			String tagCountSql="select Count(1) as tagCount from oe_ask_tag where is_delete=0 and menu_id=:id";
			List<MenuVo>  tags = this.getNamedParameterJdbcTemplate().query(tagCountSql, paramMap,BeanPropertyRowMapper.newInstance(MenuVo.class));
			m.setTagCount(tags.get(0).getTagCount());
    ...
    public Menu updateDirectionUp(Integer id){
        String sql=" select * from oe_menu where bo_sort > (select bo_sort from oe_menu where id=:id ) and  is_delete=0 order by bo_sort asc";
    ...
    public Menu updateDirectionUp(Integer parentNumber,Integer id,Integer level){
        String sql=" select * from oe_menu where bo_sort<(select bo_sort from oe_menu where id=:id ) and level=:level and is_delete=0 and number like :number order by bo_sort desc";
        Map<String,Object> params=new HashMap<String,Object>();
        params.put("id", id);
    ...
    public Menu updateDirectionDown(Integer id){
        String sql=" select * from oe_menu where bo_sort < (select bo_sort from oe_menu where id=:id )  and is_delete=0 and bo_status=1 order by bo_sort desc";
        Map<String,Object> params=new HashMap<String,Object>();
        params.put("id", id);
        List<Menu> menus=this.getNamedParameterJdbcTemplate().query(sql.toString(),params,BeanPropertyRowMapper.newInstance(Menu.class));
        if(menus.size()>0){
    ...
    public Menu updateDirectionDown(Integer parentNumber,Integer id,Integer level){
        String sql=" select * from oe_menu where bo_sort>(select bo_sort from oe_menu where id=:id ) and level=:level and is_delete=0  and number like :number order by bo_sort asc";
        Map<String,Object> params=new HashMap<String,Object>();
        params.put("id", id);
        params.put("level",level);
        params.put("number",parentNumber+"%");
    ...
    public List<Menu> findChildrenByNumber(Integer number,Integer level){
        String sql="select * from oe_menu where number like :number and level=:level and is_delete=0 and name<>'全部' order by bo_sort asc";
        Map<String,Object> params=new HashMap<String,Object>();
        params.put("number",number+"%");
        params.put("level",level);
...
//		return this.getNamedParameterJdbcTemplate().query(delSql,params,BeanPropertyRowMapper.newInstance(MenuCourseType.class));
	...
	public String deleteById(String id) {
		...
		String sql3 = "select * from oe_ask_question where is_delete = 0 and menu_id = :id";
		params3.put("id", id);
		List<Question> query3 = this.getNamedParameterJdbcTemplate().query(sql3,params3,BeanPropertyRowMapper.newInstance(Question.class));
		
		
		String deleteSql = " update oe_menu set is_delete=1 where  id = :id ";
		Map<String,Object> params2=new HashMap<String,Object>();
		params2.put("id", id);
		if( query2.size() > 0||query3.size()>0){
			s =  "此学科已被使用,不能被删除!";
		}else{
	...
	public List<MenuVo> getMenuList(){
		String sql = "select om.id,om.name from oe_menu om where om.bo_status = 1 and om.is_delete = 0 order by om.bo_sort";
		Map<String,Object> paramMap=new HashMap<String,Object>();
