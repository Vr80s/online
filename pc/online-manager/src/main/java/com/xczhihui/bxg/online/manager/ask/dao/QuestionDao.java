...
//							        		"	oe_menu om ON(oaq.ment_id = om.id) JOIN  " +
        ...
        if(questionVo.getCreatePersonName()!=null && !"".equals(questionVo.getCreatePersonName())){
        ...
        if(questionVo.getMentId()!=null && !"".equals(questionVo.getMentId()) && questionVo.getMentId() != 0){
        ...
        if(questionVo.getOrigin()!=null && !"".equals(questionVo.getOrigin())){
        ...
        }else if(questionVo.getBrowseSum() != null && questionVo.getBrowseSum() > 0){
        	sql.append(" order by oaq.browse_sum desc, oaq.create_time desc");
        }else{
        	sql.append(" order by oaq.create_time desc ");
        }
        Page<QuestionVo> ms = this.findPageBySQL(sql.toString(), paramMap, QuestionVo.class, pageNumber, pageSize);
