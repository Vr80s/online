package com.xczhihui.bxg.online.manager.bbs.dao;
...
import com.xczhihui.bxg.online.manager.bbs.vo.BBSPostVo;
...
public class BBSPostDao extends SimpleHibernateDao {
    ...
    public int changeGoodStatus(List<Integer> ids) {
        ...
        return this.getNamedParameterJdbcTemplate().update("UPDATE quark_posts SET good = !good WHERE id IN (:ids)", mapSqlParameterSource);
    }

    public int changeTopStatus(List<Integer> ids) {
        MapSqlParameterSource mapSqlParameterSource = new MapSqlParameterSource();
        mapSqlParameterSource.addValue("ids", ids);
        return this.getNamedParameterJdbcTemplate().update("UPDATE quark_posts SET top = !top WHERE id IN (:ids)", mapSqlParameterSource);
    }
    ...
    public int changeDeleteStatus(List<Integer> ids, boolean deleted) {
        ...
        return this.getNamedParameterJdbcTemplate().update("UPDATE quark_posts SET is_delete = :deleted WHERE is_delete != :deleted AND id IN (:ids)", mapSqlParameterSource);
    }

    public int changeHotStatus(List<Integer> ids) {
        MapSqlParameterSource mapSqlParameterSource = new MapSqlParameterSource();
        mapSqlParameterSource.addValue("ids", ids);
        return this.getNamedParameterJdbcTemplate().update("UPDATE quark_posts SET hot = !hot WHERE id IN (:ids)", mapSqlParameterSource);
    }
    ...
    public int countByLabelId(int labelId) {
        ...
        return this.getNamedParameterJdbcTemplate().queryForObject("SELECT count(*) FROM quark_posts WHERE label_id = :labelId AND is_delete = FALSE", mapSqlParameterSource, Integer.class);
    }
}
