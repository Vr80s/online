...
public class InformationDao extends SimpleHibernateDao {
	...
	public Page<InformationVo> findInformationPage(InformationVo informationVo, int pageNumber, int pageSize){
		   Map<String,Object> paramMap=new HashMap<String,Object>();
		   StringBuilder sql=new StringBuilder("select * from oe_information where is_delete = 0 ");
	        if(informationVo.getName()!=null&&!"".equals(informationVo.getName())){
	            sql.append(" and name like :name ");
	            paramMap.put("name","%"+informationVo.getName()+"%");
	        }
	        
	        if(informationVo.getInformationtype()!=null&&!"".equals(informationVo.getInformationtype())){
	            sql.append(" and informationType = :informationType ");
	            paramMap.put("informationType",informationVo.getInformationtype());
	        }
	        
	        if(informationVo.getStatus()!=null&&!"".equals(informationVo.getStatus())){
	            sql.append(" and status = :status ");
	            paramMap.put("status",informationVo.getStatus());
	        }
	        
		   sql.append(" order by status desc,sort desc");
		   Page<InformationVo> page = this.findPageBySQL(sql.toString(), paramMap, InformationVo.class, pageNumber, pageSize);
		   boolean flag = true;
		   for(int i=0;i<page.getItems().size();i++){
			   InformationVo info = page.getItems().get(i);
				String str=systemVariateService.getNameByValue("informationType", info.getInformationtype());
	 ...
	 public Integer getMaxSort(){
	        ...
	        return Integer.parseInt(result.get("maxSort") != null ? String.valueOf(result.get("maxSort")) : "1");
	    }
}
