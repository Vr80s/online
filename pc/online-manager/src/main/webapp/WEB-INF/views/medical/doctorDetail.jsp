...
<div class="page-header row">
	<div class="col-xs-8" style="padding-left: 0px; padding-right: 0px;">
		当前位置：医师管理 <small> <i class="ace-icon fa fa-angle-double-right"></i></small>
		<span> 医师管理 </span> <small> <i
			class="ace-icon fa fa-angle-double-right"></i></small> <span id="titleXQ">
			医师详情 </span>
	</div>
...
<ul class="nav nav-tabs" role="tablist">
	<%--<li role="presentation" ><a href="#home" role="tab" data-toggle="tab">医师介绍</a></li>--%>
	<li role="presentation" class="active"><a href="#profile"
		role="tab" data-toggle="tab">医师详情</a></li>
	<%--<li role="presentation"><a href="#messages" role="tab" data-toggle="tab">推荐医师</a></li>--%>
</ul>
<!-- Tab panes -->
			...
			<div class="col-xs-3" style="width: 20%">
				<div class="panel panel-default">
					<div class="panel-body">
						<button class="btn btn-sm btn-block" id="tjgsbt">添加故事标题</button>
					</div>
					<ul class="list-group" id="titleUl" style="margin: 0;"></ul>
				</div>
			</div>
			<div class="col-xs-9" id="ce">
				<script id="courseContentPreviewEdit" type="text/plain"
					style="width: 90%; height: 65%;"></script>
				<br> <input type="hidden" name="courseContentPreview"
					id="courseContentPreview"
					class="col-xs-10 col-sm-12 {required:true,minlength:1}">
				<div class="row">
					<div class="col-xs-4" style="text-align: right">
						<!-- 第一个复选框是控制单个故事的 暂时保留 -->
						<input type="checkbox" id="desStatusEdit" name="status"
							checked="checked" style="display: none">
						<!-- 控制全局展示 -->

						<input type="checkbox" id="descriptionShow" name="descriptionShow"
							style="cursor: pointer;"> <label for="descriptionShow"
							style="cursor: pointer;"> 设置在前台展示 </label>
					</div>
		...
		<div id="divKcxq">
			...
			<div id="detailDiv">
				<form class="form-horizontal" id="courseDetailForm" method="post"
					...
					<%-- 	<c:choose>
					   <c:when test="${mdaiId != null}">  
						  <input type="hidden" name="authenticationInformationId" " value="${mdaiId}"> 
					   </c:when>
				</c:choose> --%>
...
<div role="tabpanel" class="tab-pane" id="messages">
	<div id="divKctj">
		<div class="mainrighttab tabresourse bordernone" id="courseRecDiv">
			<p class="col-xs-4">
				<!-- <button class="btn btn-sm btn-success" title="新增推荐医师" onclick="addRecCourse()" style="margin-left: -10px">
					<i class="glyphicon glyphicon-plus"></i> 新增推荐医师
				</button> -->
			</p>
...
<div id="desDialog" class="hide">
	<form action="" method="post" class="form-horizontal" role="form"
		id="addDes-form" onkeydown="if(event.keyCode==13){return false;}">
		<input type="hidden" name="desCourseId" id="desCourseId"
			value="${param.courseId}">
		<div class="space-4"></div>
		<div class="form-group" style="margin-top: 18px;">
			<label class="col-sm-3 control-label no-padding-right"><i
				class="text-danger">*</i> 故事标题: </label>
			<div class="col-sm-9">
				<input type="text" name="courseTitle" id="courseTitle" maxlength="8"
					class="propertyValue1 col-xs-8 {required:true}">
			</div>
		</div>
	</form>
</div>
...
<div id="desDialogUpdate" class="hide">
	<form action="" method="post" class="form-horizontal" role="form"
		id="updateDes-form" onkeydown="if(event.keyCode==13){return false;}">
		<input type="hidden" name="id" id="update_desId"> <input
			type="hidden" name="status" id="update_status"> <input
			type="hidden" name="courseContent" id="update_courseContent">
		<input type="hidden" name="courseContentPreview"
			id="update_courseContentPreview">
		<div class="space-4"></div>
		<div class="form-group" style="margin-top: 18px;">
			<label class="col-sm-3 control-label no-padding-right"><i
				class="text-danger">*</i> 故事标题: </label>
			<div class="col-sm-9">
				<input type="text" name="courseTitle" id="update_courseTitle"
					maxlength="8" class="propertyValue1 col-xs-8 {required:true}">
			</div>
		</div>
	</form>
</div>
...
<div id="courseDialog" class="hide">
			...
			<div class="profile-info-row" style="float: left">
				...
				<table frame=void style="width: 100%">
					<tr>
						<!-- <td>
	                        <div class="profile-info-value searchTr">
	                            <input type="text" placeholder = "医师名称" class="propertyValue1" id="search_courseName" style="width: 150px;">
	                            <input type="hidden" value="search_courseName" class="propertyName"/>
	                        </div>
	                    </td> -->
						<td>
							<div class="profile-info-value searchTr">
								<select name="menuName" id="search_menu" value=""
									class="propertyValue1" style="width: 150px;">
									<option value="">学科</option>
									<c:forEach var="menus" items="${menuVo}">
										<option value="${menus.id}">${menus.name}</option>
									</c:forEach>
								</select> <input type="hidden" value="search_menu" class="propertyName" />
							</div>
						</td>
						<td>
							<div class="profile-info-value searchTr">
								<select name="courseTypeId" id="search_scoreType" value=""
									class="propertyValue1">
									<option value="">医师类别</option>
									<%-- <c:forEach var="scoreTypes" items="${scoreTypeVo}">
				                        <option value="${scoreTypes.id}">${scoreTypes.name}</option>
				                    </c:forEach> --%>
								</select> <input type="hidden" value="search_scoreType"
									class="propertyName" />
							</div>
						</td>
						<td>
							<div class="profile-info-value searchTr">
								<select name="search_courseType" id="search_courseType" value=""
									class="propertyValue1">
								...
								</select> <input type="hidden" value="search_courseType"
									class="propertyName" />
							</div>
						</td>
						<!-- 	                    <td>
 ...
 -->
						<td>
							...
							<div class="profile-info-value searchTr">
								<input type="hidden" value="1" id="search_status"
									name="search_status" class="propertyValue1" /> <input
									type="hidden" value="search_status" class="propertyName" />
							</div>
							<div class="profile-info-value searchTr">
								<input type="hidden" value="${param.courseId}"
									id="search_courseId" name="search_courseId"
									class="propertyValue1" /> <input type="hidden"
									value="search_courseId" class="propertyName" />
							</div>
						</td>
					</tr>
				</table>
		...
		<div class="row">
			...
			<div class="col-xs-1"
				style="padding: 0px; text-align: center; width: 4%; margin-top: 200px">
				<button type="button" class="btn btn-xs  btn-primary "
					id="ydRecCourse">→</button>
			</div>
			<div class="col-xs-3">
				<form action="" method="post" role="form" id="addRecCourse-form">
					<div class="panel panel-default" style="min-height: 388px">
						...
						<div class="panel-body"
							style="padding: 5px; border-bottom: 1px solid #DDDDDD;">
							<h4>已选择医师</h4>
						</div>
