...
$(function(){
    ...
    appraiseTable = initTables("appraiseTable", basePath + "/headline/appraise/list", objData,true, true, 0,null,searchCase,function(data){
...
function showAppraise(obj){
	...
	var dialog = openDialog("showAppraise","dialogShowAppraiseDiv","评价内容",480,300,true,"删除此评论",function(){
		ajaxRequest(basePath+"/headline/appraise/delete/"+aData.id,null,function(res){
			if(res.success){
...
function deleteRow(obj){
	...
	showDelDialog(function(){
		ajaxRequest(basePath+"/headline/appraise/delete/"+aData.id+"/"+aData.articleId,null,function(res){
			if(res.success){
				freshTable(appraiseTable);
			}
			//layerAlterInfo(res.errorMessage);
