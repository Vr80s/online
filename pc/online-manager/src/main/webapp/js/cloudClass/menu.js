...
//							   '<a class="blue" href="javascript:void(-1);" title="关联课程类别" onclick="openMenuManage(this)"><i class="glyphicon glyphicon-wrench"></i></a>';
//					   			if(row.yunStatus==1) {
//									buttons+='<a class="gray" href="javascript:void(-1);" title="禁用"><i class="ace-icon fa fa-ban bigger-130"></i></a> ';
//								}else{
//									buttons+='<a class="gray" href="javascript:void(-1);" title="启用"><i class="ace-icon fa fa-check-square-o bigger-130"></i></a> ';
//								};
//								return buttons;
//	            	   		}
	...
	cloudClassMenuTable = initTables("cloudClassTable",basePath+"/cloudClass/menu/list",objData,true,true,2,null,searchCase,function(data){
...
$(function(){
	 ...
	 /* 添加 */
      $(".add_bx").click(function(){
		  ajaxRequest(basePath+"/cloudClass/menu/maxSort",null,function(result){
			  $("input[name='code']").removeAttr("disabled");//增加角色把代码框放开
			  roleForm.resetForm();
			  $("#type").val($("#search_status").val());
...
function updateStatus(obj){
	...
	ajaxRequest(basePath+"/cloudClass/menu/updateStatus",{"id":aData.id,"status":aData.status},function(){
		freshTable(cloudClassMenuTable);
	});
}
...
function childrenMenusUpdateStatus(event,id,menuId,check){
	if($(event.target).is(':checked') == false){
			ajaxRequest(basePath+"/cloudClass/menu/removeMenuCourseType",{"id":id,"menuId":menuId},function(){
				//freshTable(cloudClassMenuTable);
		});
	}else if($(event.target).is(':checked') == true){
			ajaxRequest(basePath+"/cloudClass/menu/addMenuCourseType",{"id":id,"menuId":menuId},function(){
				//freshTable(cloudClassMenuTable);
		});
	}
...
function childrenMenusUpMove(id,courseCount){
	...
	ajaxRequest(basePath+"/cloudClass/menu/scoreTypeUp",{"id":id},function(res){
		ajaxRequest(basePath+"/cloudClass/menu/childMenu",{'id':parentId},function(data) {
			drawMenusPage(data,parentId,courseCount);
			if(courseCount==0){
				$("input:checkbox").removeAttr("disabled");
			}
			var dialog = openDialog("childMenuDialog", "childMenuDialogDiv", "关联课程类别",580,450, true, "确定", function(){
				$("input:checkbox").removeAttr("disabled");
				mask();
...
function childrenMenusDownMove(id,courseCount){
	var parentId =$("#parentId").val();
	ajaxRequest(basePath+"/cloudClass/menu/scoreTypeDown",{"id":id},function(res){
		ajaxRequest(basePath+"/cloudClass/menu/childMenu",{'id':parentId},function(data) {
			drawMenusPage(data,parentId,courseCount);
			if(courseCount==0){
				$("input:checkbox").removeAttr("disabled");
...
function upMove(obj){
	var oo = $(obj).parent().parent().parent();
	var aData = cloudClassMenuTable.fnGetData(oo);
	ajaxRequest(basePath+"/cloudClass/menu/up",{"id":aData.id},function(res){
		if(res.success){
			freshTable(cloudClassMenuTable);
		}
	});
...
function downMove(obj){
	var oo = $(obj).parent().parent().parent();
	var aData = cloudClassMenuTable.fnGetData(oo);
	ajaxRequest(basePath+"/cloudClass/menu/down",{"id":aData.id},function(res){
		if(res.success){
			freshTable(cloudClassMenuTable);
		}
	});
...
function openMenuManage(obj){
	...
	ajaxRequest(basePath+"/cloudClass/menu/childMenu",{'id':row.id},function(data) {
...
//		ajaxRequest(basePath+"/cloudClass/menu/addChildren",{'menuName':menuName,'parentId':pid,'rowId':rowId},function(data) {
//			if(data.success){
	...
	if(data.success){
		for(var i=0;i<data.resultObject.length;i++){
			...
			if(data.resultObject[i].checked=='true'){
				rowData+="<input style='margin-top:-1px;cursor: pointer;' type='checkbox' name='childMenuNames'  checked='checked' disabled='disabled' value='"+data.resultObject[i].id+"' id='childMenuNames_"+i+"' /></td><td><label style='cursor: pointer;' for='childMenuNames_"+i+"'>"+data.resultObject[i].name+"</label></td>";
			}else{
				rowData+="<input style='margin-top:-1px;cursor: pointer;' type='checkbox' name='childMenuNames'  value='"+data.resultObject[i].id+"' id='childMenuNames_"+i+"' /></td><td><label style='cursor: pointer;' for='childMenuNames_"+i+"'>"+data.resultObject[i].name+"</label></td>";
			}
			rowData+="</td>";
			rowData+="<td>";
			rowData+="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
...
function deleteBatch(){
	deleteAll(basePath+"/cloudClass/menu/deletes",cloudClassMenuTable);
}
 ...
 * 中医学习
 */
function scoreTypeTableZyxx(){
	$("#search_status").val("1");
	searchButton(cloudClassMenuTable);
}

/**
 * 身心修养
 */
function scoreTypeTableSxxy(){
	$("#search_status").val("2");
	searchButton(cloudClassMenuTable);
}
