...
$(function(){
    ...
    bannerTable = initTables("bannerTable", basePath + "/boxueshe/banner/list", dataFields, true, true, 1,null,searchJson,function(data){
...
function updateRec(obj){
	...
	ajaxRequest(basePath+"/boxueshe/banner/updateRec",{"id":row.id},function(data){
		if(data.success){
			layer.msg("取消成功！");
...
function updateRecImg(obj){
	...
	var dialog = openDialog("updateRecImgDialog","dialogUpdateRecImgDiv","设置banner图片",580,330,true,"确定",function(){
 		if($("#updateRecImg-form").valid()){
 			 mask();
 			 $("#updateRecImg-form").attr("action", basePath+"/boxueshe/banner/updateBannerPath");
...
function upMove(obj){
	...
	ajaxRequest(basePath+'/boxueshe/banner/upMove',{"id":aData.id},function(res){
		if(res.success){
			freshTable(bannerTable);
		}else{
...
function downMove(obj){
	var oo = $(obj).parent().parent().parent();
	var aData = bannerTable.fnGetData(oo);
	ajaxRequest(basePath+'/boxueshe/banner/downMove',{"id":aData.id},function(res){
		if(res.success){
			freshTable(bannerTable);
		}else{
			layer.msg(res.errorMessage);
