...
$(function(){
	...
	tagTable = initTables("tagTable",basePath+"/boxueshe/tag/findTagList",objData,true,true,0,null,searchCase,function(data){
...
$(".add_bx").click(function(){
	...
	var dialog = openDialog("addTagDialog","dialogAddTagDiv","新增标签",580,200,true,"确定",function(){
		if($("#addTag-form").valid()){
			mask();
			 $("#addTag-form").attr("action", basePath+"/boxueshe/tag/addTag");
...
function deleteBatch(){
	...
	deleteAll(basePath+"/boxueshe/tag/deletes",tagTable);
}
...
function updateStatus(obj){
	...
	var aData = tagTable.fnGetData(oo);
	ajaxRequest(basePath+"/boxueshe/tag/updateStatus",{"id":aData.id},function(){
		freshTable(tagTable);
	});
}
...
function toEdit(obj){
	...
	openDialog("editTagDialog","dialogEditTagDiv","修改标签",580,200,true,"提交",function(){
		if($("#update-form").valid()){
			mask();
			$("#update-form").attr("action",basePath+"/boxueshe/tag/update");
