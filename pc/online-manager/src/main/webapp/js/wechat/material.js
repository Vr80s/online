...
$(function(){
    ...
    { "title": "礼物名称", "class":"center","width":"15%","sortable":false,"data": 'name' ,"mRender":function (data, display, row) {
    		return "<span name='courseNameList'>"+data+"</span>";
    } },
    {title: '缩略图', "class": "center", "width": "144px","height":"38px","data": 'smallimgPath', "sortable": false,"mRender":function(data,display,row){
    	return "<img src='"+data+"' style='width:30px;height:30px;cursor:pointer;'/>";
    }},
	...
	_courseTable = initTables("courseTable",basePath+"/gift/list",objData,true,true,2,null,searchCase,function(data){
...
//	debugger;
	...
	//是否可以连击
	if(row.isContinuous == 1){ //本站
		$("#this_web_edit").attr('checked',true);
		$("#other_web_edit").removeAttr('checked');
	...
	}else if(row.isContinuous == 0){//外站
		$("#other_web_edit").attr('checked',true);
		$("#this_web_edit").removeAttr('checked');
	...
	$("#continuousCount_edit").val(row.continuousCount);//连击数
...
function updateStatus(obj){
	...
	var oo = $(obj).parent().parent().parent();
	var row = _courseTable.fnGetData(oo); // get datarow
	ajaxRequest(basePath+"/gift/updateStatus",{"id":row.id},function(data){
		console.log(data);
...
$(".dele_bx").click(function(){
	deleteAll(basePath+"/gift/deletes",_courseTable);
});

$(".fencheng_bx").click(function(){
	setBrokerage(basePath+"/gift/deletes",_courseTable);
});
//设置平台分成
...
function upMove(obj){
	...
	ajaxRequest(basePath+'/gift/upMove',{"id":aData.id},function(res){
		if(res.success){
			freshTable(_courseTable);
		}else{
...
function downMove(obj){
	var oo = $(obj).parent().parent().parent();
	var aData = _courseTable.fnGetData(oo);
	ajaxRequest(basePath+'/gift/downMove',{"id":aData.id},function(res){
		if(res.success){
			freshTable(_courseTable);
		}else{
			layer.msg(res.errorMessage);
