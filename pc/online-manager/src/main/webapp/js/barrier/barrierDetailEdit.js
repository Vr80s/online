$(function() {
	...
	//初始化值
	ajaxRequest(basePath + "/barrier/barrier/getBarrierById",{"id":$("#id").val()},function(data){
		if(data.success){
			var results = data.resultObject;
			$("#name").val(results.name);
			$("#wordCnt").text(results.name.split("").length);
			$("#parentId").val(results.parentId);
			$("#limitTime").val(results.limitTime);
			$("#totalScore").val(results.totalScore);
			$("#passScorePercent").val(results.passScorePercent);
			$("#kpointIds").val(results.kpointIds);
			$("#oldKpointIds").val(results.kpointIds);
			$("#selectionKpoints").text(results.kpointNames);
			$("#selectionKpoints").attr("title",results.kpointNames);
			var strategyCnt = results.strategyCnt;
			var strategyTotalScore = results.strategyTotalScore;
			var strategyScore = results.strategyScore;
			var strategyQuestionType = results.strategyQuestionType;
			
			for(var i = 0;i<strategyCnt.length;i++){
				$("input[name='strategyCnt']").eq(strategyQuestionType[i]).val(strategyCnt[i]);
				$("input[name='strategyTotalScore']").eq(strategyQuestionType[i]).val(strategyTotalScore[i]);
				$("input[name='strategyScore']").eq(strategyQuestionType[i]).val(strategyScore[i]);
			}
			
			$("#totalScoreSpan").text(results.totalScore);
			changeInfo($("input[name='strategyCnt']").eq(0));//计算已分配和未分配
...
//								$("#nonQuestionCnt").val(0);
	...
	function getBarriersSelect(){
		ajaxRequest(basePath+"/barrier/barrier/getBarriersSelect",{"courseId":$("#courseId").val(),"id":$("#id").val()},function(data){
			if(data.success){
				//获取到结果添加到下拉框中
				var results = data.resultObject;
				for(var i = 0;i<results.length;i++){
