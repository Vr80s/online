...
window.onload = function() {
	...
	var myAnswer =
		...
		'{{if items.accused == true&&items.handleStatus!=2}}' +
		'<span class="complaint ansTopRight qingjian block" data-id="{{items.id}}">' +
		'<img src="../images/ansandqus/delete_normal.png"/>删除</span><span class="fengexian"></span>' +
		'<span class="complaint ansTopRight mark"  data-type="0" data-id="{{items.id}}">' +
		'标记处理</span><span class="fengexian"></span>' +
		'{{/if}}'+
		'{{if items.handleStatus==2}}' +
		'<span class="complaint ansTopRight remark"  data-type="0" data-id="{{items.id}}">' +
		'已标记处理</span><span class="fengexian"></span>' +
		'{{/if}}'+
		'{{if items.accused == true}}' +
		'<span class="complaint ansTopRight colorgren ts" style="cursor: auto;color:#2cb82c" data-type="0">' +
		'<img src="../images/ansandqus/tousu_hover.png" style="cursor: auto"/>已投诉</span><span class="fengexian"></span>' +
		'{{/if}}' +
...
//		'{{else}}' +
		'<span data-id="{{$value.id}}" class="ts" style="cursor: auto"><img style="cursor: auto" src="../images/ansandqus/tousu_hover.png"/><span style="cursor: auto" class="complaint colorgren" data-type="1">已投诉</span></span>' +
		'{{/if}}' +
		'{{if $value.handleStatus==2}}' +
		'<span class="remark"  data-type="1" data-id="{{$value.id}}">' +
		'已标记处理</span>' +
		'{{/if}}'+
		'{{if $value.handleStatus==0}}'+
		'<span class="mark"  data-type="1" data-id="{{$value.id}}">' +
		'标记处理</span>' +
		'{{/if}}'+
		...
		'{{/if}}' +
		'</span>' +
		'{{if $value.accused == true&&$value.handleStatus!=2}}' +
		'<span class="complaint ansTopRight qingjian1 block" data-delectId="{{$value.id}}" data-create="{{$value.create_person}}">' +
...
//		'<div class="comment-btn">评论</div>' +
	...
	template.helper('glhref', function(num) {
		if(num != "") {
			return 'javascript:;';
//			return '' + bath + '/web/html/qusAndAnsDetailTouSu.html?qid=' + num;
		} else {
			return 'javascript:;';
		}
	});
	var simliar = ' <p class="rel-course">相似问题</p>' +
		...
		'<a href="{{#glhref(i.id)}}"  class="similarHover"><p class="rel-content" title="{{i.title}}">{{i.title}}</p></a>' +
		'</div>' +
	...
	function mark(){
		$(".mark").off().on("click",function(){
			var $this=$(this);
			RequestService("/online/menutag/updateAccuseStatus","get",{
				target_type: $this.attr("data-type"),
				target_id: $this.attr("data-id")
			},function(data){
				if(data.resultObject=="true"){
					$this.removeClass("mark").addClass("remark");
					$this.html("已标记处理");
					$this.parent().children(".block").next(".fengexian").remove();
					$this.parent().children(".ts").next(".fengexian").remove();
					$this.parent().children(".ts").remove();
					$this.parent().children(".block").remove();
					$(".mark").each(function(){
			...
			$(".answer-comment").off().on("click", function() {
				...
				function goAnswers() {
					RequestService('/ask/comment/findComments', 'GET', answerList, function(data) {
						...
						$(".reply-delete").unbind().click(function() {
							var $this = $(this);
							var $grfa = $this.parent().parent().parent().parent().parent();
							var datasums = $grfa.children(".answer-comment").attr("data-sum");
