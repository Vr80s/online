window.onload = function() {
		if(window.sessionStorage.aboutus) {
			...
			$("." + window.sessionStorage.aboutus).addClass("active").siblings().removeClass("active");
			if(window.sessionStorage.aboutus=="three"){
				initMap() //创建和初始化地图
			}
		} else {
		...
		$(".left-nav li").on("click", function() {
			...
			$(".aboutus-nav-2").text($(this).find("a").text());
			window.sessionStorage.aboutus = $(this).find("a").attr("data-id");
			if(window.sessionStorage.aboutus=="three"){
				initMap() //创建和初始化地图
			}
		})
...
function createMap() {
	...
	var point = new BMap.Point(110.556820,19.948380); //定义一个中心点坐标
...
var markerArr = [{
	title: "我的位置",
	content: "海南省海口市美兰区演丰镇瑶城一路D栋心承智慧大厦",//19.9502500000,110.5556500000
	point: "110.555650|19.950250",
	isOpen: 1,
	icon: {
		w: 21,
		h: 21,
...
function createIcon(json) {
	var icon = new BMap.Icon("../images/aboutus/location.png", new BMap.Size(json.w, json.h), {
