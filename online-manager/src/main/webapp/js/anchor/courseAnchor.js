...
$(function () {
    ...
    $(".rec_P").click(function () {
        ...
        if (ids.length > 0) {
            ajaxRequest(basePath + "/anchor/courseAnchor/updateRec", {
                'ids': ids.join(","),
                "isRec": 1
            }, function (data) {
...
function loginBusiness(obj) {
    ...
    var oo = $(obj).parent().parent().parent();
    var row = P_courseTable.fnGetData(oo);
    var businessNames = row.businessNames;
    if (businessNames && businessNames.length == 1) {
        window.open(basePath + '/anchor/courseAnchor/loginBusiness/' + row.businessNames[0], '_blank');
    } else {
        var $JUsername = $('.J-username');
        $JUsername.html('');
        var tmpl = '';
        for (var i = 0; i < businessNames.length; i++) {
            tmpl = tmpl + '<option value="' + businessNames[i] + '">' + businessNames[i] + '</option>';
        }
        $JUsername.html(tmpl);
        var dialog = openDialog("LoginBusinessDialog", "dialogLoginBusinessDiv", "登陆商家后台", 350, 200, true, "确定", function () {
            mask();
            window.open(basePath + '/anchor/courseAnchor/loginBusiness/' + $JUsername.val(), '_blank');
            unmask();
            $("#LoginBusinessDialog").dialog("close");
        });
    }
}
