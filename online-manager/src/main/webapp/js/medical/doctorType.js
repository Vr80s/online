...
function loadBanner2List(){
    ...
    var dataFields = [
        ...
        {title: '序号', "class": "cnter", "width": "5%","height":"68px","data": 'id',datafield: 'xuhao', "sortable": false},
        {title: '医师类别名称', "class": "center","width":"15%","data": 'title', "sortable": false},
        { "title": "医师数量", "class":"center","width":"8%","sortable":false,"data": 'doctorNumber' },
        { "title": "创建人", "class":"center","width":"8%","sortable":false,"data": 'createPerson' },
		{ "title": "创建日期", "class":"center","width":"8%","sortable":false,"data": 'createTime' },
    ...
    banner2Table = initTables("banner2Table", basePath + "/medical/doctorType/list", dataFields, true, true, 2,null,searchJson,function(data){
    ...
    $(".zykgl_bx").click(function(){
        ...
        $("#adddoctorType").val(banneType);
        $("#updatedoctorType").val(banneType);
...
//新增框
 $(".add_bx").click(function(){
 	...
 	var dialog = openDialog("addBanner2Dialog","dialogAddBanner2Div","新增医师类别",500,330,true,"确定",function(){
 		if($("#addBanner2-form").valid()){
 			 mask();
 			 $("#addBanner2-form").attr("action", basePath+"/medical/doctorType/add");
	...
	$(".remove").hide();
 	...
 	var dialog = openDialog("updateBanner2Dialog","dialogUpdateBanner2Div","修改医师类别",500,330,true,"确定",function(){
 		if($("#updateBanner2-form").valid()){
 			 mask();
 			 $("#updateBanner2-form").attr("action", basePath+"/medical/doctorType/update");
...
function updateStatus(obj){
	var oo = $(obj).parent().parent().parent();
	var row = banner2Table.fnGetData(oo); // get datarow
	ajaxRequest(basePath+"/medical/doctorType/updateStatus",{"id":row.id},function(data){
...
$(".dele_bx").click(function(){
 	deleteAll(basePath+"/medical/doctorType/deletes",banner2Table);
});
...
function upMove(obj){
	...
	ajaxRequest(basePath+'/medical/doctorType/upMove',{"id":aData.id},function(res){
		if(res.success){
			freshTable(banner2Table);
		}else{
...
function downMove(obj){
	var oo = $(obj).parent().parent().parent();
	var aData = banner2Table.fnGetData(oo);
	ajaxRequest(basePath+'/medical/doctorType/downMove',{"id":aData.id},function(res){
		if(res.success){
			freshTable(banner2Table);
		}else{
			layer.msg(res.errorMessage);
