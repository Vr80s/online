...
//				'<a class="blue" href="javascript:void(-1);" id="moveDown" title="下移" onclick="downMove(this)" name="downa"><i class="glyphicon glyphicon-arrow-down bigger-130"></i></a></div>';
	...
	typeTable = initTables("typeTable", basePath + "/headline/articletype/types", dataFields, true, true, 0,null,searchJson,function(data){
...
$(".add_type").click(function(){
	...
	openDialog("addDialog", "addTypeDiv", "新增分类", 350, 200, true, "确定", function () {
		if ($("#add-form").valid()) {
			mask();
			$("#add-form").attr("action", basePath + "/headline/articletype/addType");
...
function updateArticleType(obj) {
	...
	openDialog("updateDialog", "updateTypeDiv", "修改分类", 350, 200, true, "确定", function () {
		if ($("#update-form").valid()) {
			mask();
			$("#update-form").attr("action", basePath + "/headline/articletype/updateTypeById");
...
$(".del_batch").click(function(){
 	deleteAll(basePath+"/headline/articletype/deletes",typeTable);
});
...
function updateStatus(obj){
	var oo = $(obj).parent().parent().parent();
	var row = typeTable.fnGetData(oo);
	ajaxRequest(basePath+"/headline/articletype/updateStatus",{"id":row.id},function(res){
		if(res.success) {
...
function upMove(obj){
	...
	ajaxRequest(basePath+'/headline/articletype/upMove',{"id":aData.id},function(res){
		if(res.success){
			layer.msg(res.resultObject);
			freshTable(typeTable);
		}else{
...
function downMove(obj){
	var oo = $(obj).parent().parent().parent();
	var aData = typeTable.fnGetData(oo);
	ajaxRequest(basePath+'/headline/articletype/downMove',{"id":aData.id},function(res){
		if(res.success){
			layer.msg(res.resultObject);
			freshTable(typeTable);
		}else{
