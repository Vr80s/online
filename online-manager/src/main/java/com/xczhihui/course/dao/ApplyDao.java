...
public class ApplyDao extends HibernateDao<Apply> {
	public Page<ApplyVo> findApplyPage(ApplyVo vo, int pageNumber, int pageSize) {
		...
		// if (StringUtils.isNotEmpty(vo.getIsPayment()) &&
		// !"-1".equals(vo.getIsPayment())) {
		...
		// }
		return this.findPageBySQL(sql.toString(), paramMap, ApplyVo.class,
				pageNumber, pageSize);
	}
	...
	public void deleteStudents(String[] _ids) {
		...
		if (_ids.length > 0) {
			StringBuilder sql = new StringBuilder(
					" delete from apply_r_grade_course  where  id IN (");

			// StringBuilder findByIdSql = new
			// StringBuilder(" select apply_id as id from apply_r_grade_course  where  id IN (");
			 ...
			 * OnlineUser OnlineById =
			 * this.findOneEntitiyByProperty(OnlineUser.class,
			 * "id",applyById.getUserId()); OnlineById.setIsApply(false);
			 * this.update(OnlineById); this.delete(apply.getId(), entityClass);
			 * }
			 */
