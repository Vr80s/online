...
public class BarrierController {
	...
	public TableVo findBarrierList(TableVo tableVo) {
		...
		Page<BarrierVo> page = barrierService.findBarrierPage(searchVo,
	...
	public TableVo findCourseList(TableVo tableVo) {
		...
		Page<CourseVo> page = barrierService.findCoursePage(searchVo,
				currentPage, pageSize);
		int total = page.getTotalCount();
		tableVo.setAaData(page.getItems());
		tableVo.setiTotalDisplayRecords(total);
	...
	@ResponseBody
	public ResponseObject addBarrier(BarrierVo barrierVo,
		...
		String[] strategyTotalScore = request
				.getParameterValues("strategyTotalScore");// 每个题型多少分
	...
	public ResponseObject getBarriersSelect(String courseId, String id) {
		ResponseObject responseObject = new ResponseObject();
		responseObject.setResultObject(barrierService.getBarriersSelect(
				courseId, id));
		responseObject.setSuccess(true);
		return responseObject;
	}
