 ...
 */
@Service
public class DoctorServiceImpl extends OnlineBaseServiceImpl implements
        DoctorService {
    ...
    public void updateMedicalDoctor(MedicalDoctor medicalDoctor) {
        ...
        // dao.findOneEntitiyByProperty(MedicalDoctor.class, "id",
        // medicalDoctorvo.getId());
    ...
    @Override
    public void updateMedicalDoctorDetail(String medicalDoctorId,
        ...
        // 根据医师id获取医师详情
        MedicalDoctor doctor = dao.findOneEntitiyByProperty(
                MedicalDoctor.class, "id", medicalDoctorId);
        if (doctor == null) {
        ...
        if (!StringUtils.isNotBlank(authenticationInformationId)) {
            ...
            mdai.setCreatePerson(ManagerUserUtil.getId());
            mdai.setCreateTime(new Date());
        } else {

            mdai = dao.findOneEntitiyByProperty(
                    MedicalDoctorAuthenticationInformation.class, "id",
                    authenticationInformationId);
            mdai.setUpdatePerson(ManagerUserUtil.getId());
    ...
    public List<MedicalDoctor> findByName(String name) {
        List<MedicalDoctor> MedicalDoctors = dao.findEntitiesByProperty(
                MedicalDoctor.class, "name", name);
    ...
    public MedicalDoctorAuthenticationInformation mdaiDetail(String mdaiId) {
        return dao.findOneEntitiyByProperty(
                MedicalDoctorAuthenticationInformation.class, "id", mdaiId);
    }
     ...
     */
    @Override
    public void updateMedicalHospitalDoctorDetail(String doctorId,
                                                  String hospitalId) {

        // 根据医师id获取医师详情
        List<MedicalDoctor> doctors = dao.findEntitiesByProperty(
                MedicalDoctor.class, "id", doctorId);
        ...
        // 删除之前关系表
        List<MedicalHospitalDoctor> mhps = dao.findEntitiesByProperty(
                MedicalHospitalDoctor.class, "doctorId", doctorId);
    ...
    public List<MedicalHospital> getMedicalHospital(String doctorId) {
        ...
        List<MedicalHospitalDoctor> mhds = dao.findEntitiesByProperty(
                MedicalHospitalDoctor.class, "doctorId", doctorId);
    ...
    public Integer updateRec(String[] ids, int isRecommend) {
        ...
        Integer code = 1000; // code：1000 操作成功! code：1001 最多设置十个推荐医师! code：1002
        // 你所选中的被推荐人已是被推荐!
        ...
        if (isRecommend == 1) {// 如果是要推荐 那么就验证 推荐数量是否大于4
            ...
            // 已经存在的数量 + 即将添加的数量
            /*
             * if((list.size()+ids2.size()) > 10){ return 1001; }
			 */
    ...
    public Page<MedicalDoctor> findRecMedicalDoctorPage(
        ...
        Page<MedicalDoctor> page = doctorDao.findRecMedicalDoctorPage(
                medicalDoctor, currentPage, pageSize);
        return page;
    }
    ...
    public List<MedicalHospital> getMedicalHospitals() {
        ...
        List<MedicalHospital> voList = dao.findEntitiesByJdbc(
                MedicalHospital.class, sql, null);
        return voList;
    }
    ...
    public void updateSortUpRec(String id) {
        ...
        MedicalDoctor courseNext = dao.findByHQLOne(hqlNext,
                new Object[]{id});
    ...
    public List<MedicalDoctor> findAllDoctor(String id) {
        ...
        List<MedicalDoctorAuthorArticle> mdaa = dao.findEntitiesByProperty(
                MedicalDoctorAuthorArticle.class, "articleId", id);
    ...
    public void addDoctorAuthorArticle(String id, String[] doctor) {
        List<MedicalDoctorAuthorArticle> mdaas = dao.findEntitiesByProperty(
                MedicalDoctorAuthorArticle.class, "articleId", id);
    ...
    public void addDoctorReport(String id, String[] doctor) {
        List<MedicalDoctorReport> mdaas = dao.findEntitiesByProperty(
                MedicalDoctorReport.class, "articleId", id);
        for (int i = 0; i < mdaas.size(); i++) {
            dao.delete(mdaas.get(i));
        }
        if (doctor != null) {
    ...
    public List<MedicalDoctor> allListForReport(String articleId) {
        List<MedicalDoctor> list = list();
        List<MedicalDoctorReport> mdr = dao.findEntitiesByProperty(
                MedicalDoctorReport.class, "articleId", articleId);
    ...
    public List<MedicalDoctor> getMedicalDoctor(String writingsId) {
        // TODO Auto-generated method stub
        List<MedicalDoctor> list = list();
        List<MedicalDoctorWritings> mdr = dao.findEntitiesByProperty(
                MedicalDoctorWritings.class, "writingsId", writingsId);
        for (int i = 0; i < mdr.size(); i++) {
            for (int j = 0; j < list.size(); j++) {
                if (mdr.get(i).getDoctorId().equals(list.get(j).getId())) {
                    list.get(j).setHas(true);
    ...
    public List<String> updateDefaultSort() {
        List<String> updateRecommendIds = getUpdateRecommendId();
        updateRecommendIds.forEach(id -> updateDefaultSort(id));
        return updateRecommendIds;
    }

    public void updateDefaultSort(String id) {
        String sql = " UPDATE  medical_doctor  SET recommend_sort=0 WHERE id=:id";
    ...
    @Override
    public List<String> getUpdateRecommendId() {
        String sql = "SELECT id FROM medical_doctor WHERE sort_update_time<= NOW() AND recommend_sort != 0";
        List<String> ids = dao.getNamedParameterJdbcTemplate().getJdbcOperations()
