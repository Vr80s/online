package com.xczhihui.common.support.dao;
...
public class SimpleHibernateDao implements ApplicationContextAware {
    ...
    public <T> void saveOrUpdate(final List<T> entities) {
        this.getHibernateTemplate().executeWithNativeSession(new HibernateCallback<T>() {
    ...
    public <T> List<T> getAll(Class<T> clazz) {
        return (List<T>) this.getHibernateTemplate().getSessionFactory().getCurrentSession().createCriteria(clazz).list();
    }
    ...
    public <T> List<T> findEntities(DetachedCriteria detachedCriteria, final int limit, final int count) {
        return (List<T>) this.getHibernateTemplate().findByCriteria(detachedCriteria, limit, count);
    }

    /**
     * 通过HQL查询实体。
     *
     * @param hql    如:from User where name=? and age=?
     * @param params 与hql中？对应的参数值
     * @return
     */
    @SuppressWarnings("unchecked")
    ...
    public int queryForInt(String sql, Object... args) {
        Number number = this.getNamedParameterJdbcTemplate().getJdbcOperations().queryForObject(sql, args, Integer.class);
        return (number != null ? number.intValue() : 0);
    }

    public Double queryForDouble(String sql, Object... args) {
        Number d = this.getNamedParameterJdbcTemplate().getJdbcOperations().queryForObject(sql, args, Double.class);
     ...
     *
     * @param criteria
     * @param pageNumber 页码(表示第几页，从1开始)
     * @param pageSize
     * @return
     */
    @SuppressWarnings("unchecked")
     ...
     * 分页通过HQL查询实体。
     *
     * @param hql        如：from TestBean where name=:name
     * @param paramMap   查询条件变量名和值,如：paramMap.put("name","张三")
     * @param pageNumber 页码(表示第几页，从1开始)
    ...
    public <T> Page<T> findPageByHQL(final String hql, final Map<String, Object> paramMap, final int pageNumber,
        ...
        List<T> list = this.getHibernateTemplate().executeWithNativeSession(new HibernateCallback<List<T>>() {
            @Override
     ...
     * 分页通过SQL查询实体。
     *
     * @param sql         如：select * from test_bean where name=:name
     * @param paramMap    如：paramMap.put("name","11");
     * @param elementType 如：TestBean.class
     * @param pageNumber  页码(表示第几页，从1开始)
     * @param pageSize
     * @return
     */
    public <T> Page<T> findPageBySQL(String sql, Map<String, Object> paramMap, Class<T> elementType, int pageNumber,
        ...
        int totalCount = this.getNamedParameterJdbcTemplate().queryForObject(totalSql, paramMap, Integer.class);
        ...
        List<T> list = this.getNamedParameterJdbcTemplate().query(querySql, paramMap,
     ...
     * @param sql      如:select * from user where name=:name and age=:age
     * @param paramMap 参数，key为sql中的参数名如:name,age
     * @return
     */
    public <T> List<T> findEntitiesByJdbc(Class<T> clazz, String sql, Map<String, ?> paramMap) {
        return this.getNamedParameterJdbcTemplate().query(sql, paramMap, BeanPropertyRowMapper.newInstance(clazz));
    }

    public <T> T findEntityByJdbc(Class<T> clazz, String sql, Map<String, ?> paramMap) {
        return this.getNamedParameterJdbcTemplate().queryForObject(sql, paramMap, BeanPropertyRowMapper.newInstance(clazz));
    }
    ...
    protected int getTotal(String queryStr, final List<String> paramList, final List<Object> valueList) {
        ...
        List result = this.getHibernateTemplate().findByNamedParam(countStr, list2Array(paramList), valueList.toArray());
